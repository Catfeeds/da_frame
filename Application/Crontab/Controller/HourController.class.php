...
class HourController extends BaseCronController {
    /**
     * 执行频率常量 1小时
     * @var int
     */
        ...
        try {
            ...
            if ($count != 0) {
                for ($i = 0; $i <= $count; $i = $i + $step_num){
                    if (C('dbdriver') == 'mysqli') {
                        $goods_list = $model_goods->getGoodsOnlineList(array(), '*,'.$_field.' spd_distinct', 0, '', "{$i},{$step_num}", $_distinct);
                    }
    ...
    public function _xs_update() {
        ...
        try {
            ...
            for ($i = 0; $i <= $count; $i = $i + $step_num){
                if (C('dbdriver') == 'mysqli') {
                    $goods_list = $model_goods->getGoodsOnlineList($condition, '*,'.$_field.' spd_distinct', 0, '', "{$i},{$step_num}", $_distinct);
                }
