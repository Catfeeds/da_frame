...
class TaskController extends BaseCronController
{
	...
	//$task_pwd = md5(trim($task_pwd));
        ...
        while (list($key, $arr) = each($task)) {
            ...
            if ($isplay) {
            	$getString = trim(htmlspecialchars_decode(($arr['parameter'])));
            	$dourl = trim(htmlspecialchars_decode($arr['dourl']));
 
            	Model()->table('task')->where(array('id'=> $arr['id']))->update(array('lastrun'=>$ntime,'sta'=>'运行'));
            	if ($getString != '') {
            		$dourl = BASE_SITE_URL . '?' . $dourl . '&' . $getString;
            	} else {
            		$dourl = BASE_SITE_URL . '?' . $dourl;
            	}
