...
class CMSController extends Controller {
	...
	const PICTURE = 2;
	//用户中心文章列表页
	const CMS_MEMBER_ARTICLE_URL = 'index.php?m=Cms&c=MemberArticle&a=article_list';
	const CMS_MEMBER_PICTURE_URL = 'index.php?m=Cms&c=MemberPicture&a=picture_list';
	...
	public function __construct(){
		...
		*/
		 ...
		 * seo
		*/
		$this->assign('html_title',C('cms_seo_title').'-'.C('site_name').'');
		$this->assign('seo_keywords',C('cms_seo_keywords'));
		$this->assign('seo_description',C('cms_seo_description'));
	...
	protected function echo_json($data) {
		...
		$data['member_info'] = array("member_id" => $_SESSION['member_id'], 
				"member_name" => $_SESSION['member_name'],
				"is_login" => empty($_SESSION['member_id']) ? 0 : 1,
		);
	...
	protected function get_share_app_content($info) {
		...
		<dd>{$info['comment']}<a target=\"_blank\" href=\"{$info['url']}\">".Language::get('spd_common_goto')."</a></dd>
		</dl>
				</div>
				</div>
				";
