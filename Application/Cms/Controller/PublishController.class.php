...
class PublishController extends CMSMemberController{
     ...
     * 文章发布
     */
    public function publish_article_save(){
        ...
        if($article_id) {
            showMessage(Language::get('spd_common_save_succ'),self::CMS_MEMBER_ARTICLE_URL);
        } else {
            showMessage(Language::get('spd_common_save_fail'),self::CMS_MEMBER_ARTICLE_URL,'','error');
        }
    }
     ...
     * 画报发布
     */
    public function publish_picture(){
     ...
     * 画报保存
     */
    public function publish_picture_save() {
        ...
        if($picture_id) {
            showMessage(Language::get('spd_common_save_succ'),self::CMS_MEMBER_PICTURE_URL);
        } else {
            showMessage(Language::get('spd_common_save_fail'),self::CMS_MEMBER_PICTURE_URL,'','error');
        }
     ...
     * 文章图片上传
     */
    public function article_image_upload() {
        $data = array();
        $data['status'] = 'success';
     ...
     * 文章图片删除
     */
    public function article_image_drop() {
        $data = array();
        $data['status'] = 'success';
