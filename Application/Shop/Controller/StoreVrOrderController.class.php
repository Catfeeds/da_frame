...
class StoreVrOrderController extends BaseSellerController {
    ...
    public function index() {
        ...
        $this->assign('show_page',$model_vr_order->showpage());
        self::profile_menu('list',$_GET['state_type']);

        $this->render('store_vr_order.index');
    }
    ...
    public function show_order() {
        ...
        $this->render('store_vr_order.show');
    }
    ...
    private function _order_cancel($order_info, $post) {
        if(!chksubmit()) {
            ...
            $this->assign('order_info',$order_info);
            $this->render('store_vr_order.cancel','null_layout');
            exit();
        } else {
            $model_vr_order = Model('vr_order');
    ...
    public function exchange() {
        if (chksubmit()) {
            ...
            $this->render('store_vr_order.exchange');
        }
    }
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
            $menu_array = array(
            array('menu_key'=>'store_order',        'menu_name'=>Language::get('spd_member_path_all_order'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder'),
            array('menu_key'=>'state_new',          'menu_name'=>Language::get('spd_member_path_wait_pay'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=index&state_type=state_new'),
            array('menu_key'=>'state_pay',          'menu_name'=>'已付款',  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=index&state_type=state_pay'),
            array('menu_key'=>'state_success',      'menu_name'=>Language::get('spd_member_path_finished'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=index&state_type=state_success'),
            array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('spd_member_path_canceled'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=index&state_type=state_cancel'),
            );
            break;
            case 'exchange':
                $menu_array = array(
                ...
                array('menu_key'=>'state_success',      'menu_name'=>Language::get('spd_member_path_finished'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=index&state_type=state_success'),
                array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('spd_member_path_canceled'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=index&state_type=state_cancel'),
                array('menu_key'=>'exchange',           'menu_name'=>'兑换码兑换',  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrOrder&a=exchange'),
                );
                break;
        }
