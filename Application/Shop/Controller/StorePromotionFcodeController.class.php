...
class StorePromotionFcodeController extends BaseSellerController {
    ...
    public function fcode_list() {
        ...
        if (checkPlatformStore()) {
            ...
            $this->assign('fcode_quota', $fcode_quota);
        ...
        $this->render('store_promotion_fcode.goods_list');
    }

    /**
     * 购买套餐
     */
    public function fcode_quota_add() {
        if (chksubmit()) {
        ...
        $this->render('store_promotion_fcode.quota_add');
    }

    /**
     * 套餐续费
     */
    public function fcode_renew() {
        if (chksubmit()) {
            $model_fcode = Model('p_fcode');
            $quantity = intval($_POST['fcode_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_fcode_price')); // 扣款数
     ...
     * 选择商品
     */
    public function fcode_select_goods() {
        ...
        $this->render('store_promotion_fcode.select_goods', 'null_layout');
    }
    ...
    public function choosed_goods() {
        ...
        if (empty($goods_info) || $goods_info['store_id'] != $_SESSION['store_id'] || !$model_goods->checkIsGeneral($goods_info)) {
            ...
            $this->assign('error', '参数错误，或该商品已经添加过活动');
        }
        if (chksubmit()) {
        ...
        $this->assign('fcode_info', $fcode_info);
        ...
        $this->render('store_promotion_fcode.choosed_goods', 'null_layout');
    }
