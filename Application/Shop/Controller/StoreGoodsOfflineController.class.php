...
class StoreGoodsOfflineController extends BaseSellerController {
    ...
    public function goods_storage() {
        ...
        switch ($_GET['type']) {
            // 违规的商品
            case 'lock_up':
                $this->render('store_goods_list.offline_lockup');
                break;
            // 等待审核或审核失败的商品
            case 'wait_verify':
                $this->assign('verify', array('0' => '未通过', '10' => '等待审核'));
                $this->render('store_goods_list.offline_waitverify');
                break;
            // 仓库中的商品
            default:
                $this->render('store_goods_list.offline');
                break;
        }
    }
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = array(
            array('menu_key' => 'goods_storage',    'menu_name' => L('spd_member_path_goods_storage'),   'menu_url' => urlShop('store_goods_offline', 'index')),
            array('menu_key' => 'goods_lockup',     'menu_name' => L('spd_member_path_goods_state'),     'menu_url' => urlShop('store_goods_offline', 'index', array('type' => 'lock_up'))),
            array('menu_key' => 'goods_verify',     'menu_name' => L('spd_member_path_goods_verify'),    'menu_url' => urlShop('store_goods_offline', 'index', array('type' => 'wait_verify')))
        );
