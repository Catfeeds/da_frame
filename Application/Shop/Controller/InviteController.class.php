...
class InviteController extends BaseHomeController{
	public function index(){
		$myurl=$this->maker_qrcode($_SESSION['member_id']);
		$this->assign('myurl', $myurl);
		
		$mydownurl=BASE_SITE_URL."?&c=Invite&a=downqrfile&id=".$_SESSION['member_id'];
		$this->assign('mydownurl', $mydownurl);
		$this->render('invite');
	}
	
	public function maker_qrcode($id)
	{
		$id=intval($id);
		if($id<=0)
		{
		...
		if(!file_exists($imgfile)){	
			...
			$myurl=BASE_SITE_URL."/#V1".$member_id;
