...
class MemberComplainController extends BaseMemberController{
    //定义投诉状态常量
    const STATE_NEW = 10;
    ...
    public function complain_new() {
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showMessage(Language::get('wrong_argument'),$GLOBALS['_PAGE_URL'] . '&c=MemberOrder&a=index','html','error');
        }
        $condition = array();
    ...
    public function complain_save() {
        ...
        if($if_complain < 1) {
            showDialog(Language::get('para_error'),$GLOBALS['_PAGE_URL'] . '&c=MemberOrder&a=index','error');
        }
        //检查是不是正在进行投诉
        ...
        if ($state) {
            showDialog(Language::get('complain_submit_success'),$GLOBALS['_PAGE_URL'] . '&c=MemberComplain&a=index','succ');
        } else {
    ...
    public function apply_handle() {
        ...
        showMessage(Language::get('handle_submit_success'),$GLOBALS['_PAGE_URL'] . '&c=MemberComplain');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'complain_accuser_list','menu_name'=>Language::get('complain_manage_title'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberComplain')
        );
