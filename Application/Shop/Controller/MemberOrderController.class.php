...
class MemberOrderController extends BaseMemberController {
    ...
    public function index() {
        ...
        $this->render('member_order.index');
    }
    ...
    public function search_deliver(){
        ...
        $this->render('member_order_deliver.detail');
    }
    ...
    public function show_order() {
        ...
        $this->assign('refund_all',$result['data']['refund_all']);
        $this->assign('order_info',$result['data']['order_info']);
        $this->assign('daddress_info',$result['data']['daddress_info']);

        $this->render('member_order.show');
    }
    ...
    private function _order_cancel($order_info, $post) {
        if (!chksubmit()) {
            $this->assign('order_info', $order_info);
            $this->render('member_order.cancel','null_layout');
            exit();
        } else {
            $model_order = Model('order');
            $logic_order = Logic('order');
    ...
    private function _order_receive($order_info, $post) {
        if (!chksubmit()) {
            $this->assign('order_info', $order_info);
            $this->render('member_order.receive','null_layout');
            exit();
        } else {
            $model_order = Model('order');
            $logic_order = Logic('order');
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            array('menu_key'=>'member_order','menu_name'=>Language::get('spd_member_path_order_list'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberOrder'),
            array('menu_key'=>'member_order_recycle','menu_name'=>'回收站', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberOrder&recycle=1'),
        );
