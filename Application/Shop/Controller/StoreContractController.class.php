...
class StoreContractController extends BaseSellerController {
    ...
    public function __construct() {
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能',$GLOBALS['_PAGE_URL'] . '&c=SellerCenter&a=index','error');
        }
    ...
    public function index(){
        ...
        if (!$item_list) {
            showDialog('平台尚未开启任何消费者保障服务',$GLOBALS['_PAGE_URL'] . '&c=SellerCenter&a=index','error');
        }
        //查询店铺保障项目开启情况
        ...
        $this->render('store_contract.index');
    }
    /**
     * 申请加入保障项目
     */
    public function ctiapply(){
    ...
    private function applyadd_platformstore($item_info, $c_info){
        ...
        try {
            ...
            showDialog('加入成功',$GLOBALS['_PAGE_URL'] . '&c=StoreContract&a=index','succ');
        }catch (Exception $e){
    ...
    private function applyadd_joininstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            showDialog('加入申请成功，请耐心等待审核结果',$GLOBALS['_PAGE_URL'] . '&c=StoreContract&a=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    public function applypay(){
        ...
        if(chksubmit()){
            try {
                ...
                $model_contract->commit();
                showDialog('支付保证金操作成功，请耐心等待审核结果',$GLOBALS['_PAGE_URL'] . '&c=StoreContract&a=index','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
            ...
            $this->render('store_contract.applypay');
        }
    }
    /**
     * 查看保障项目日志
    ...
    public function contractlog()
    {
        ...
        $this->assign('costlog_count',$costlog_count);
        $this->assign('item_info',$item_info);
        $this->assign('contract_info',$contract_info);
        $this->assign('log_list',$log_list);
        $this->assign('logrole_arr',$model_contract->getLogRole());
        $this->assign('show_page',$model_contract->showpage(2));
        self::profile_menu('log','contractlog');
        $this->render('store_contract.log');
    }
    /**
     * 查看保证金日志
     */
    public function costlog()
    {
        ...
        $this->assign('item_info',$item_info);
        $this->assign('contract_info',$contract_info);
        $this->assign('costlog_list',$costlog_list);
        $this->assign('show_page',$model_contract->showpage(2));
        self::profile_menu('cost','cost');
        $this->render('store_contract.costlog');
    }
    /**
     * 申请退出保障项目
     */
    public function ctiquit(){
        $itemid = intval($_GET['itemid']);
        if ($itemid <= 0) {
            showDialog('参数错误','','error');
        }
    ...
    private function applyquit_platformstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            QueueClient::push('updateStoreGoodsContract', array('store_id'=>$_SESSION['store_id'],'item_id'=>$item_info['cti_id']));
            showDialog('退出成功',$GLOBALS['_PAGE_URL'] . '&c=StoreContract&a=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
    ...
    private function applyquit_joininstore($item_info, $c_info){
        ...
        try {
            ...
            $model_contract->commit();
            showDialog('退出申请提交成功，请耐心等待审核结果',$GLOBALS['_PAGE_URL'] . '&c=StoreContract&a=index','succ');
        }catch (Exception $e){
            $model_contract->rollback();
            showDialog($e->getMessage(), '', 'error');
        }
   ...
   /**
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'log':
                $menu_array = array(
                    1=>array('menu_key'=>'index', 'menu_name'=>'消费者保障服务', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreContract&a=index'),
