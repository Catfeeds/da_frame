...
class PointprodController extends BasePointShopController{
    public function __construct() {
        ...
        if (C('pointprod_isuse') != 1){
            showDialog(L('pointprod_unavailable'),"{$GLOBALS['_PAGE_URL']}",'error');
        }
    ...
    public function plist(){
        ...
        $this->assign('show_page', $model_pointprod->showpage(5));
    ...
    public function pinfo() {
        ...
        if (!in_array($pid, $tm_tm_visite_pgoods)){//如果已经浏览过该商品则不重复累计浏览次数
            ...
            if ($result['state'] == true){//累加成功则cookie中增加该商品ID
                ...
                setDaCookie('tm_visite_pgoods',implode(',', $tm_tm_visite_pgoods));
            }
        }
