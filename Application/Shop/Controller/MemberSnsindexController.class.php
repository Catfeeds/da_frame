...
class MemberSnsindexController extends BaseMemberController {
    const MAX_RECORDNUM = 20;//允许插入新记录的最大条数(注意在sns中该常量是一样的，注意与member_snshome中的该常量一致)
    public function __construct(){
        parent::__construct();
        $this->assign('relation','3');//为了跟home页面保持一致所以输出此变量
    ...
    public function index(){
        ...
        //信息输出
        $this->assign('shopdamap',substr(md5(SHOP_SITE_URL.convert_word_underscore(CONTROLLER_NAME).ACTION_NAME),0,8));
        $this->render('member_snsindex');
    }
    private function formatDate($time){
        ...
        if ($reference_date_time == $handle_date_time){
            ...
            $month_text = Language::get('spd_month');
            $day_text = Language::get('spd_day');
    ...
    public function addtrace(){
        ...
        if ($result){
            ...
            if (cookie('weibonum') != null && intval(cookie('weibonum')) >0){
                setDaCookie('weibonum',intval(cookie('weibonum'))+1,2*3600);//保存2小时
            }else{
                setDaCookie('weibonum',1,2*3600);//保存2小时
            }
            $js = "var obj = $(\"#weiboform\").find(\"[spd_type='formprivacytab']\");$(obj).find('span').removeClass('selected');$(obj).find('ul li:nth-child(1)').find('span').addClass('selected');";
            $js .= "$(\"#content_weibo\").val('');$(\"#privacy\").val('0');$('#friendtrace').lazyshow({url:\"{$GLOBALS['_PAGE_URL']}&c=MemberSnshome&a=tracelist&curpage=1\",'iIntervalId':true});";
    ...
    public function sharegoods(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                ...
                $content_str .= "<div class=\"fd-media\">
                    ...
                    <div class=\"goodsinfo\">
                        <dl>
                            ...
                            <dd>".Language::get('sns_sharegoods_price').Language::get('spd_colon').Language::get('currency').$goods_info['goods_price']."</dd>
                            <dd>".Language::get('sns_sharegoods_freight').Language::get('spd_colon').Language::get('currency').$goods_info['goods_freight']."</dd>
                            <dd datype=\"collectbtn_{$goods_info['goods_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_goods(\'{$goods_info['goods_id']}\',\'succ\',\'collectbtn_{$goods_info['goods_id']}\');\">".Language::get('sns_sharegoods_collect')."</a></dd>
                        </dl>
                      </div>
                 </div>";
                ...
                //输出js
                $js = "DialogManager.close('sharegoods');var countobj=$('[spd_type=\'sharecount_{$goods_info['goods_id']}\']');$(countobj).html(parseInt($(countobj).text())+1);";
                $url = '';
                if ($_GET['irefresh']){
                    $js .= "$('#friendtrace').lazyshow({url:\"{$GLOBALS['_PAGE_URL']}&c=MemberSnsindex&a=tracelist&curpage=1\",'iIntervalId':true});";
                }else{
                    $url = 'reload';
                }
            ...
            $this->render('member_snssharegoods','null_layout');
        }
    }
    /**
     * 分享店铺
    ...
    public function sharestore(){
        ...
        if ($_POST['form_submit'] == 'ok'){
                ...
                $content_str .= "<div class=\"fd-media\">
                    ...
                    <div class=\"goodsinfo\">
                        <dl>
                            ...
                            <dd datype=\"storecollectbtn_{$store_info['store_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_store(\'{$store_info['store_id']}\',\'succ\',\'storecollectbtn_{$store_info['store_id']}\');\">".Language::get('sns_sharestore_collect')."</a></dd>
                        </dl>
                      </div>
                 </div>";
                $insert_arr['trace_content'] = $content_str;
            ...
            $this->render('member_snssharestore','null_layout');
        }
    }
    /**
     * 删除动态
    ...
    public function deltrace(){
        ...
        if ($result){
            ...
            if ($_GET['type'] == 'href'){
                showDialog(Language::get('spd_common_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=MemberSnshome&a=trace&mid='.$_SESSION['member_id'],'succ');
            }else {
                $js = "location.reload();";
                showDialog(Language::get('spd_common_del_succ'),'','succ',$js);
            }
        } else {
            showDialog(Language::get('spd_common_del_fail'),'','error');
        }
    }
    /**
     * SNS动态列表
    ...
    public function tracelist(){
        ...
        if (!empty($tracelist)){
            foreach ($tracelist as $k=>$v){
                if ($v['trace_title']){
                    $v['trace_title'] = str_replace("%siteurl%", SHOP_SITE_URL, $v['trace_title']);
                    $v['trace_title_forward'] = '|| @'.$v['trace_membername'].Language::get('spd_colon').preg_replace("/<a(.*?)href=\"(.*?)\"(.*?)>@(.*?)<\/a>([\s|:|：]|$)/is",'@${4}${5}',$v['trace_title']);
                }
                if(!empty($v['trace_content'])){
                    //替换内容中的siteurl
                    $v['trace_content'] = str_replace("%siteurl%", SHOP_SITE_URL, $v['trace_content']);
                }
                $tracelist[$k] = $v;
            }
        }
        ...
        $this->render('member_snstracelist','null_layout');
    }
    /**
     * 编辑分享商品的可见权限(主人登录后操作)
     */
    public function editprivacy(){
        ...
        if ($result){
            ...
            $js = "var obj = $(\"#recordone_{$id}\").find(\"[spd_type='privacytab']\"); $(obj).find('span').removeClass('selected');$(obj).find('li:nth-child(".$privacy_item.")').find('span').addClass('selected');";
    ...
    public function addcomment(){
        ...
        if ($result){
            ...
            if (cookie('commentnum') != null && intval(cookie('commentnum')) >0){
                setDaCookie('commentnum',intval(cookie('commentnum'))+1,2*3600);//保存2小时
            }else{
                setDaCookie('commentnum',1,2*3600);//保存2小时
            }
            $js = "$(\"#content_comment{$originalid}\").val('');";
            if ($_POST['showtype'] == 1){
                $js .="$(\"#tracereply_{$originalid}\").load({$GLOBALS['_PAGE_URL']}&c=MemberSnshome&a=commenttop&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }else {
                $js .="$(\"#tracereply_{$originalid}\").load({$GLOBALS['_PAGE_URL']}&c=MemberSnshome&a=commentlist&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }
    ...
    public function delcomment(){
        ...
        if ($result){
            ...
            $js .="$('.comment-list [spd_type=\"commentrow_{$id}\"]').remove();";
            showDialog(Language::get('spd_common_del_succ'),'','succ',$js);
        }else {
            showDialog(Language::get('spd_common_del_fail'),'','error');
        }
    ...
    public function editlike(){
        ...
        if ($result){
            ...
            $content_str .= "<div class=\"fd-media\">
                ...
                <div class=\"goodsinfo\">
                    <dl>
                        ...
                        <dd>".Language::get('sns_sharegoods_price').Language::get('spd_colon').Language::get('currency').$goods_info['goods_price']."</dd>
                        <dd>".Language::get('sns_sharegoods_freight').Language::get('spd_colon').Language::get('currency').$goods_info['goods_freight']."</dd>
                        <dd datype=\"collectbtn_{$goods_info['goods_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_goods(\'{$goods_info['goods_id']}\',\'succ\',\'collectbtn_{$goods_info['goods_id']}\');\">".Language::get('sns_sharegoods_collect')."</a>&nbsp;&nbsp;(".$goods_info['goods_collect'].Language::get('sns_collecttip').")</dd>
                    </dl>
                  </div>
             </div>";
            $insert_arr['trace_content'] = $content_str;
            ...
            $insert_arr['trace_copycount'] = 0;
            $result = $tracelog_model->tracelogAdd($insert_arr);
            $js = "var obj = $(\"#likestat_{$goods_info['goods_id']}\"); $(\"#likestat_{$goods_info['goods_id']}\").find('i').addClass('noaction');$(obj).find('a').addClass('noaction'); var countobj=$('[spd_type=\'likecount_{$goods_info['goods_id']}\']');$(countobj).html(parseInt($(countobj).text())+1);";
            showDialog(Language::get('spd_common_op_succ'),'','succ',$js);
        }else {
            showDialog(Language::get('spd_common_op_fail'),'','error');
        }
    }
    /**
     * 添加转发
    ...
    public function addforward(){
        ...
        if ($tracelog_info['trace_originalid'] > 0 || $tracelog_info['trace_from'] != 1){
            ...
            $content_str ="<div class=\"title\"><a href=\"%siteurl%&c=MemberSnshome&mid={$tracelog_info['trace_memberid']}\" target=\"_blank\" class=\"uname\">{$tracelog_info['trace_membername']}</a>";
            $content_str .= Language::get('spd_colon')."{$tracelog_info['trace_title']}</div>";
        ...
        if ($result){
            ...
            if ($_GET['irefresh']){
                ...
                if (cookie('forwardnum') != null && intval(cookie('forwardnum')) >0){
                    setDaCookie('forwardnum',intval(cookie('forwardnum'))+1,2*3600);//保存2小时
                }else{
                    setDaCookie('forwardnum',1,2*3600);//保存2小时
                }
                if ($_GET['type']=='home'){
                    $js = "$('#friendtrace').lazyshow({url:\"{$GLOBALS['_PAGE_URL']}&c=MemberSnshome&a=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else if ($_GET['type']=='snshome'){
                    $js = "$('#forward_".$originalid."').hide();$('#friendtrace').lazyshow({url:\"{$GLOBALS['_PAGE_URL']}&c=MemberSnshome&a=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else {
                    $js = "$('#friendtrace').lazyshow({url:\"{$GLOBALS['_PAGE_URL']}&c=MemberSnsindex&a=tracelist&curpage=1\",'iIntervalId':true});";
                }
    ...
    public function sharegoods_one(){
        ...
        if ($_GET['dialog']){
            $js = "CUR_DIALOG = ajax_form('sharegoods', '".Language::get('sns_sharegoods_tofriend')."', '{$GLOBALS['_PAGE_URL']}&c=MemberSnsindex&a=sharegoods_one&gid={$gid}', 480);";
            showDialog('','','js',$js);
        }
        //查询商品信息
        ...
        $this->render('member_snssharegoods_one','null_layout');
    }
    /**
     * 店铺收藏页面分享店铺
     */
    public function sharestore_one(){
        ...
        if ($_GET['dialog']){
            $js = "ajax_form('sharestore', '".Language::get('sns_sharestore')."', '{$GLOBALS['_PAGE_URL']}&c=MemberSnsindex&a=sharestore_one&sid={$sid}', 480);";
            showDialog('','','js',$js);
        }
        //查询店铺信息
        $store_model = Model('store');
        ...
        $this->render('member_snssharestore_one','null_layout');
    }
