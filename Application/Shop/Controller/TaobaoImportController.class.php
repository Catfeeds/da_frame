...
class TaobaoImportController extends BaseSellerController {
        ...
        if(!checkPlatformStore()){
            ...
            if (intval($this->store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $this->store_grade['sg_goods_limit']) {
                    showMessage(L('store_goods_index_goods_limit') . $this->store_grade['sg_goods_limit'] . L('store_goods_index_goods_limit1'), $GLOBALS['_PAGE_URL'] . '&c=StoreGoods&a=goods_list', 'html', 'error');
                }
            }
        }
    }
	...
	public function index(){
		   ...
		   if (strtoupper(CHARSET) == 'GBK'){
			...
			if(intval($store_grade['sg_goods_limit']) != 0) {
				if($goods_num >= $store_grade['sg_goods_limit']) {
					showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'],$GLOBALS['_PAGE_URL'] . '&c=StoreGoods&a=goods_list','html','error');
				}
			...
			if(intval($store_info['store_end_time']) != 0) {
				if(time() >= $store_info['store_end_time']) {
					showMessage($lang['store_goods_index_time_limit'],$GLOBALS['_PAGE_URL'] . '&c=StoreGoods&a=goods_list','html','error');
				}
			}
			/**
			 * 循环添加数据
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					if($remain_num>0 and $k>=$remain_num){
						showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'].$lang['store_goods_import_end'].(count($records)-$remain_num).$lang['store_goods_import_products_no_import'],$GLOBALS['_PAGE_URL'] . '&c=StoreGoods&a=taobao_import&step=4','html','error');
					}
					...
					//导入后直接生成二维码
					$PhpQRCode->set('data',WAP_SITE_URL . '/tmpl/product_detail.html?goods_id='.$goods_id1);
		...
		$this->assign('menu_sign','taobao_import');
		$this->render('store_goods_import');
	}
