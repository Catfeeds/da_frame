...
class MemberFavoriteStoreController extends BaseMemberController{
    ...
    public function more() {
        ...
        $this->assign('count', array('new' => $new_count, 'promotion' => $promotion_count, 'hot' => $salenum_count));
        ...
        if (!empty($_GET['curpage'])) {
            $this->render('favorites_store_index.more_item','null_layout');
        } else {

            self::profile_menu('favorites','more');
            $this->render('favorites_store_index.more');
        }
    }
    /**
     * 删除收藏
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),$GLOBALS['_PAGE_URL'] . '&c=MemberFavoriteStore&a=fslist','succ');
                }
            }else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_store','menu_name'=>Language::get('spd_member_path_collect_store'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberFavoriteStore&a=fslist')
        );
        if ($menu_key == 'more') {
