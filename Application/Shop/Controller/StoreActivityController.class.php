...
class StoreActivityController extends BaseSellerController {
    ...
    public function store_activity(){
        ...
        $this->render('store_activity.list');
    }
    /**
     * 参与活动
     */
    public function activity_apply(){
        ...
        if($activity_id <= 0){
            showMessage(Language::get('para_error'),$GLOBALS['_PAGE_URL'] . '&c=StoreActivity&a=store_activity','html','error');
        }
        ...
        if(empty($activity_info) || $activity_info['activity_type'] != '1' || $activity_info['activity_state'] != 1 || $activity_info['activity_start_date']>time() || $activity_info['activity_end_date']<time()){
            showMessage(Language::get('store_activity_not_exists'),$GLOBALS['_PAGE_URL'] . '&c=StoreActivity&a=store_activity','html','error');
        }
        ...
        $this->render('store_activity.apply');
    }
    ...
    public function activity_apply_save(){
        ...
        if(empty($_POST['item_id'])){
            showDialog(Language::get('store_activity_choose_goods'),$GLOBALS['_PAGE_URL'] . '&c=StoreActivity&a=store_activity');
        }
        $activity_id = intval($_POST['activity_id']);
        if($activity_id <= 0){
            showDialog(Language::get('para_error'),$GLOBALS['_PAGE_URL'] . '&c=StoreActivity&a=store_activity');
        }
        ...
        if(empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_start_date']>time() || $activity['activity_end_date']<time()){
            showDialog(Language::get('store_activity_not_exists'),$GLOBALS['_PAGE_URL'] . '&c=StoreActivity&a=store_activity');
        }
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
                1=>array('menu_key'=>'store_activity','menu_name'=>Language::get('spd_member_path_activity_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreActivity&a=store_activity'),
                2=>array('menu_key'=>'activity_apply','menu_name'=>Language::get('spd_member_path_join_activity'),'menu_url'=>'')
        );
