...
class StoreBillController extends BaseSellerController {
    ...
    public function index() {
        ...
        $this->assign('show_page',$model_bill->showpage());

        $model_store_ext = Model('store_extend');
        $ext_info = $model_store_ext->getStoreExtendInfo(array('store_id'=>$_SESSION['store_id']));
        $this->assign('bill_cycle',$ext_info['bill_cycle'] ? $ext_info['bill_cycle'].'天' : '1个月');

        $this->profile_menu('list','list');
        $this->render('store_bill.index');
    }
    ...
    public function show_bill(){
        ...
        if ($_GET['type'] =='refund'){
            ...
            $this->assign('show_page',$model_store_cost->showpage());
            ...
            $this->assign('deposit_list',$tmp_book);
        ...
        $this->assign('sub_tpl_name',$sub_tpl_name);
        $this->assign('bill_info',$bill_info);
        $this->render('store_bill.show');
    }
    ...
    public function bill_print(){
        ...
        $this->assign('bill_info',$bill_info);
        $this->render('store_bill.print','null_layout');
    }
    ...
    public function export_order(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=StoreBill&a=show_bill&ob_id='.$_GET['ob_id']);
    ...
    public function export_refund_order(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('list',$array);
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=StoreBill&a=show_bill&query_type=refund&ob_id='.$_GET['ob_id']);
                $this->render('store_export.excel');
                exit();
            }else{
                //如果数量小，直接下载
        ...
        foreach ($data as $v) {
            ...
            $rpt_amount += $export_data[$k+1][] = daPriceFormat($v['rpt_amount']);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Bill&a=lists'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBill&a=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBill&a=show_bill&type=refund&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'cost_list','menu_name'=>'促销费用','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBill&a=show_bill&type=cost&ob_id='.$_GET['ob_id'])
                );
                if (floatval($this->_bill_info['ob_order_book_totals']) > 0) {
                    array_push($menu_array,array('menu_key'=>'book_list','menu_name'=>'未退定金', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBill&a=show_bill&type=book&ob_id='.$_GET['ob_id']));
                }
                break;
        }
