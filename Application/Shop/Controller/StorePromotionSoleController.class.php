...
class StorePromotionSoleController extends BaseSellerController {
    ...
    public function index() {
        $this->sole_goods_list();
    }

    /**
     * 套餐商品列表
     */
    public function sole_goods_list() {
        ...
        if (checkPlatformStore()) {
            ...
            $this->assign('sole_quota', $sole_quota);
            if (!empty($sole_quota)) {
                $hasList = true;
            }
        }
        ...
        $this->render('store_promotion_sole.goods_list');
    }

    /**
     * 选择商品
     */
    public function sole_select_goods() {
        ...
        $this->render('store_promotion_sole.select_goods', 'null_layout');
    }

    /**
     * 购买套餐
     */
    public function sole_quota_add() {
        if (chksubmit()) {
        ...
        $this->render('store_promotion_sole.quota_add');
    }

    /**
     * 套餐续费
     */
    public function sole_renew() {
        if (chksubmit()) {
            $model_sole = Model('p_sole');
            $quantity = intval($_POST['sole_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_sole_price')); // 扣款数
    ...
    public function choosed_goods() {
        ...
        $this->assign('solegoods_info', $solegoods_info);
        
        $this->render('store_promotion_sole.choosed_goods', 'null_layout');
    }
    
    /**
     * 保存选择商品
     */
    public function choosed_goods_save() {
        $model_sole = Model('p_sole');
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
        ...
        $sole_price = daPriceFormat(floatval($_POST['sole_price']));
        if ($gid <= 0) {
