...
class StoreSettingController extends BaseSellerController {
    ...
    public function store_setting(){
        ...
        if (chksubmit()){
			...
			$PhpQRCode->set('data',WAP_SITE_URL . '/tmpl/store.html?store_id='.$store_id);
            		$PhpQRCode->set('pngTempName', $store_id . '_store.png');
            		$PhpQRCode->init();

            $model_class->editStore($param, array('store_id' => $store_id));
            showDialog(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=store_setting','succ');
        }
        /**
         * 实例化店铺等级模型
         */
        ...
        if($subdomain_edit == 1 && ($subdomain_times > $store_domain_times)){//可继续修改二级域名
            $this->assign('subdomain_edit',$subdomain_edit);
        }
        /**
         * 输出店铺信息
         */
        ...
        $this->assign('subdomain',$config_enabled_subdomain);
        $this->assign('subdomain_times',$config_subdomain_times);
        /**
         * 页面输出
         */
        $this->render('store_setting_form');
    }
    ...
    public function store_slide() {
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            showDialog(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=store_slide','succ');
        }
    ...
    public function dorp_img(){
        ...
        echo json_encode(array('succeed'=>Language::get('spd_common_save_succ','UTF-8')));die;
    }
    ...
    public function theme(){
        ...
        $curr_image = BASE_SITE_URL. "/Application/Shop/View/" . TPL_SHOP_NAME . "/"  . '/store/style/' . $curr_store_theme   .'/images/preview.jpg';
        $curr_theme = array(
        ...
        foreach ($style_data as $key => $val){
            if (in_array($key,$themes)){
                ...
                'image'=>BASE_SITE_URL. "/Application/Shop/View/" . "/" . TPL_SHOP_NAME .'/store/style/'.$key.'/images/preview.jpg'
                );
            }
        }
        /**
    ...
    public function set_theme(){
        ...
        if (!empty($style) && file_exists(dirname(__FILE__) . DS . '..' . DS . 'View' . DS . TPL_SHOP_NAME . '/store/style/'.$style.'/images/preview.jpg')){
            $store_class = Model('store');
    ...
    public function store_mobile()
    {
        ...
        if (chksubmit()) {
            ...
            showDialog('保存成功', $GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=store_mobile', 'succ');
        }
        ...
        $this->render('store_setting.store_mobile');
    }
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            1=>array('menu_key'=>'store_setting','menu_name'=>Language::get('spd_member_path_store_config'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=store_setting'),
            4=>array('menu_key'=>'store_slide','menu_name'=>Language::get('spd_member_path_store_slide'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=store_slide'),
            5=>array('menu_key'=>'store_theme','menu_name'=>'店铺主题','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=theme'),

            7 => array(
                'menu_key' => 'store_mobile',
                'menu_name' => '手机店铺设置',
                'menu_url' => $GLOBALS['_PAGE_URL'] . '&c=StoreSetting&a=store_mobile',
            ),
        );
