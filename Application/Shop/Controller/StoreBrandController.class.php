...
class StoreBrandController extends BaseSellerController {
    ...
    public function brand_add() {
        ...
        $this->render('store_brand.add','null_layout');
    }
    ...
    public function brand_save() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                showDialog($lang['store_goods_brand_apply_success'],$GLOBALS['_PAGE_URL'] . '&c=Storebrand&a=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            }else {
        ...
        if ($_POST['form_submit'] == 'ok' and intval($_POST['brand_id']) != 0) {
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showDialog($lang['spd_common_save_succ'],$GLOBALS['_PAGE_URL'] . '&c=Storebrand&a=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
                }else {
                    showDialog($lang['spd_common_save_fail']);
                }
            }
    ...
    public function drop_brand() {
        ...
        if ($brand_id > 0){
            ...
            showDialog(Language::get('spd_common_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=Storebrand&a=brand_list','succ');
        }else {
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'brand_list':
                $menu_array = array(
                    1=>array('menu_key'=>'brand_list', 'menu_name'=>$lang['spd_member_path_brand_list'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Storebrand&a=brand_list')
                );
                break;
        }
        if(!empty($array)) {
