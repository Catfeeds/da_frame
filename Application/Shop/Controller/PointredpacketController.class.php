...
class PointredpacketController extends BasePointShopController {
    ...
    public function pointredpacket(){
        ...
        $this->render('pointredpacket');
    }
    
    /**
     * 兑换红包
     */
    public function rptexchange(){
        $tid = intval($_GET['tid']);
        if($tid <= 0){
        ...
        if($_SESSION['is_login'] != '1'){
            ...
            $js = "CUR_DIALOG = ajax_form('rptexchange', '您要兑换的红包', '" . 
            $GLOBALS['_PAGE_URL'] . 
            "&c=Pointredpacket&a=rptexchange&tid={$tid}', 550);";
            showDialog('','','js',$js);
            die;
        }
        $result = true;
        ...
        $this->render('pointredpacket.exchange','null_layout');
    }
    /**
     * 兑换红包保存信息
     */
    public function rptexchange_save(){
        if($_SESSION['is_login'] != '1'){
            $js = "login_dialog();";
            showDialog('','','js',$js);
        }
