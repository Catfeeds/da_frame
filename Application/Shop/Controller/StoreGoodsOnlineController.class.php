...
class StoreGoodsOnlineController extends BaseSellerController {
    ...
    public function goods_list() {
        ...
        $this->render('store_goods_list.online');
    }
    ...
    public function edit_save_goods() {
        ...
        if (!checkPlatformStore()) {
            //商品分类 by shopda. com 提供批量显示所有分类插件
        ...
        if ($result['state']) {
            //提交事务
            showDialog(L('spd_common_op_succ'), $_POST['ref_url'], 'succ');
        } else {
            //回滚事务
    ...
    public function edit_save_image() {
        if (chksubmit()) {
            ...
            if ($rs['state']) {
                ...
                showDialog(L('spd_common_save_fail'), urlShop('store_goods_online', 'index'));
            }
        }
    }
    ...
    public function edit_class() {
        ...
        $this->assign('commonid', $_GET['commonid']);
    ...
    public function goods_unshow() {
        ...
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : $GLOBALS['_PAGE_URL'] . '&c=StoreGoodsOnline&a=goods_list', 'succ', '', 2);
        } else {
    ...
    public function edit_jingle() {
        ...
        $this->render('store_goods_list.edit_jingle', 'null_layout');
    }
    ...
    public function edit_plate() {
        ...
        $this->render('store_goods_list.edit_plate', 'null_layout');
    }
    ...
    public function add_gift() {
        ...
        $this->render('store_goods_edit.add_gift');
    }
    ...
    public function save_gift() {
        ...
        showDialog(L('spd_common_save_succ'), $_POST['ref_url'], 'succ');
    }
	
   /**
   * 设置商品佣金
     */
    public function edit_invite_price() {
		$model_goods = Model('goods');
        if (chksubmit()) {
			$where = array();
        ...
        $this->render('store_goods_list.edit_invite_price', 'null_layout');
    }
    ...
    public function search_goods() {
        ...
        $this->assign('goods_list', $goods_list);
        $this->render('store_goods_edit.search_goods', 'null_layout');
    }
    ...
    public function get_goods_list_ajax() {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            foreach ($goods_spec as $k => $v) {
                $spec_array[] = '<div class="spdgoods-spec">' . $spec_name[$k] . L('spd_colon') . '<em title="' . $v . '">' . $v .'</em>' . '</div>';
            }
    ...
    public function edit_iframe_ajax() {

        $this->render('store_goods_list.edit_iframe', 'null_layout');
    }
    
    public function edit_body_ajax() {
        $model_goods = Model('goods');
        if (chksubmit()) {
            $where = array();
            ...
            if ($result) {
                showDialog(L('spd_common_op_succ'), 'reload', 'succ', 'setTimeout(function () {$(".dialog_close_button", parent.document).click();}, 2000)');
            } else {
        ...
        $this->render('store_goods_list.edit_body', 'null_layout');
    }
