...
class StoreGroupbuyController extends BaseSellerController {
    ...
    public function __construct() {
        ...
        if (intval(C('groupbuy_allow')) !== 1){
            showMessage(Language::get('groupbuy_unavailable'),$GLOBALS['_PAGE_URL'] . '&c=SellerCenter','','error');
        }
    }
    /**
     * 默认显示抢购列表
    ...
    public function groupbuy_quota_add() {
        ...
        $this->render('store_groupbuy_quota.add');
    }
    ...
    public function groupbuy_quota_add_save() {
        ...
        $this->recordSellerLog('购买'.$groupbuy_quota_quantity.'份抢购套餐，单价'.$current_price.L('spd_yuan'));
    ...
    public function groupbuy_add() {
        ...
        $this->assign('groupbuy_start_time', TIMESTAMP + intval(C('groupbuy_review_day')) * 86400);

        $this->assign('groupbuy_classes', Model('groupbuy')->getGroupbuyClasses());

        self::profile_menu('groupbuy_add');
        $this->render('store_groupbuy.add');
    ...
    public function groupbuy_save() {
        ...
        $param['groupbuy_rebate'] = daPriceFormat(floatval($_POST['groupbuy_price'])/floatval($goods_info['goods_price'])*10);
        ...
        if ($_GET['vr']) {
            if ($param['upper_limit'] > 0 && $goods_info['virtual_limit'] > 0
                ...
                showDialog(sprintf('虚拟抢购活动的限购数量(%d)不能大于虚拟商品本身的限购数量(%d)',
                    $param['upper_limit'], $goods_info['virtual_limit']
                    ), $GLOBALS['_PAGE_URL'] . '&c=SellerCenter');
            }
        ...
        if($result) {
            ...
            showDialog(Language::get('groupbuy_add_success'),$GLOBALS['_PAGE_URL'] . '&c=SellerCenter','succ');
        }else {
            showDialog(Language::get('groupbuy_add_fail'),$GLOBALS['_PAGE_URL'] . '&c=SellerCenter');
        }
    }
    ...
    public function search_goods() {
        ...
        $this->render('store_groupbuy.goods', 'null_layout');
    }
    ...
    public function groupbuy_add_vr()
    {
        ...
        $this->render('store_groupbuy.add_vr');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'groupbuy_list','menu_name'=>L('spd_member_path_group_list'),'menu_url'=>urlShop('store_groupbuy', 'groupbuy_list'))
        );
        switch ($menu_key){
        case 'groupbuy_add':
            $menu_array[] = array('menu_key'=>'groupbuy_add','menu_name'=>L('spd_member_path_new_group'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SellerCenter&a=groupbuy_add');
            break;
        case 'groupbuy_add_vr':
            $menu_array[] = array('menu_key'=>'groupbuy_add_vr','menu_name'=>'新增虚拟抢购','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SellerCenter&a=groupbuy_add_vr');
            break;
        case 'groupbuy_quota_add':
            ...
            break;
        case 'groupbuy_edit':
            $menu_array[] = array('menu_key'=>'groupbuy_edit','menu_name'=>L('spd_member_path_edit_group'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SellerCenter');
            break;
        case 'cancel':
            $menu_array[] = array('menu_key'=>'groupbuy_cancel','menu_name'=>L('spd_member_path_cancel_group'));
            break;
        }
