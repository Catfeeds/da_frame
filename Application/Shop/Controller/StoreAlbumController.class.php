...
//      $this->assign('show_page',$page->show());
    ...
    public function album_add(){
        ...
        $this->render('store_album.class_add','null_layout');
    }
    /**
     * 相册保存
     *
    ...
    public function album_add_save(){
        if (chksubmit()){
            ...
            if($class_count['count'] >= 40){
                showDialog(Language::get('album_class_save_max_20'),$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_edit(){
        ...
        $this->render('store_album.class_edit','null_layout');
    }
    /**
     * 相册分类编辑保存
     */
    public function album_edit_save(){
        ...
        if($return){
            ...
            if($re){
                showDialog(Language::get('album_class_edit_succeed'),$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }else{
    ...
    public function album_del(){
        ...
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum','succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
    ...
    public function album_pic_list(){
        ...
        $this->render('store_album.pic_list');
    }
    /**
     * 图片列表，外部调用
     */
    public function pic_list(){
        ...
        switch($_GET['item']) {
        case 'goods':
            $this->render('store_goods_add.step2_master_image','null_layout');
            break;
        case 'des':
            $this->render('store_goods_add.step2_desc_image','null_layout');
            break;
        case 'groupbuy':
            $this->render('store_groupbuy.album','null_layout');
            break;
        case 'store_sns_normal':
            $this->render('store_sns_add.album', 'null_layout');
            break;
        case 'goods_image':
            $this->assign('color_id', $_GET['color_id']);
            $this->render('store_goods_add.step3_goods_image', 'null_layout');
            break;
        case 'mobile':
            $this->assign('type', $_GET['type']);
            $this->render('store_goods_add.step2_mobile_image', 'null_layout');
            break;
        }
    }
    /**
    ...
    public function album_pic_info(){
        ...
        $this->assign('tatal_page',$tatal_page);
        $this->assign('curpage',$curpage);
        ...
        $this->render('store_album.pic_info');
    }
    ...
    public function album_ad_ajax(){
        ...
        $this->render('store_album.pic_scroll_ajax','null_layout');
    }
    ...
    public function album_pic_move(){
        ...
        $this->assign('class_list',$class_list);
        $this->render('store_album.move','null_layout');
    }
	...
	public function album_pic_watermark(){
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"{$GLOBALS['_PAGE_URL']}&c=StoreAlbum&a=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
    ...
    public function store_watermark(){
         ...
         */
        $dir_list = array();
        readFileList(COMMON_RESOURCE_PATH.DS.'font',$dir_list);
        if (!empty($dir_list) && is_array($dir_list)){
            $fontInfo = array();
            require_once(COMMON_RESOURCE_PATH.DS.'font'.DS.'font.info.php');
        ...
        $this->render('store_watermark.form');
    }    /**
     * 上传图片
     *
     */
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('spd_member_path_my_album'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum'),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('spd_member_path_watermark'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum&a=store_watermark')
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('spd_member_path_my_album'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum'),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('spd_member_path_album_pic_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum&a=album_pic_list&id='.intval($_GET['id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('spd_member_path_watermark'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum&a=store_watermark')
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('spd_member_path_my_album'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum'),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('spd_member_path_album_pic_info'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum&a=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('spd_member_path_watermark'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreAlbum&a=store_watermark')
                );
                break;
        }
