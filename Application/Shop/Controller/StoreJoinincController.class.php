...
class StoreJoinincController extends BaseHomeController {
    ...
    public function __construct() {
		...
		showMessage('开店数量超过允许开店最大值，不能继续申请', "{$GLOBALS['_PAGE_URL']}", 'html', 'error');
        }
        ...
        if(!empty($seller_info)) {
            @header("location: {$GLOBALS['_PAGE_URL']}&c=SellerLogin");
        }

        if($_GET['a'] != 'check_seller_name_exist' && $_GET['a'] != 'checkname' && 
        		($_GET['is_check_joinin_state'] != 1)) { //构造方法内防止循环调用
...
//             var_dump($joinin_detail['joinin_state']);
//             exit;
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_SUCCESS:
                    if(!in_array(ACTION_NAME, array('pay', 'pay_save'))) {
                        $this->pay();
                    }
                    break;
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array(ACTION_NAME, array('step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.'&c=StoreJoininc&a=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array(ACTION_NAME, array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL . '&c=StoreJoininc&a=pay');
                    }
                    break;
    ...
    public function step0() {
        ...
        $this->render('store_joininc_apply');
	exit;
    }
    ...
    public function step4() {
        ...
        @header('location: ' . $GLOBALS['_PAGE_URL'] . '&c=StoreJoininc');
    ...
    public function pay_save() {
        ...
        @header('location: ' . $GLOBALS['_PAGE_URL'] . '&c=StoreJoinin');
    }
    ...
    public function show_join_message($message, $btn_next = FALSE, $step = '2') {
