...
class StoreRefundController extends BaseSellerController {
    ...
    public function edit() {
        ...
        if (chksubmit()) {
            $reload = $GLOBALS['_PAGE_URL'] . '&c=StoreRefund&lock=1';
            if ($refund['order_lock'] == '2') {
                $reload = $GLOBALS['_PAGE_URL'] . '&c=StoreRefund&lock=2';
            }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'refund':
                $menu_array = array(
                    array('menu_key'=>'2','menu_name'=>'售前退款',  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreRefund&lock=2'),
                    array('menu_key'=>'1','menu_name'=>'售后退款','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreRefund&lock=1')
                );
                break;
        }
