...
class StoreVoucherController extends BaseSellerController{
    //定义代金券类常量
    ...
    public function __construct() {
        ...
        if (C('voucher_allow') != 1){
            showMessage(Language::get('voucher_unavailable'),$GLOBALS['_PAGE_URL'] . '&c=Store','html','error');
        }
        //申请记录状态
    ...
    public function templatelist(){
        ...
        $this->assign('show_page',$model_voucher->showpage(2));
        $this->profile_menu('voucher','templatelist');
        $this->render('store_voucher_template.index') ;
    }
    ...
    public function voucherlist(){
        ...
        if($t_id <= 0){
            $this->render('store_voucher.list','null_layout');
        }
        $model_voucher = Model('voucher');
        //查询代金券模板
        $where = array();
    ...
    public function voucher_export(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=voucher_export');
                $this->render('store_export.excel');
            }else{  //如果数量小，直接下载
    ...
    public function quotaadd(){
        if (chksubmit()){
            ...
            $this->recordSellerLog('购买'.$quota_quantity.'份代金券套餐，单价'.$current_price.L('spd_yuan'));

            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=quotalist','succ');
            } else {
                showDialog(Language::get('spd_common_op_fail'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=quotalist');
            }
        }else {
            //输出导航
            self::profile_menu('quota_add','quotaadd');
            $this->render('store_voucher_quota.add');
        }
    }
    /*
     * 代金券模版添加
    ...
    public function templateadd(){
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=quotaadd','html','error');
            }
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                ...
                showMessage($message,$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','succ');
                }else{
                    showDialog(Language::get('spd_common_save_fail'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','error');
                }
            }
        }else{
            //店铺分类
            ...
            $this->render('store_voucher_template.add');
        }
    }
    /*
     * 代金券模版编辑
    ...
    public function templateedit(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('spd_common_op_succ'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','succ');
                }else{
                    showDialog(Language::get('spd_common_op_fail'),$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist','error');
                }
            }
        }else{
    ...
    public function templatedel(){
        ...
        if ($rs){
            ...
            showDialog(Language::get('spd_common_del_succ'),'reload','succ');
        }else {
    ...
    public function templateinfo(){
        ...
        $this->render('store_voucher_template.info');
    }
    /*
     * 把代金券模版设为失效
     */
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('spd_member_path_store_voucher'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('spd_member_path_store_voucher'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&A=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('spd_member_path_store_voucher'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVoucher&a=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
