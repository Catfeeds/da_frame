...
class PaymentController extends BaseHomeController{
    ...
    public function real_order(){
        ...
        $url = "{$GLOBALS['_PAGE_URL']}&c=MemberOrder";
        ...
        if (empty($api_pay_amount)) {
            redirect(SHOP_SITE_URL.'&c=Buy&a=pay_ok&pay_sn='.$order_pay_info['data']['pay_sn'].'&pay_amount='.daPriceFormat($pay_amount));
        	exit;
        }
        ...
        $order_pay_info['data']['api_pay_amount'] = daPriceFormat($api_pay_amount);
    ...
    public function vr_order(){
        ...
        $payment_code = $_POST['payment_code'];
        $url = $GLOBALS['_PAGE_URL'] . '&c=MemberVrOrder';
        ...
        $api_pay_amount = 0;
        if ($order_info['order_state'] == ORDER_STATE_NEW) {
            $api_pay_amount = floatval(daPriceFormat($order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount']));
        }
        ...
        $order_info['api_pay_amount'] = daPriceFormat($api_pay_amount);
    ...
    private function _api_pay($order_info, $payment_info) {
    	...
    	$order_info['site_name'] = C("site_name");
        ...
        if($payment_info['payment_code'] == 'chinabank') {
            ...
            if (array_key_exists('order_list', $order_info)) {
                ...
                $this->assign('args','buyer_id='.$_SESSION['member_id'].'&pay_id='.$order_info['pay_id']);
            } else {
                $this->assign('order_list',array($order_info));
                $this->assign('args','buyer_id='.$_SESSION['member_id'].'&order_id='.$order_info['order_id']);
            }
            $this->assign('api_pay_amount',$order_info['api_pay_amount']);
			
            $pay_url = base64_encode(encrypt($payment_api->get_payurl(),MD5_KEY));
            $this->assign('pay_url',$pay_url);
            $this->assign('nav_list', rkcache('nav',true));
            $this->render('payment.wxpay');
            
        } elseif ($payment_info['payment_code'] == 'pingxxpay') {
            if (!extension_loaded('curl')) {
                showMessage('系统curl扩展未加载，请检查系统配置', '', 'html', 'error');
            }
            $this->setDir('buy');
            ...
            $pingxx_payment_list = array();
            $pingxx_support_channel = array("alipay_pc_direct" => '支付宝PC', 
            		"upacp_pc" => '银联PC', 
            		"cp_b2b" => '银联商户PC',
            		"isv_wap" => '扫码支付',
            );
            foreach ($pingxx_support_channel as $channel_item => $channel_name)
            {
            	if ($payment_info['payment_config'][$channel_item . "_state"] == 1)
            	{
            		$pingxx_payment_list[] = array('payment_code' => $channel_item, 'payment_name' => $channel_name);
            	}
            }
 
            //设置支付跳转URL
            $this->assign('payment_list', $pingxx_payment_list);
            
            $this->assign('nav_list', rkcache('nav',true));
            $this->render('payment.pingxxpay');
            
        } else {
        	$payment_url = $payment_api->get_payurl();
//         	var_dump($payment_api);
        	//var_export($payment_url);
        	//exit;
    ...
    public function notify(){
    	...
    	//TODO：根据NOTIFY 回传的 payment_code适配参数，包括订单号，订单状态等，以及写回的字符串
    	//TODO：微信的支付通知需要单独考虑处理，参考PC端微信支付

    	$payment_code = I2("payment_code");
        ...
        $order_type =  I2("extra_common_param");
        $out_trade_no =  I2("out_trade_no");
        $trade_no =  I2("trade_no");
        ...
        if ($result['state']) {
            ...
            'consume_amount'=>daPriceFormat($api_pay_amount),'consume_time'=>TIMESTAMP,'consume_remark'=>$log_desc));
        }
    ...
    public function pay_returns() {
        ...
        if ($payment_state != 'success') {
            ...
            //返回参数判断
            //取得支付结果，这里做了安全校验，所有的支付渠道（除微信外）均需要实现 return_verify 方法，
            //用于对通知做安全校验
        ...
        //支付成功后跳转
        if ($order_type == 'real_order') {
            $pay_ok_url = SHOP_SITE_URL.'&c=Buy&a=pay_ok&pay_sn='.$out_trade_no.'&pay_amount='.daPriceFormat($api_pay_amount);
        } elseif ($order_type == 'vr_order') {
            $pay_ok_url = SHOP_SITE_URL.'&c=BuyVirtual&a=pay_ok&order_sn='.$out_trade_no.'&order_id='.$order_pay_info['order_id'].'&order_amount='.daPriceFormat($api_pay_amount);
        } elseif ($order_type == 'pd_order') {
    ...
    public function qrcode() {
        ...
        require_once BASE_ROOT_PATH.'/Api/phpqrcode/phpqrcode.php';
        \QRcode::png($data);
    }
    ...
    public function wxpay_return() {
        ...
        require_once BASE_ROOT_PATH.'/Api/payment/shop/wxpay/native_notify.php';
    }
    ...
    public function wxpay_notify() {
    	...
    	Log::record("get_params_" . json_encode($_GET) . "_post_params_" . json_encode($_POST));
        ...
        new $payment_code($result['data'],array());
        //回调支付结果，
        //also can get the notify detail data of wx from $GLOALS['wxpay']['pay_data']
        ...
        {
        	//返回 true表示已经通知微信 SUCCESS 否则表示未通知微信
        	$ret = require_once(BASE_ROOT_PATH.'/Api/payment/shop/wxpay/notify.php');
        	$pay_data = $GLOBALS['wxpay']['pay_data'];
        }
        
        //根据微信通知构造支付结果
        Log::record("wxpay_notify_data_____" . json_encode($pay_data));
        $_GET['extra_common_param'] = $pay_data['attach'];
        $_GET['out_trade_no'] = $pay_data['out_trade_no'];
        $_GET['trade_no'] = $pay_data['transaction_id'];
        $_GET['result_code'] = $pay_data['result_code'];
        $_GET['payment_code'] = $payment_code;
        
        $_GET = array_merge($pay_data, $_GET);
...
//         	}
        	...
        	$pay_result = $payment_api->getPayResult($_GET);
        	if (!$pay_result) {
        		showMessage('非常抱歉，您的订单支付没有成功，请联系商家',$url,'html','error');
        	}
...
//         }
    ...
    public function query_state() 
    {
        if ($_GET['pay_id'] && intval($_GET['pay_id']) > 0) {
            ...
            exit(json_encode(array('state'=>($info['api_pay_state'] == '1'),
            		'pay_sn'=>$info['pay_sn'],'type'=>'r')));
            
        } elseif (intval($_GET['order_id']) > 0) {
            $info = Model('vr_order')->getOrderInfo(array('order_id'=>intval($_GET['order_id']),
            		'buyer_id'=>intval($_GET['buyer_id'])));
            exit(json_encode(array('state'=>($info['order_state'] == '20'),
