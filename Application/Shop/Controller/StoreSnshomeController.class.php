...
class StoreSnshomeController extends BaseStoreSnsController{
    public function __construct() {
    	parent::init_view();
        parent::__construct();
        $this->assign('page','store_sns');
    ...
    public function index(){
        ...
        if(!empty($strace_array) && is_array($strace_array)){
            foreach ($strace_array as $key=>$val){
                if($val['strace_content'] == ''){
                    ...
                    $strace_array[$key]['strace_content'] = str_replace("%siteurl%", SHOP_SITE_URL, $content);
                }
            }
        }
        ...
        $this->assign('show_page',$model_stracelog->showpage(2));
    ...
    public function commenttop(){
        ...
        $this->render('store_snscommentlist','null_layout');
    }
    /**
     * 评论列表
     */
    public function commentlist(){
        ...
        $this->assign('commentlist',$commentlist);
        $this->assign('show_page',$model_storesnscomment->showpage(2));
        $this->assign('countnum',$countnum);
        $this->assign('stid',$stid);
...
//      //查询会员信息
        ...
        if ($result){
            ...
            if ($_POST['showtype'] == 1){
                $js .="$('#tracereply_".$stid."').load({$GLOBALS['_PAGE_URL']}&c=StoreSnshome&a=commenttop&id=".$stid."');";
            }else {
                $js .="$('#tracereply_".$stid."').load({$GLOBALS['_PAGE_URL']}&c=storeSnshome&a=commentlist&id=".$stid."');";
            }
    ...
    public function addforward(){
        ...
        $insert_arr['trace_content']            = "<dl class=\"fd-wrap\">
                                                        <dt>
                                                            <h3><a href=\"{$GLOBALS['_PAGE_URL']}&c=StoreSnshome&sid=".$stracelog_info['strace_storeid']."\" target=\"_blank\">".$stracelog_info['strace_storename']."</a>".Language::get('spd_colon')."
    ...
    public function deltrace(){
        ...
        if ($result){
            ...
            $js = "$('[spd_type=\"tracerow_{$stid}\"]').remove();";
    ...
    public function delcomment(){
        ...
        if ($result){
            ...
            $js .="$('.comment-list [spd_type=\"commentrow_".$scid."\"]').remove();";
            showDialog(L('spd_common_del_succ'),'','succ',$js);
        }else {
    ...
    public function straceinfo(){
        ...
        if(!empty($strace_info)){
            if($strace_info['strace_content'] == ''){
                ...
                $strace_info['strace_content'] = str_replace("%siteurl%", SHOP_SITE_URL, $content);
            }
        }
        $this->assign('strace_info', $strace_info);
        //验证码
        $this->assign('shopdamap',substr(md5(SHOP_SITE_URL.convert_word_underscore(CONTROLLER_NAME).ACTION_NAME),0,8));
        $this->render('store_snstraceinfo');
    }
    /**
     * 验证用户是否登录
     */
