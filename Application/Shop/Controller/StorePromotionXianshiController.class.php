...
class StorePromotionXianshiController extends BaseSellerController {

    const LINK_XIANSHI_LIST = 'index.php?m=Shop&c=storePromotionXianshi&a=xianshi_list';
    const LINK_XIANSHI_MANAGE = 'index.php?m=Shop&c=StorePromotionXianshi&a=xianshi_manage&xianshi_id=';
    ...
    public function xianshi_add() {
        ...
        $this->render('store_promotion_xianshi.add');
    ...
    public function xianshi_edit_save() {
        ...
        if($result && $result1) {
            ...
            showDialog(Language::get('spd_common_op_succ'), self::LINK_XIANSHI_LIST,'succ','',3);
        }else {
    ...
    public function xianshi_del() {
        ...
        if($result) {
            ...
            showDialog(L('spd_common_op_succ'), urlShop('store_promotion_xianshi', 'xianshi_list'), 'succ');
        } else {
    ...
    public function xianshi_manage() {
        ...
        $this->assign('show_page', $model_xianshi_goods->showpage());
        $this->assign('xianshi_goods_list', $xianshi_goods_list);

        //输出导航
        self::profile_menu('xianshi_manage');
        $this->render('store_promotion_xianshi.manage');
    }
    ...
    public function xianshi_quota_add() {
        ...
        $this->render('store_promotion_xianshi_quota.add');
    }
    ...
    public function xianshi_quota_add_save() {
        ...
        $this->recordSellerLog('购买'.$xianshi_quota_quantity.'份限时折扣套餐，单价'.$current_price.$lang['spd_yuan']);
    ...
    public function goods_select() {
        ...
        $this->render('store_promotion_xianshi.goods', 'null_layout');
    }
    ...
    public function xianshi_goods_price_edit() {
        ...
        if($result) {
            ...
            $data['result'] = false;
            $data['message'] = L('spd_common_op_succ');
        }
        echo json_encode($data);die;
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'xianshi_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StorePromotionXianshi&a=xianshi_list'),
        );
        switch ($menu_key){
            case 'xianshi_add':
                $menu_array[] = array('menu_key'=>'xianshi_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StorePromotionXianshi&a=xianshi_add');
                break;
            case 'xianshi_edit':
                $menu_array[] = array('menu_key'=>'xianshi_edit','menu_name'=>'编辑活动','menu_url'=>'javascript:;');
                break;
            case 'xianshi_quota_add':
                $menu_array[] = array('menu_key'=>'xianshi_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StorePromotionXianshi&a=xianshi_quota_add');
                break;
            case 'xianshi_manage':
                $menu_array[] = array('menu_key'=>'xianshi_manage','menu_name'=>Language::get('promotion_goods_manage'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Store_PromotionXianshi&a=xianshi_manage&xianshi_id='.$_GET['xianshi_id']);
                break;
        }
