...
class StoreVrBillController extends BaseSellerController {
    /**
     * 每次导出多少条记录
     * @var unknown
     */
    ...
    public function index() {
        ...
        $this->render('store_vr_bill.index');
    }
    ...
    public function show_bill(){
        ...
        $this->assign('sub_tpl_name','store_vr_bill.show.code_list');
        $this->assign('bill_info',$bill_info);
        $this->profile_menu('show',$_GET['type']);
        $this->render('store_vr_bill.show');
    }
    ...
    public function bill_print(){
        ...
        $this->assign('bill_info',$bill_info);
        $this->render('store_vr_bill.print','null_layout');
    }
    ...
    public function export_order(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=StoreVrBill&a=show_bill&ob_id='.$_GET['ob_id']);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'虚拟订单结算', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Bill&a=lists'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'','menu_name'=>'已使用', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrBill&a=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'timeout','menu_name'=>'已过期',  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreVrBill&a=show_bill&type=timeout&ob_id='.$_GET['ob_id'])
                );
                break;
        }
