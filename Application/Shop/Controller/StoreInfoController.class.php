...
class StoreInfoController extends BaseSellerController {
    ...
    public function bind_class() {
        ...
        $this->render('store_bind_class.index');
    }
    ...
    public function bind_class_add() {
        ...
        self::profile_menu('index','bind_class');
        $this->render('store_bind_class.add','null_layout');
    }
    ...
    public function bind_class_save() {
        ...
        if (preg_match('/^[\d,]+$/',$_POST['goods_class'])) {
            ...
            showDialog($lang['spd_common_save_fail']);
        }
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核',$GLOBALS['_PAGE_URL'] . '&c=StoreInfo&a=bind_class','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['spd_common_save_fail']);
        }
    }
    ...
    public function reopen(){
        ...
        //默认选中当前级别
        $this->assign('current_grade_id',$_SESSION['grade_id']);
        ...
        if ($reopen_info) {
            ...
            $this->assign('applay_reopen',$store_info['reopen'] ? true : false);
        }

        self::profile_menu('index','reopen');

        $this->render('store_reopen.index');
    }
...
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
                ...
                showDialog(Language::get('spd_common_save_succ').'，需付款金额'.daPriceFormat($data['re_pay_amount']).'元，请尽快完成付款，付款完成后请上传付款凭证','reload','succ','',5);
            }
        } else {
            showDialog(Language::get('spd_common_del_fail'));
        }
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['spd_member_path_bind_class'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreInfo&a=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['spd_member_path_store_info'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreInfo&a=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['spd_member_path_store_reopen'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreInfo&a=reopen');
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['spd_member_path_bind_class'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBindClass&a=index'),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['spd_member_path_bind_class'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBindClass&a=index'),
                array('menu_key'=>'add', 'menu_name'=>$lang['spd_member_path_bind_class_add'], 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreBindClass&a=add')
                );
                break;
        }
        if(!empty($array)) {
