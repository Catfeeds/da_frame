...
class MemberFavoriteGoodsController extends BaseMemberController{
    ...
    public function fglist(){
        ...
        $this->assign('show_page',$favorites_model->showpage(2));
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),$GLOBALS['_PAGE_URL'] . '&c=MemberFavoriteGoods&a=fglist&show='.$_GET['show'],'succ');
                }
            }else {
    ...
    public function log_msg(){
        ...
        $this->render('favorites_goods_msg','null_layout');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_goods','menu_name'=>Language::get('spd_member_path_collect_list'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberFavoriteGoods&a=fglist'),
            array('menu_key'=>'fav_goodsstore','menu_name'=>'同店商品', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberFavoriteGoods&a=fglist&show=store'),
        );
