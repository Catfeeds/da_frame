...
class SnsAlbumController extends BaseSNSController {
    ...
    public function album_add(){
        ...
        $this->render('sns_album_class_add','null_layout');
    }
    /**
     * 相册保存
     *
    ...
    public function album_add_save(){
        if (chksubmit()){
            ...
            if($class_count >= 10){
                showDialog(Language::get('album_class_save_max_10'),$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum','error');
            }
            $insert = array();
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_edit(){
        ...
        $this->render('sns_album_class_edit','null_layout');
    }
    /**
     * 相册分类编辑保存
     */
    public function album_edit_save(){
        ...
        if($re){
            showDialog(Language::get('album_class_edit_succeed'),$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
        }else{
    ...
    public function album_del(){
        ...
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum','succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
    ...
    public function album_pic_list(){
        ...
        $this->render('sns_album_pic_list');
    }
    /**
     * 修改相册封面
     */
    ...
    public function album_pic_info(){
        ...
        $this->render('sns_album_pic_info');
    }
    ...
    public function album_pic_scroll_ajax(){
        ...
        self::profile_menu('album_pic_info','pic_info');
        $this->render('sns_album_pic_info_scroll_ajax','null_layout');
    }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array();
        $album_menuname = ($this->relation==3?Language::get('sns_me'):'TA').Language::get('spd_sns_path_my_album');
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                    1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum&mid='.$this->master_id),
                );
                break;
            case 'album_pic':
                $menu_array = array(
                    1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum&mid='.$this->master_id),
                    2=>array('menu_key'=>'pic_list','menu_name'=>Language::get('spd_sns_path_album_pic_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum&a=album_pic_list&id='.intval($_GET['id']).'&mid='.$this->master_id),
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                    1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum&mid='.$this->master_id),
                    2=>array('menu_key'=>'pic_info','menu_name'=>Language::get('spd_sns_path_album_pic_info'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsAlbum&a=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id']).'$mid='.$this->master_id),
                );
                break;
        }
