...
class StorePromotionComboController extends BaseSellerController {
    ...
    public function combo_list() {
        ...
        if (checkPlatformStore()) {
            ...
            $this->assign('combo_quota', $combo_quota);
        ...
        if ($hasList) {
            ...
            $this->assign('show_page', $model_combo_goods->showpage());
        }
        
        $this->profile_menu('combo_goods_list', 'combo_goods_list');
        $this->render('store_promotion_combo.goods_list');
    }
     ...
     * 购买套餐
     */
    public function combo_quota_add() {
        if (chksubmit()) {
        ...
        $this->render('store_promotion_combo.quota_add');
    }

    /**
     * 套餐续费
     */
    public function combo_renew() {
        if (chksubmit()) {
            $model_combo = Model('p_combo_quota');
            $quantity = intval($_POST['combo_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_combo_price')); // 扣款数
     ...
     * 选择商品
     */
    public function combo_select_goods() {
        $model_goods = Model('goods');
        $condition = array();
        ...
        $this->render('store_promotion_combo.select_goods', 'null_layout');
    }
    ...
    public function choosed_goods() {
        ...
        if (empty($goods_info) || $goods_info['store_id'] != $_SESSION['store_id'] || !$model_goods->checkIsGeneral($goods_info)) {
            ...
            $this->assign('error', '参数错误，该商品不能参加推荐展位');
        }
        ...
        $this->render('store_promotion_combo.choosed_goods');
    }
