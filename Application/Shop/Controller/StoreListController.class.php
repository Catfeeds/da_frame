...
class StoreListController extends BaseHomeController {
	...
	public function index(){
		...
		if (intval($_GET['cate_id']) > 0){
			$nav_link[1]['link'] = $GLOBALS['_PAGE_URL'] . '&c=StoreList';
			...
			if ($nav['sc_parent_id'] > 0){
				...
				$nav_link[] = array(
					'title'=>$tmp['sc_name'],
					'link'=>"{$GLOBALS['_PAGE_URL']}&c=StoreList&cate_id=".$nav['sc_parent_id']
				);
			}
			//存入当前级
			$nav_link[] = array(
			...
			$nav_link[1]['link'] = "{$GLOBALS['_PAGE_URL']}";
		...
		$this->assign('purl', urlShop(convert_word_underscore(CONTROLLER_NAME), ACTION_NAME, $purl['param']));

		//SEO
		Model('seo')->type('index')->show();
		$this->assign('html_title',(empty($_GET['keyword']) ? '' : $_GET['keyword'].' - ').C('site_name').$lang['spd_common_search']);
