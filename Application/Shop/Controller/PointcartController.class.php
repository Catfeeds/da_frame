...
class PointcartController extends BasePointShopController {
    public function __construct() {
        ...
        if (C('pointprod_isuse') != 1){
            showDialog(L('pointcart_unavailable'),"{$GLOBALS['_PAGE_URL']}",'error');
        }
        //验证是否登录
    ...
    public function add() {
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=plist')); die;
                    break;
                default:
    ...
    public function step1(){
        ...
        if (!$data['state']){
            showDialog($data['msg'],$GLOBALS['_PAGE_URL'] . '&c=Pointprod','error');
        }
    ...
    public function step2() {
        ...
        if (!$data['state']){
            showDialog($data['msg'],$GLOBALS['_PAGE_URL'] . '&c=Pointcart','error');
        }
        ...
        if (!$data['state']){
            showDialog($data['msg'],$GLOBALS['_PAGE_URL'] . '&c=Pointcart&a=step1','error');
        }
        $order_id = $data['data']['order_id'];
        @header("Location:{$GLOBALS['_PAGE_URL']}&c=Pointcart&a=step3&order_id=".$order_id);
    }
    /**
     * 流程第三步
     */
    public function step3($order_arr=array()) {
        ...
        if ($order_id <= 0){
            showDialog(L('pointcart_record_error'),"{$GLOBALS['_PAGE_URL']}",'error');
        }
        $where = array();
