...
class StoreDeliverSetController extends BaseSellerController {
    ...
    public function daddress_list() {
       ...
       $this->render('store_deliver_set.daddress_list');
    }
    ...
    public function daddress_add() {
        ...
        if (chksubmit()) {
            ...
            showDialog($lang['spd_common_op_succ'],'reload','succ','CUR_DIALOG.close()');
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['store_daddress_wrong_argument'],$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=daddress_list','html','error');
            }
            $this->assign('address_info',$address_info);
        }
        $this->render('store_deliver_set.daddress_add','null_layout');
    }
    ...
    public function daddress_del() {
        ...
        if ($delete){
            showDialog(Language::get('store_daddress_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=daddress_list','succ');
        }else {
            showDialog(Language::get('store_daddress_del_fail'),'','error');
        }
    }
   ...
   public function daddress_default_set() {
    ...
    public function express() {
        ...
        $this->render('store_deliver_express');
    }
    ...
    public function free_freight(){
        ...
        $this->render('store_free_freight.index');
    }
    ...
    public function deliver_region(){
        ...
        $this->render('store_default_deliver_region.index');
    }
    ...
    public function print_set(){
        ...
        if(empty($store_info)){
            showDialog(Language::get('store_storeinfo_error'),$GLOBALS['_PAGE_URL'] . '&c=StorePrintsetup','error');
        }
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch ($menu_type) {
            case 'daddress':
                $menu_array = array(
                array('menu_key'=>'daddress',   'menu_name'=>Language::get('store_deliver_daddress_list'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=daddress_list'),
                array('menu_key'=>'express',        'menu_name'=>Language::get('store_deliver_default_express'),    'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=express'),
                array('menu_key'=>'free_freight',   'menu_name'=>'免运费额度',      'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=free_freight'),
                array('menu_key'=>'deliver_region',   'menu_name'=>'默认配送地区',      'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=deliver_region'),
                array('menu_key'=>'print_set', 'menu_name'=>'发货单打印设置', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreDeliverSet&a=print_set')
                );
                break;
        }
