...
class BuyController extends BaseBuyController {
    ...
    public function buy_step1() {
        ...
        // 加价购
        $this->assign('jjgValidSkus', $result['jjgValidSkus']);
        $this->assign('jjgStoreCosts', $result['jjgStoreCosts']);
        ...
        //返回平台可用红包
        $this->assign('rpt_list_json', json_encode($result['rpt_list']));
        ...
        //输出是否有门店自提支付
        $this->assign('ifshow_chainpay', $result['ifshow_chainpay']);
        $this->assign('chain_store_id', $result['chain_store_id']);
        ...
        $this->assign('ifchain', $_POST['ifchain']);

        //输出会员折扣
        $this->assign('zk_list',$result['zk_list']);
        ...
        $this->assign('current_goods_info',$current_goods_info[0]);

        $this->render('buy_step1');
    }
    ...
    public function buy_step2() {
        ...
        //转向到商城支付页面
		$url = url('buy', 'pay', array('pay_sn' => $result['data']['pay_sn']));
        redirect($url);
    }
    ...
    public function pay() {
        ...
        if ($order_list[0]['if_buyer_repay'] && $order_list[0]['pay_sn1'] == '') {
            ...
            if (!$update) {
				...
				$url = url('buy', 'pay', array('pay_sn' => $pay_sn_new));
                redirect($url);exit;
            }
        }
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
			...
			$url = url('buy', 'pay_ok', array('pay_sn' => $pay_sn, 'is_chain' => $flag_chain, 'pay_amount' => daPriceFormat($order_info['order_amount'])));
            redirect($url);
        }
...
//             }
        ...
        //var_dump($this->getTplLayoutPath("buy_step2"));
        
        $this->render('buy_step2');
    }
    ...
    public function load_addr() {
        ...
        $this->render('buy_address.load','null_layout');
    }

    /**
     * 载入门店自提点
     */
    public function load_chain() {
    ...
    public function change_chain() {
     ...
     public function add_addr(){
        ...
        if (chksubmit()){
            ...
            $this->render('buy_address.add','null_layout');
        }
     }
     ...
     public function add_chain(){
         $this->render('buy_address.add_chain','null_layout');
     }
    ...
    public function load_inv() {
        ...
        $this->assign('inv_list',$list);
        $this->render('buy_invoice.load','null_layout');
    }
        ...
        if (chksubmit()){
            ...
            if ($_POST['invoice_type'] == 2) {
                if (empty($_POST['inv_company']) || empty($_POST['inv_code']) || empty($_POST['inv_reg_addr'])) {
                    exit(json_encode(array('state'=>false,'msg'=>Language::get('spd_common_save_fail','UTF-8'))));
                }
            }
            $data = array();
            ...
            if ($insert_id) {
                ...
                exit(json_encode(array('state'=>'fail','msg'=>Language::get('spd_common_save_fail','UTF-8'))));
            }
        } else {
            $this->render('buy_address.add','null_layout');
        }
    ...
    private function _buy_branch($post) {
        if (!$post['ifcart']) {
            ...
            if ($goods_info['is_virtual']) {
				$url = url('buy_virtual', 'buy_step1', array('goods_id' => $goods_id, 'quantity' => $quantity));
                redirect($url);
            }
        }
    }
