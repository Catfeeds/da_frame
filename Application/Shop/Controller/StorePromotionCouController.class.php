...
class StorePromotionCouController extends BaseSellerController
{
    const LINK_COU_LIST = 'index.php?m=Shop&c=StorePromotionCou&a=cou_list';
    ...
    public function cou_list()
    {
        if (checkPlatformStore()) {
            ...
            $this->assign('current_cou_quota', $current_cou_quota);
        }
        ...
        $this->assign('list', $couModel->getCouList($condition, 10, 'state desc, tend desc'));
        $this->assign('show_page', $couModel->showpage());
        $this->assign('couStates', $couModel->getCouStates());

        self::profile_menu('cou_list');
        $this->render('store_promotion_cou.list');
    }

    /**
     * 加价购套餐购买
     */
    public function cou_quota_add()
    {
        // 输出导航
        $this->profile_menu('cou_quota_add');
        $this->render('store_promotion_cou.quota_add');
    }

    /**
     * 加价购套餐购买保存
     */
    public function cou_quota_add_save()
    {
        ...
        $this->recordSellerLog('购买' . $cou_quota_quantity . '份加价购套餐，单价' . $current_price . $lang['spd_yuan']);
    ...
    public function cou_add()
    {
        ...
        $this->render('store_promotion_cou.add');
    }

    /**
     * 保存添加的加价购活动
     */
    public function cou_add_save()
    {
        // 验证输入
        ...
        if ($result) {
            ...
            $editUrl = $GLOBALS['_PAGE_URL'] . '&c=StorePromotionCou&a=cou_edit&cou_id=' . $result;
    ...
    public function cou_edit()
    {
        ...
        $this->render('store_promotion_cou.edit');
    }

    /**
     * 保存编辑的加价购活动
     */
    public function cou_edit_save()
    {
        // 验证输入
        ...
        try {
            ...
            showDialog(Language::get('spd_common_op_fail') . ': ' . $ex->getMessage());
        }

        $this->recordSellerLog('编辑加价购活动，活动名称：' . $cou_name . '，活动编号：' . $couId);
        showDialog(Language::get('spd_common_op_succ'), self::LINK_COU_LIST, 'succ', '', 3);
    }
    ...
    public function cou_del()
    {
        ...
        if ($result) {
            ...
            showDialog(L('spd_common_op_succ'), self::LINK_COU_LIST, 'succ');
        } else {
    ...
    public function cou_level_sku()
    {
        ...
        $this->assign('level', (int) $_REQUEST['level']);

        $this->render('store_promotion_cou.cou_level_sku', 'null_layout');
    }

    /**
     * 异步设置参与加价购活动的商品
     */
    public function cou_sku()
    {
        $model_goods = Model('goods');
        ...
        $this->render('store_promotion_cou.cou_sku', 'null_layout');
    }
    ...
    private function profile_menu($menu_key='')
    {
        $menu_array = array(
            1 => array(
                'menu_key' => 'cou_list',
                'menu_name' => '活动列表',
                'menu_url' => $GLOBALS['_PAGE_URL'] . '&c=StorePromotionCou&a=cou_list',
            ),
        );
        switch ($menu_key) {
            case 'cou_quota_add':
                $menu_array[] = array(
                    ...
                    'menu_name' => '购买套餐',
                    'menu_url' => $GLOBALS['_PAGE_URL'] . '&c=StorePromotionCou&a=cou_add',
                );
                break;
            case 'cou_add':
                $menu_array[] = array(
