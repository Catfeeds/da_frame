...
class MemberSnshomeController extends BaseSNSController {
    ...
    public function shareglist(){
        ...
        if ($_GET['type'] == 'like'){
            $this->render('sns_likegoodslist');
        }else {
            $this->render('sns_sharegoodslist');
        }
    }
    /**
     * 分享和喜欢商品详细页面
    ...
    public function goodsinfo(){
        ...
        if ($share_id <= 0){
            showDialog(Language::get('wrong_argument'),"{$GLOBALS['_PAGE_URL']}&c=MemberSnshome&mid={$this->master_id}",'error');
        }
        //查询分享和喜欢商品信息
        $sharegoods_model = Model('sns_sharegoods');
        $condition = array();
    ...
    public function commenttop(){
        ...
        $this->render('sns_commentlist','null_layout');
    }
    /**
     * 评论列表
     */
    ...
    public function tracelist(){
        ...
        $this->assign('shopdamap',substr(md5(SHOP_SITE_URL.convert_word_underscore(CONTROLLER_NAME).ACTION_NAME),0,8));
        $this->assign('menu_sign', 'snstrace');
        $this->render('sns_tracelist','null_layout');
    }
    /**
     * 一条SNS动态及其评论
     */
    public function traceinfo(){
        ...
        if (!empty($tracelist)){
            ...
            if ($traceinfo['trace_title']){
                $traceinfo['trace_title'] = str_replace("%siteurl%", SHOP_SITE_URL, $traceinfo['trace_title']);
            ...
            if(!empty($traceinfo['trace_content'])){
                //替换内容中的siteurl
                $traceinfo['trace_content'] = str_replace("%siteurl%", SHOP_SITE_URL, $traceinfo['trace_content']);
            }
        }
    ...
    public function add_share(){
        ...
        $this->assign('sharegoods_info', $sharegoods_info);
        $this->assign('sid', $sid);
        $this->render('sns_addshare', 'null_layout');
    }
    /**
     * ajax图片上传
     */
