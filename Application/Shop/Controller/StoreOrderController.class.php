...
class StoreOrderController extends BaseSellerController {
    ...
    private function _order_cancel($order_info, $post) {
        ...
        if(!chksubmit()) {
            ...
            $this->render('store_order.cancel','null_layout');
            exit();
         } else {
    ...
    private function _order_ship_price($order_info, $post) {
        ...
        if(!chksubmit()) {
            $this->assign('order_info',$order_info);
            $this->assign('order_id',$order_info['order_id']);
            $this->render('store_order.edit_price','null_layout');
            exit();
        } else {
	...
	private function _order_spay_price($order_info, $post) {
	    ...
	    if(!chksubmit()) {
	        $this->assign('order_info',$order_info);
	        $this->assign('order_id',$order_info['order_id']);
            $this->render('store_order.edit_spay_price','null_layout');
            exit();
        } else {
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
            $menu_array = array(
            array('menu_key'=>'store_order',        'menu_name'=>Language::get('spd_member_path_all_order'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder'),
            array('menu_key'=>'state_new',          'menu_name'=>Language::get('spd_member_path_wait_pay'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder&a=index&state_type=state_new'),
            array('menu_key'=>'state_pay',          'menu_name'=>Language::get('spd_member_path_wait_send'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder&a=store_order&state_type=state_pay'),
            array('menu_key'=>'state_notakes',      'menu_name'=>'待自提', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder&a=store_order&state_type=state_notakes'),
            array('menu_key'=>'state_send',         'menu_name'=>Language::get('spd_member_path_sent'),      'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder&a=index&state_type=state_send'),
            array('menu_key'=>'state_success',      'menu_name'=>Language::get('spd_member_path_finished'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder&a=index&state_type=state_success'),
            array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('spd_member_path_canceled'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreOrder&a=index&state_type=state_cancel'),
            );
            break;
        }
