...
class CartController extends BaseBuyController {
    ...
    public function __construct() {
        ...
        //允许不登录就可以访问的op
        $a_arr = array('ajax_load','add','del');
        if (!in_array($a,$a_arr) && !$_SESSION['member_id'] ){
        	$urlLogin = urlLogin('login', 'index', array('ref_url' => request_uri()));
            redirect($urlLogin);
        }
    ...
    public function index() {
        ...
        // 加价购活动
        $this->assign('jjgDetails', $jjgObj->details);
        ...
        $this->render(empty($cart_list) ? 'cart_empty' : 'cart');
    }
    ...
    public function ajax_load() {
        ...
        setDaCookie('cart_goods_num',$model_cart->cart_goods_num,2*3600);
        $cart_array['cart_all_price'] = daPriceFormat($model_cart->cart_all_price);
        $cart_array['cart_goods_num'] = $model_cart->cart_goods_num;
        if ($_GET['type'] == 'html') {
            $this->assign('cart_list',$cart_array);
            $this->render('cart_mini','null_layout');
        } else {
    ...
    public function add() {
        ...
        if ($insert) {
            //购物车商品种数记入cookie
            setDaCookie('cart_goods_num',$model_cart->cart_goods_num,2*3600);
            $data = array('state'=>'true', 'num' => $model_cart->cart_goods_num, 'amount' => daPriceFormat($model_cart->cart_all_price));
        } else {
