...
class PublishController extends MircroShopController{
    ...
    public function goods_buy() {
        ...
        $goods_list = $model_order->getOrderGoodsList($condition, '*', null, 20, "rec_id desc", "goods_id");
        
        //var_dump($goods_list);
        
        $this->assign('list',$goods_list);
        $this->assign('goods_type','buy');
        $this->assign('show_page',$model_order->showpage());
        $this->get_commend_goods_list();
        //获得分享app列表
        self::get_share_app_list();
        $this->render('publish_goods');
    }

    public function goods_favorites() {
        ...
        $this->assign('list',$goods_list);
        $this->assign('goods_type','favorites');
        $this->assign('show_page',$model_favorites->showpage());
        $this->get_commend_goods_list();
    ...
    private function get_commend_goods_list() {
        ...
        $this->assign('commend_goods_array',$commend_goods_array);
    }

    public function goods_save() {
    	$_POST['commend_message'] = trim($_POST['commend_message']);
        $model_goods = Model('goods');
        $model_microshop_goods = Model('micro_goods');
        ...
        $message = Language::get('spd_common_save_fail');
            //分享内容
            if($result) {
                $message = Language::get('spd_common_save_succ');
                //计数
                ...
                if(isset($_POST['share_app_items'])) {
                    ...
                    $commend_goods_info['url'] = MICROSHOP_SITE_URL ."&c=Goods&a=detail&goods_id=".$result;
     ...
     * 个人秀图片上传
     **/
    public function personal_image_upload() {
        $data = array();
        $data['status'] = 'success';
     ...
     * 个人秀图片删除
     **/
    public function personal_image_delete() {
        $data = array();
        $data['status'] = 'success';
     ...
     * 个人秀数量限制检查
     **/
    public function personal_limit() {
     ...
     * 个人秀购买链接添加
     */
    public function personal_link_add() {
    ...
    public function personal_save() {
        ...
        if($result) {
            ...
            if(isset($_POST['share_app_items'])) {
                ...
                $personal_info['url'] = MICROSHOP_SITE_URL."&c=Personal&a=detail&personal_id=".$result;
                self::share_app_publish('publish',$personal_info);
            }
        }
        showDialog($message,MICROSHOP_SITE_URL . '&c=Home&a=personal',$result? 'succ' : 'error','');
    }



    public function album() {
        $this->render('publish_album');
    }
}
