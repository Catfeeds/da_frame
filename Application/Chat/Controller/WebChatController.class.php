...
use Chat\Controller\BaseController;
...
class WebChatController extends BaseController {
    ...
    public function send_msg(){
        ...
        if (($member_id < 1) || ($member_id != $f_id)) $this->error(Language::get('spd_member_chat_login'));

        $member = $model_chat->getMember($t_id);

        if ($t_name != $member['member_name']) $this->error(Language::get('spd_member_chat_name_error'));
        ...
        if($chat_msg['m_id']) {
            ...
            $this->error(Language::get('spd_member_chat_add_error'));
    ...
    public function get_info(){
        ...
        if ($_SESSION['member_id'] < 1) $this->error(Language::get('spd_member_chat_login'));
        ...
        if ($s_id > 0) {
            ...
            if(!empty($list) && is_array($list)) {
                ...
                foreach($list as $k => $v) {
                    ...
                    $v['goods_promotion_price'] = daPriceFormat($v['goods_promotion_price']);
