...
use Common\Lib\Sms;
use Common\Lib\Tpl;
use Common\Lib\SendMemberMsg;
use Common\Lib\SendStoreMsg;
...
class queueLogic {
    ...
    public function createGoodsQRCode($param) {
        ...
        // 生成商品二维码
        require_once(BASE_ROOT_PATH . DS . 'Api'  . DS .'phpqrcode'.DS.'index.php');
        ...
        foreach ($param['goodsid_array'] as $goods_id) {
            // 生成商品二维码
            $PhpQRCode->set('data',WAP_SITE_URL.DS.tmpl.DS.product_detail.'.'.html.'?'.goods_id.'='.$goods_id);
    ...
    public function sendVrCode($param) {
        ...
        $message    = daReplaceText($tpl_info['content'],$data);
        $sms = new Sms();
    ...
    public function flushIndexer() {
        try {
            require_once(BASE_ROOT_PATH . DS . 'Api' . '/xs/lib/XS.php');
    ...
    public function updateXS($goods_ids) {
        try {
            require_once(BASE_ROOT_PATH.'/Api/xs/lib/XS.php');
            $obj_doc = new XSDocument();
            $obj_xs = new XS(C('fullindexer.appname'));
