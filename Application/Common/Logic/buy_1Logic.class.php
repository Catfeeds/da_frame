...
class buy_1Logic {
    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
                $store_cart[$key]['goods_total'] = daPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
            ...
            $store_goods_total[$store_id] = daPriceFormat($tmp_amount);
        }
        ...
        if (array_key_exists('store_id', $goods_list)) {
            if (array_key_exists($goods_list['store_id'], $store_order_discount)) {
                ...
                if (!$goods_list['is_book']) {
                    $goods_list['goods_price'] = daPriceFormat($goods_list['goods_price'] * $store_order_discount[$goods_list['store_id']]/100);
                    $goods_list['zkj'] = true;                     
                }
            }
        } else {
                ...
                if (array_key_exists($goods_info['store_id'], $store_order_discount)) {
                    ...
                    if (is_array($goods_info['bl_goods_list']) && !empty($goods_info['bl_goods_list'])) {
                        foreach ($goods_info['bl_goods_list'] as $kk => $vv) {
                            $goods_list[$k]['bl_goods_list'][$kk]['bl_goods_price'] = daPriceFormat($vv['bl_goods_price'] * $orderdiscount/100);
                            $goods_list[$k]['bl_goods_list'][$kk]['zkj'] = true;
                        }
                    }
                    $goods_list[$k]['goods_price'] = daPriceFormat($goods_info['goods_price'] * $orderdiscount/100);
    ...
    public function getMansongRuleCartListByTotal($store_goods_total) {
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            if (is_array($rule_info) && !empty($rule_info)) {
                ...
                $rule_info['discount'] = daPriceFormat($rule_info['discount']);
            ...
            case 'rpt':
                foreach ($store_goods_total as $store_id => $goods_total) {
                    if (array_key_exists($store_id, $preferential_array)) {
                        $store_goods_total[$store_id] = daPriceFormat($store_goods_total[$store_id]*$preferential_array[$store_id]/100);
                    }
                }
                break;
    ...
    public function getStoreAvailableVoucherList($store_goods_total, $member_id) {
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            $voucher_list[$store_id] = $model_voucher->getCurrentAvailableVoucher($condition,$goods_total, MODULE_NAME == 'Mobile' ? 'voucher_limit desc' : '');
        }
        return $voucher_list;
    }
    ...
    public function getXianshiInfo( & $goods_info, $quantity) {
        ...
        $goods_info['xianshi_info']['down_price'] = daPriceFormat($goods_info['goods_price'] - $goods_info['xianshi_info']['xianshi_price']);
    ...
    /**
     *    shopda 跟据商品金额返回免运费店铺ID及免运费下限金额描述[已设置并达到免运费金额标准]
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        foreach ($buy_list as $key => $goods_info) {
            ...
            if (!intval($goods_info['transport_id']) &&  !in_array($goods_info['store_id'],$free_freight_sid_list)) {
                $freight_list[$goods_info['store_id']] += $goods_info['goods_freight'];//   shopda
                unset($buy_list[$key]);
            }
        }
        //计算优惠套装商品运费
    ...
    public function calcStoreFreight($freight_list, $city_id) {
        ...
        $tpl_trans_list = $freight_list['tpl_trans_list'];// shopda
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        $available_rcb_amount = floatval(daPriceFormat($buyer_info['available_rc_balance']));
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                $available_rcb_amount = floatval(daPriceFormat($available_rcb_amount));
                //记录订单日志(已付款)
                $data = array();
                ...
                if ($order_info['order_type'] == 2) {
                    $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = daPriceFormat(floatval($order_info['original_rcb_amount']) + $order_amount);
                } else {
                ...
                if ($available_rcb_amount > 0) {
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = daPriceFormat(floatval($order_info['original_rcb_amount']) + $available_rcb_amount);
                    } else {
    ...
    public function pdPay($order_list, $input, $buyer_info) {
        ...
        $available_pd_amount = floatval(daPriceFormat($buyer_info['available_predeposit']));
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $order_amount = floatval(daPriceFormat($order_amount));
            $data_pd = array();
            $data_pd['member_id'] = $member_id;
            $data_pd['member_name'] = $member_name;
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $available_pd_amount = floatval(daPriceFormat($available_pd_amount));
                ...
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'predeposit';
                if ($order_info['order_type'] == 2) {
                    $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = daPriceFormat(floatval($order_info['original_pd_amount']) + $order_amount);
                } else {
                ...
                if ($available_pd_amount > 0) {
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = daPriceFormat(floatval($order_info['original_pd_amount']) + $available_pd_amount);
                    } else {
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array['goods_trans_v'] = $goods_info['goods_trans_v'];// shopda
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $cart_list[$key]['goods_trans_v'] = $goods_online_info['goods_trans_v'];// shopda
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_info) {
                if (array_key_exists($goods_info['goods_id'],$goods_online_list)) {
                    ...
                    //每个商品直降多少
                    $total_down_price += $cart_list[$key]['bl_goods_list'][$k]['down_price'] = daPriceFormat($goods_online_info['goods_price'] - $goods_info['bl_goods_price']);
                } else {
                    //商品已经下架
                    $cart_list[$key]['state'] = false;
                    $cart_list[$key]['storage_state'] = false;
            ...
            $cart_list[$key]['down_price'] = daPriceFormat($total_down_price);
        }
    }
    ...
    private function _parseMansongRuleDesc($rule_info) {
        ...
        if (MODULE_NAME == 'Mobile') {
            ...
            if (!empty($rule_info['mansong_goods_name'])) {
                ...
                $goods_desc .= '<a class="spdc-store-gift" title="'.$rule_info['mansong_goods_name'].'" target="_blank" href="'.urlShop('goods','index',array('goods_id'=>$rule_info['goods_id'])).'"><img alt="'.$rule_info['mansong_goods_name'].'" src="'.cthumb($rule_info['goods_image'], 60).'"></a>';                    
            }
