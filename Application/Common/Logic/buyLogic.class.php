    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0) {
        ...
        foreach ((array) $store_goods_total as $k => $v) {
            if (isset($jjgStoreCosts[$k])) {
                $v += $jjgStoreCosts[$k];
                $store_goods_total[$k] = daPriceFormat($v);
            }
        }
...
//     	var_dump($freight_hash, $city_id, $area_id, $member_id);
...
//                 $order_common['promotion_info'] .= '<dl class="spd-store-sales"><dt>会员等级折扣</dt><dd>'.addslashes(sprintf(' [V%s]级会员享受原价%s折',$member_level,$orderdiscount[$store_id]/10)).'</dd></dl>';
//             }
    ...
    private function _getGoodsList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['goods_trans_v'] = $cart['goods_trans_v'];// shopda
