...
class memberModel extends Model {
	...
	*/
    ...
    public function auto_login() {
        // 自动登录标记 保存7天
        setDaCookie('auto_login', encrypt($_SESSION['member_id'], MD5_KEY), 7*24*60*60);
    }
    
    public function set_avatar_cookie() {
        setDaCookie('member_avatar', $_SESSION['avatar'], 365*24*60*60);
    }
    ...
    public function batchGetMemberInfo($member_id_list, $is_hash = false)
    {
    	$ret = array();
    	...
    	$in_str = "(" . implode(",", $member_id_list) . ")";
    	$member_list = $this->where("member_id in $in_str")->select();
