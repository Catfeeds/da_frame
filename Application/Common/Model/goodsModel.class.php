...
class goodsModel extends Model{
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0) {
	    ...
	    $_field = "CONCAT(goods_commonid) as spd_distinct ,". $field;
			$_distinct = 'spd_distinct';
		}
        ...
        $goods_list = array();
        if (C('dbdriver') == 'mysqli') {
            $goods_list = $this->getGoodsOnlineList($condition, $_field.' spd_distinct,'.$field, $page, $order, $limit, $_distinct, false, $count);
        }
        return $goods_list;
    }
    ...
    public function getGoodsInfoAndPromotionById($goods_id) {
        ...
        // 手机专享
        if (C('promotion_allow') && MODULE_NAME == 'Mobile') {
    ...
    public function getGoodsDetail($goods_id) {
        ...
        if ($goods_info['is_book'] != '1') {
            ...
            if (!empty($goods_info['xianshi_info'])) {
                ...
                $goods_info['promotion_price'] = $goods_info['xianshi_info']['xianshi_price'];
                $goods_info['down_price'] = daPriceFormat($goods_info['goods_price'] - $goods_info['xianshi_info']['xianshi_price']);
            ...
            if (!empty($goods_info['groupbuy_info'])) {
                ...
                $goods_info['promotion_price'] = $goods_info['groupbuy_info']['groupbuy_price'];
                $goods_info['down_price'] = daPriceFormat($goods_info['goods_price'] - $goods_info['groupbuy_info']['groupbuy_price']);
