...
class mb_paymentModel extends Model {
    ...
    public function hashPaymentList($paymentList)
    {
    	$ret = array();
    	foreach ($paymentList as $item)
    	{
    		$ret[$item['payment_code']] = $item;
    	}
    	return $ret;
    }
    
    public function getPaymentConf($paymentCode)
    {
    	$ret = array();
    	$payment_list = $this->getMbPaymentOpenList();
    	$payment_hash = $this->hashPaymentList(array_values($payment_list));
    	if (isset($payment_hash[$paymentCode]))
    	{
    		$ret = $payment_hash[$paymentCode];
    		$ret['payment_config'] = unserialize($ret['payment_config']);
    	}
    	return $ret;
    }
}
