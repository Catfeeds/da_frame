...
class wxapp_specialModel extends Model{
    ...
    public function __construct() {
        parent::__construct('wxapp_special');
    }
    ...
    public function getMbSpecialList($condition, $page='', $order='special_id desc', $field='*') {
        $list = $this->table('wxapp_special')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    ...
    public function getMbSpecialInfoByID($special_id) {
        ...
        $info = $this->table('wxapp_special')->where($condition)->find();
        return $info;
    }
    ...
    public function addMbSpecial($param){
        return $this->table('wxapp_special')->insert($param);
    }
    /*
     * 更新专题
     * @param array $update
    ...
    public function editMbSpecial($update, $special_id) {
        ...
        $condition = array();
        $condition['special_id'] = $special_id;
        $result = $this->table('wxapp_special')->where($condition)->update($update);
        if($result) {
            //删除缓存
    ...
    public function delMbSpecialByID($special_id) {
        ...
        return $this->table('wxapp_special')->where($condition)->delete();
    }
    ...
    public function getMbSpecialdesc($special_id){
        $condition = array();
        $condition['special_id'] = $special_id;
        $list = $this->table('wxapp_special')->where($condition)->find();
    ...
    private function _getMbSpecialItemList($condition, $order = 'item_sort asc') {
        $item_list = $this->table('wxapp_special_item')->where($condition)->order($order)->select();
    ...
    public function isMbSpecialItemExist($condition) {
        $item_list = $this->table('wxapp_special_item')->where($condition)->select();
        if($item_list) {
            return true;
        } else {
            return false;
    ...
    public function getMbSpecialItemInfoByID($item_id) {
        ...
        $item_info = $this->table('wxapp_special_item')->where($condition)->find();
    ...
    public function addMbSpecialItem($param) {
        ...
        $param['item_sort'] = 255;
        $result = $this->table('wxapp_special_item')->insert($param);
        //删除缓存
        if($result) {
            //删除缓存
    ...
    public function editMbSpecialItemByID($update, $item_id, $special_id) {
        ...
        return $this->table('wxapp_special_item')->where($condition)->update($update);
    }
    ...
    public function delMbSpecialItem($condition, $special_id) {
        ...
        return $this->table('wxapp_special_item')->where($condition)->delete();
    }
    ...
    private function _delMbSpecialCache($special_id) {
        //清理缓存
        dcache($special_id, 'wxapp_special');
