...
class refund_returnModel extends Model{
    ...
    public function editOrderRefund($refund,$user = '') {
        ...
        if ($refund_id > 0) {
            Language::read('model_lang_index', 'Home');
            ...
            try {
                ...
                if ($state) {
                    ...
                    $order_array['refund_amount'] = daPriceFormat($refund_amount);
    ...
    public function addDetail($refund,$order) {
        ...
        $detail_array['refund_amount'] = daPriceFormat($refund['refund_amount']);
        ...
        if (!empty($order['trade_no']) && in_array($order['payment_code'],array('wxpay','wx_jsapi','wx_saoma'))) {//微信支付
            $api_file = BASE_ROOT_PATH . DS . 'Api'.DS.'refund'.DS.'wxpay'.DS.'WxPay.Config.php';
            if ($order['payment_code'] == 'wxpay') {
                $api_file = BASE_ROOT_PATH.DS.'Api'.DS.'refund'.DS.'wxpay'.DS.'WxPayApp.Config.php';
            }
    ...
    public function getPayDetailInfo($detail_array) {
        ...
        $order['pay_amount'] = daPriceFormat($order['order_amount']-$order['rcb_amount']-$order['pd_amount']);//在线支付金额=订单总价格-充值卡支付金额-预存款支付金额
        ...
        $order['pay_refund_amount'] = daPriceFormat($refund_amount);
        $condition = array();
        ...
        foreach ($order_list as $key => $order_info) {//在线支付金额
            $pay_amount += daPriceFormat($order_info['order_amount']-$order_info['rcb_amount']-$order_info['pd_amount']);
        }
        $order['pay_amount'] = daPriceFormat($pay_amount);
        return $order;
    }
    ...
    public function getRightOrderList($order_condition, $order_goods_id = 0){
        ...
        $this->assign('order_common',$order_common);
        if ($order_common['shipping_express_id'] > 0) {
            $express = rkcache('express',true);
            $this->assign('e_code',$express[$order_common['shipping_express_id']]['e_code']);
            $this->assign('e_name',$express[$order_common['shipping_express_id']]['e_name']);
        }
