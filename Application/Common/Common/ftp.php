...
use Common\Lib\Daftp;

/**
 * 通过FTP同步图片到远程服务器
 *
 * @param string $path
 *        	图片路径（upload/store/goods/4）
 * @param string $file
 *        	图片名称（含年月日2012/06/12/03f625d923bfb1ca84355007487ed68b.jpg）
 * @param boolean $ifdel
 *        	是否删除本地图片，目前淘宝导入的图片上传到远程时，不会删除本地图片
...
function _dfsockopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE, $encodetype = 'URLENCODE', $allowcurl = TRUE) {
	...
	if (function_exists ( 'curl_init' ) && function_exists ( 'curl_exec' ) && $allowcurl) {
		$ch = curl_init ();
		$ip && curl_setopt ( $ch, CURLOPT_HTTPHEADER, array (
				"Host: " . $host 
		) );
