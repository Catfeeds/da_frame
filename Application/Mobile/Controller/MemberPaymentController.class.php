...
class MemberPaymentController extends MobileMemberController {
    ...
    public function __construct() {
        ...
        if($_GET['a'] != 'payment_list' && !$payment_code) {

            $payment_code = 'alipay';

            if(in_array($_GET['a'], array('wx_app_pay', 'wx_app_pay3', 'wx_app_vr_pay', 'wx_app_vr_pay3'), true)) {
                $payment_code = 'wxpay';
            }
            else if (in_array($_GET['a'],array('alipay_native_pay','alipay_native_vr_pay'),true)) {
        ...
        else 
        {
        	...
        	$payment_config['openId'] = $this->wx_openid;
        	$payment_config['unionId'] = $this->wx_unionid;

        	$this->payment_config = $payment_config;
        	
//        	var_dump($this->wx_unionid, $this->payment_config);
//         	var_dump($this->wx_openid, $this->wx_unionid);
//         	exit;
...
//         var_dump($pay_sn, $pay_info, $_POST['key']);
//         exit;
    ...
    private function _api_pay($order_pay_info) {
        ...
        $inc_file = BASE_ROOT_PATH . DS.'Api'.DS.'payment/mobile'.DS.$this->payment_code.DS.$this->payment_code.'.php';
        ...
        //TODO 目前问题 支付微信小程序APPID 和 OPENID生成使用的APPID 不匹配
        //var_dump($param);
        
        // wxpay_jsapi
        if ($this->payment_code == 'wxpay_jsapi') {
            $param['orderSn'] = $order_pay_info['pay_sn'];
            $param['orderFee'] = (int) (daPriceFormat(100 * $order_pay_info['api_pay_amount']));
        ...
        // wxpay_wxapp
        if ($this->payment_code == 'wxpay_wxapp') {
        	$param['orderSn'] = $order_pay_info['pay_sn'];
        	$param['orderFee'] = (int) (daPriceFormat(100 * $order_pay_info['api_pay_amount']));
        	$param['orderInfo'] = $order_pay_info['pay_sn'] . '订单';
        	$param['orderAttach'] = ($order_pay_info['order_type'] == 'real_order' ? 'r' : 'v');
        	$param['openId'] = $this->wx_openid;
        	$api = new \wxpay_wxapp();
        	...
        	$api->setConfigs($param);
        	try {
        		$ret = $api->getPayRequestData();
        	} catch (Exception $ex) {
        		
        		$msg = $ex->getMessage();
        	}
        	
        	$ret = array("data" => $ret, "msg" => $msg);
        	output_data($ret);
        	exit;
        }

        $param['order_id'] = $order_pay_info['order_id'];
    ...
    private function _get_real_order_info($pay_sn,$rcb_pd_pay = array()) {
        ...
        if (!empty($result['data']['order_list'])) {
            foreach ($result['data']['order_list'] as $order_info) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) {
                    ...
                    $pay_amount = floatval(daPriceFormat($pay_amount));
        ...
        $result['data']['api_pay_amount'] = daPriceFormat($pay_amount);
    ...
    private function _get_wx_package($param) {
        ...
        $array['notify_url'] = BASE_API_URL . '/payment/wxpay/notify_url.php';
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_pay3() {
        $pay_sn = $_POST['pay_sn'];
    ...
    private function _get_wx_pay_info3($pay_param) {
        ...
        $param['notify_url'] = BASE_API_URL . '/payment/wxpay3/notify_url.php';
     ...
     * 取得支付宝移动支付 订单信息 实物订单
     */
    public function alipay_native_pay() {
        $pay_sn = $_POST['pay_sn'];
        ...
        $inc_file = BASE_ROOT_PATH.DS.'Api'.DS.'payment'.DS.$this->payment_code.DS.$this->payment_code.'.php';
        if(!is_file($inc_file)){
            exit('支付接口不存在');
        }
        require($inc_file);
     ...
     * 取得支付宝移动支付 订单信息 虚拟订单
     */
    public function alipay_native_vr_pay() {
        $pay_sn = $_POST['pay_sn'];
        if (!preg_match('/^\d+$/',$pay_sn)){
            output_error('支付单号错误');
        }
