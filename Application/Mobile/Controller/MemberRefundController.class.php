...
class MemberRefundController extends MobileMemberController {
    ...
    public function refund_all_form(){
        ...
        if(!empty($order_info) && is_array($order_info)) {
            ...
            $order['order_amount'] = daPriceFormat($order_info['order_amount']);
            $order['order_sn'] = $order_info['order_sn'];
            $order['store_name'] = $order_info['store_name'];
            $order['store_id'] = $order_info['store_id'];
            $order['allow_refund_amount'] = daPriceFormat($order_info['order_amount'] - $book_amount);//可退款金额
            $order['book_amount'] = daPriceFormat($book_amount);
    ...
    public function refund_all_post(){
        ...
        if (empty($order_info) || $payment_code == 'offline' || $refund['refund_id'] > 0) {//检查数据,防止页面刷新不及时造成数据错误
            ...
            $book_amount = Logic('order_book')->getDepositAmount($order_info);//订金金额
            $allow_refund_amount = daPriceFormat($order_info['order_amount'] - $book_amount);//可退款金额
            ...
            $refund_array['refund_amount'] = daPriceFormat($allow_refund_amount);
    ...
    public function refund_form(){
        ...
        if($refund_state == 1 && $goods_id > 0) {
            ...
            $goods['goods_price'] = daPriceFormat($goods_info['goods_price']);
            ...
            $goods['goods_pay_price'] = daPriceFormat($goods_pay_price);
    ...
    public function get_refund_list() {
        ...
        if(!empty($list) && is_array($list)) {
            ...
            foreach($list as $k => $v) {
                ...
                $val['refund_amount'] = daPriceFormat($v['refund_amount']);
    ...
    public function get_refund_info(){
        ...
        if(!empty($refund_info) && is_array($refund_info)) {
            ...
            $refund['refund_amount'] = daPriceFormat($refund_info['refund_amount']);
            ...
            if(!empty($detail_info) && $detail_info['refund_state'] == 2) {
                ...
                $detail_array['pay_amount'] = daPriceFormat($detail_info['pay_amount']);
                $detail_array['pd_amount'] = daPriceFormat($detail_info['pd_amount']);
                $detail_array['rcb_amount'] = daPriceFormat($detail_info['rcb_amount']);
            }
