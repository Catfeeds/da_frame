...
class ConnectController extends MobileHomeController {
        ...
        if(!empty($_GET['code'])) {
            ...
            if(!empty($user_info['unionid'])){
                ...
                if($token) {
                    ...
                    redirect(WAP_SITE_URL.'/tmpl/member/member.html?info=shopda&username='.$state_data['username'].'&key='.$state_data['key']);
                } else {
                    output_error('会员登录失败');
                }
            } else {
                //output_error('微信登录失败'); V1.0
    ...
    public function get_qq_info(){
        ...
        if(!empty($user_info['openid'])){
            ...
            if($token) {
                ...
                $state_data['userid'] = $member['member_id'];
                if($client == 'wap'){
                    $url = WAP_SITE_URL.'/tmpl/member/member.html?info=shopda&username='.$state_data['username'].'&key='.$state_data['key'];
                    @header("location: " . $url);
                    exit;
                }
     ...
     * 新浪微博获取应用唯一标识
     */
    public function get_sina_appid(){
