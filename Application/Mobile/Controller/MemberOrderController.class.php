...
class MemberOrderController extends MobileMemberController {
    ...
    public function order_list() {
        ...
        if ($_POST['is_merge_order_list'] == 1)
        {
        	$new_order_group_list = $this->_mergeOrderList($new_order_group_list);
        }
    ...
    //合并订单，供销微信小程序使用
    private function _mergeOrderList($new_order_group_list)
    {
    	$ret = array();
    	...
    	foreach ($new_order_group_list as $pay_order_item)
    	{
    		$temp_order = $pay_order_item;
    		$temp_order = array_merge($pay_order_item['order_list'][0], $temp_order);
    		unset($temp_order['order_list']);
    		
    		$pure_order_list = $pay_order_item['order_list'];
    		foreach ($pure_order_list as $order_item)
    		{
    			$goods_list = $order_item['extend_order_goods'];
    			foreach ($goods_list as $goods_item)
    			{
    				$temp_order['goods_list'][] = $goods_item;
    			}
    		}
    		unset($temp_order['extend_order_goods']);
    		$ret[] = $temp_order;
    	}
    	return $ret;
    }
    ...
    public function order_cancel() {
        ...
        if(!$result['state']) {
        	...
        	$order_info['pay_amount'] += $order_info['order_amount'] - $order_info['rcb_amount'] - $order_info['pd_amount'];
        	
            output_data('1', array("order_info" => $order_info));
        }
    }
    ...
    public function order_info() {
        ...
        $data['order_amount'] = daPriceFormat($result['data']['order_info']['order_amount']);
        $data['shipping_fee'] = daPriceFormat($result['data']['order_info']['shipping_fee']);
        $data['real_pay_amount'] = daPriceFormat($result['data']['order_info']['order_amount']);
...
//         $data['refund_state'] = $result['data']['order_info']['refund_state'];
        ...
        foreach ($result['data']['order_info']['goods_list'] as $_k => $_v) {
            ...
            $data['goods_list'][$_k]['goods_price'] = daPriceFormat($_v['goods_price']);
