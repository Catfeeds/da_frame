...
class MemberSecurityController extends BaseMemberController {
    ...
    public function index() {
        ...
        $this->render('member_security.index');
    }
    ...
    public function send_bind_email() {
        ...
        $subject	= daReplaceText($tpl_info['title'],$param);
        $message	= daReplaceText($tpl_info['content'],$param);

        $email	= new Email();
	    $result	= $email->send_sys_email($_POST["email"],$subject,$message);
	    showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！如果您始终未收到邮件，请于60秒后重新发送', $GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index','succ','',10);
    ...
    public function auth() {
        ...
        if (chksubmit(false,true)) {
            if (!in_array($_POST['type'],array('modify_pwd','modify_mobile','modify_email','modify_paypwd','pd_cash'))) {
                redirect("{$GLOBALS['_PAGE_URL']}&c=MemberSecurity&a=index");
            }
            $member_common_info = $model_member->getMemberCommonInfo(array('member_id'=>$_SESSION['member_id']));
            ...
            if ($_POST['type'] == 'pd_cash') {
                $this->render('member_pd_cash.add');
            } else {
                $this->render('member_security.'.$_POST['type']);
            }
            ...
            ($_GET['type'] == 'modify_mobile' && $member_info['member_mobile_bind'] == '0')) {
                ...
                $this->render('member_security.'.$_GET['type']);
                exit;
            }
            ...
            $member_info['member_mobile_bind'] == '0') {
                showMessage('请先绑定邮箱或手机',"{$GLOBALS['_PAGE_URL']}&c=MemberSecurity&a=index",'html','error');
            }

            $this->assign('member_info',$member_info);

            $this->render('member_security.auth');
        }
    ...
    public function modify_pwd() {
        ...
        if (TIMESTAMP - $_SESSION['auth_modify_pwd'] > 1800) {
            showDialog('操作超时，请重新获得验证码',"{$GLOBALS['_PAGE_URL']}&c=MemberSecurity&a=auth&type=modify_pwd",'html','error');
        }
        ...
        showDialog($message,$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index',$update ? 'succ' : 'error');
    ...
    public function modify_paypwd() {
        ...
        if (TIMESTAMP - $_SESSION['auth_modify_paypwd'] > 1800) {
            showMessage('操作超时，请重新获得验证码',$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=auth&type=modify_paypwd','html','error');
        }
    ...
    public function modify_mobile() {
        ...
        if (chksubmit()) {
            ...
            showDialog('手机号绑定成功', $GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index','succ');
        }
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>$GLOBALS['_PAGE_URL'] .'&c=MemberSecurity&a=index'),
                );
                break;
            case 'modify_pwd':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>$GLOBALS['_PAGE_URL'] .'&c=MemberSecurity&a=index'),
                array('menu_key'=>'modify_pwd','menu_name'=>'修改登录密码','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=auth&type=modify_pwd'),
                );
                break;
            case 'modify_email':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index'),
                array('menu_key'=>'modify_email', 'menu_name'=>'邮箱验证','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=auth&type=modify_email'),
                );
                break;
            case 'modify_mobile':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index'),
                array('menu_key'=>'modify_mobile','menu_name'=>'手机验证','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=auth&type=modify_mobile'),
                );
                break;
            case 'modify_paypwd':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index'),
                array('menu_key'=>'modify_paypwd','menu_name'=>'设置支付密码','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=auth&type=modify_paypwd'),
                );
                break;
            case 'pd_cash':
                $menu_array = array(
                array('menu_key'=>'loglist','menu_name'=>'账户余额',    'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_log_list'),
                array('menu_key'=>'recharge_list','menu_name'=>'充值明细', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=index'),
                array('menu_key'=>'cashlist','menu_name'=>'余额提现', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list'),
                array('menu_key'=>'pd_cash','menu_name'=>'提现申请','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=auth&type=pd_cash'),
                );
                break;
        }
