...
class MemberBindController extends BaseMemberController {
    ...
    public function weixinbind(){
        ...
        $this->render('member_bind.weixin');
    }
    /**
     * 微信解绑
     */
    ...
    private function profile_menu($menu_key='',$array=array()) {
        ...
        $menu_array     = array();
        $menu_array = array(
            1=>array('menu_key'=>'qq_bind', 'menu_name'=>'QQ绑定',   'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberBind&a=qqbind'),
            2=>array('menu_key'=>'sina_bind',   'menu_name'=>'新浪绑定', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberBind&a=sinabind'),
            3=>array('menu_key'=>'weixin_bind',   'menu_name'=>'微信绑定', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberBind&a=weixinbind'),
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
