...
class MemberSharemanageController extends BaseMemberController {
    public function __construct(){
        parent::__construct();
        Language::read('member_sharemanage');
        //判断系统是否开启站外分享功能
        if (C('share_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),$GLOBALS['_PAGE_URL'] . '&c=MemberInformation&a=member','html','error');
        }
    }
    /**
     * 分享绑定列表
    ...
    public function index(){
        ...
        $this->render('member_sharemanage.bind');
    }
    /**
     * 解除绑定
     */
    public function unbind(){
        $type = $_GET['type'];
        if (empty($type)){
            showDialog(Language::get('wrong_argument'), $GLOBALS['_PAGE_URL'] . '&c=MemberSharemanage','error');
        }
        $model = Model('sns_binding');
    ...
    public function share_qqzone(){
        ...
        if (C('share_qqzone_isuse') != 1){
            showMessage(Language::get('sharebind_unused'), $GLOBALS['_PAGE_URL'] . '&c=MemberSharemanage','html','error');
        }
        ...
        //获取qq账号信息
        require_once (BASE_ROOT_PATH.DS.'Api'.DS.'snsapi'.DS.'qqzone'.DS.'user'.DS.'get_user_info.php');
    ...
    public function share_sinaweibo(){
        ...
        //获取新浪微博账号信息
        require_once (BASE_ROOT_PATH.DS.'Api'.DS.'snsapi'.DS.'sinaweibo'.DS.'saetv2.ex.class.php');
