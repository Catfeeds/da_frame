...
class PredepositController extends BaseMemberController {
    ...
    public function recharge_add(){
        ...
        if ($insert) {
            //转向到商城支付页面
            redirect(SHOP_SITE_URL . '&c=Buy&a=pd_pay&pay_sn='.$pay_sn);
        }
    }
    ...
    public function rechargecard_add()
    {
        if (!chksubmit()) {
            ...
            $this->render('predeposit.rechargecard_add');
            return;
        }
    ...
    public function index(){
        ...
        $this->assign('show_page',$model_pd->showpage());

        $this->render('predeposit.pd_list');
    }
    ...
    public function recharge_show(){
        ...
        $this->render('predeposit.pd_info');
    }
    ...
    public function recharge_del(){
        ...
        if ($result){
            showDialog(Language::get('spd_common_del_succ'),'reload','succ','CUR_DIALOG.close()');
        }else {
    ...
    public function pd_log_list(){
        ...
        $this->assign('show_page',$model_pd->showpage());
        $this->assign('list',$list);
        $this->render('predeposit.pd_log_list');
    }
    ...
    public function rcb_log_list()
    {
        ...
        $this->assign('show_page', $model->showpage());
        $this->assign('list', $list);
        $this->render('predeposit.rcb_log_list');
    }
    ...
    public function pd_cash_add(){
        if (chksubmit()){
            ...
            if (floatval($member_info['available_predeposit']) < $pdc_amount){
                showDialog(Language::get('predeposit_cash_shortprice_error'),
				$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list','error');
            }
            try {
                ...
                $model_pd->commit();
                showDialog(Language::get('predeposit_cash_add_success'),$GLOBALS['_PAGE_URL'] .'&c=Predeposit&a=pd_cash_list','succ');
            } catch (Exception $e) {
                $model_pd->rollback();
                showDialog($e->getMessage(),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list','error');
            }
        }
    }
    ...
    public function pd_cash_list(){
        ...
        $this->assign('show_page',$model_pd->showpage());
        $this->render('predeposit.pd_cash_list');
    }
    ...
    public function pd_cash_info(){
        ...
        if ($pdc_id <= 0){
            showMessage(Language::get('predeposit_parameter_error'),$GLOBALS['_PAGE_URL'] .'&c=Predeposit&a=pd_cash_list','html','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_member_id'] = $_SESSION['member_id'];
        ...
        if (empty($info)){
            showMessage(Language::get('predeposit_record_error'),$GLOBALS['_PAGE_URL'] .'&c=Predeposit&a=pd_cash_list','html','error');
        }

        self::profile_menu('cashinfo','cashinfo');
        $this->assign('info',$info);
        $this->render('predeposit.pd_cash_info');
    }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array(
            ...
            array('menu_key'=>'rcb_log_list',   'menu_name'=>'充值卡余额',   'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=rcb_log_list',),
        );
        switch ($menu_type) {
            case 'rechargeinfo':
            ...
            case 'rechargecard_add':
                ...
                break;
            case 'cashadd':
                $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>'提现申请',    'menu_url'=> $GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_add');
                break;
            case 'cashinfo':
