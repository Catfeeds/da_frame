...
class MemberVoucherController extends BaseMemberController
{
    ...
    public function voucher_list()
    {
        ...
        $this->render('member_voucher.list');
    }

    /**
     * 通过卡密绑定代金券
     */
    public function voucher_binding()
    {
        if (chksubmit(false, true)) {
            ...
            if ($result) {
                ...
                showDialog('代金券领取成功',  $GLOBALS['_PAGE_URL'] . '&c=MemberVoucher&a=voucher_list', 'succ');
            } else {
                showDialog('代金券领取失败','','error','submiting = false');
            }
        }
        $this->profile_menu('voucher_binding');
        $this->render('member_voucher.binding');
    }
    ...
    private function profile_menu($menu_key = '')
    {
        $menu_array = array(
            1 => array(
                ...
                'menu_name' => Language::get('spd_myvoucher'),
                'menu_url' => $GLOBALS['_PAGE_URL'] . '&c=MemberVoucher&a=voucher_list'
            ),
            2 => array(
                'menu_key' => 'voucher_binding',
                'menu_name' => '领取代金券',
                'menu_url' => $GLOBALS['_PAGE_URL'] . '&c=MemberVoucher&a=voucher_binding'
            )
        );
