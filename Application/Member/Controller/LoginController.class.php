...
class LoginController extends BaseLoginController {
    ...
    public function index(){
        ...
        if ($_GET['inajax'] == 1 && C('captcha_status_login') == '1'){
        	$randomInt = rand(1000,9999) . time();
            $script = "$(\"img[name='codeimage']\").attr('src','" . SHOP_SITE_URL . 
            "&c=Seccode&a=makecode&shopdamap=&t={$randomInt}');";
        }
        ...
        if ($result !== false){
            ...
            if ($_pic[0] != ''){
                $this->assign('lpic',UPLOAD_SITE_URL_HTTPS.'/'.ATTACH_LOGIN.'/'.$_pic[array_rand($_pic)]);
            }else{
                $this->assign('lpic',UPLOAD_SITE_URL_HTTPS.'/'.ATTACH_LOGIN.'/'.rand(1,4).'.jpg');
            }

            if(empty($_GET['ref_url'])) {
                $ref_url = getReferer();
                if (!preg_match('/c=Login&a=logout/', $ref_url)) {
                 $_GET['ref_url'] = $ref_url;
                }
            }
            $this->assign('html_title',C('site_name').' - '.$lang['login_index_login']);
            if ($_GET['inajax'] == 1){
                $this->render('login_inajax','null_layout');
            }else{
                $this->render('login');
            }
        }
    ...
    public function logout(){
        ...
        $lang   = Language::getLangContent();
        // 清理COOKIE
        setDaCookie('msgnewnum'.$_SESSION['member_id'],'',-3600);
        setDaCookie('auto_login', '', -3600);
        setDaCookie('cart_goods_num','',-3600);
        session_unset();
        session_destroy();
        if(empty($_GET['ref_url'])){
            $ref_url = getReferer();
        ...
        redirect(LOGIN_SITE_URL . '&c=Login&ref_url='.urlencode($ref_url));
    }
    ...
    public function register() {
        ...
        $model_member   = Model('member');
        $model_member->checkloginMember();
        $this->assign('html_title',C('site_name').' - '.$lang['login_register_join_us']);
    ...
    public function usersave() {
        ...
        if (Process::islock('reg')){
            showDialog(Language::get('spd_common_op_repeat'));
        }
        Language::read("home_login_register");
        $lang   = Language::getLangContent();
        $model_member   = Model('member');
    ...
    public function forget_password(){
        ...
        $this->assign('html_title',C('site_name').' - '.Language::get('login_index_find_password'));
        $this->render('find_password');
    }
    ...
    public function find_password(){
        ...
        if (Process::islock('forget')) {
            showDialog($lang['spd_common_op_repeat'],'','error');
        }
    ...
    public function bind_email() {
       ...
       showMessage('邮箱设置成功',$GLOBALS['_PAGE_URL'] . '&c=MemberSecurity&a=index');
