...
class MemberPointsController extends BaseMemberController {
    ...
    public function points_log(){
        ...
        $this->assign('show_page',$points_model->showpage());
        $this->assign('list_log',$list_log);
        $this->render('member_points');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_key='',$array=array()) {
        $menu_array = array(
            ...
            2=>array('menu_key'=>'orderlist','menu_name'=>'积分兑换',    'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberPointorder&a=orderlist')
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
