...
class MemberRedpacketController extends BaseMemberController{
    ...
    public function rp_list(){
        ...
        $this->assign('redpacketstate_arr', $model_redpacket->getRedpacketState());
        $this->assign('show_page',$model_redpacket->showpage(2)) ;
        $this->profile_menu('rp_list');
        $this->render('member_redpacket.list');
    }

    /**
     * 通过卡密绑定红包
     */
    public function rp_binding(){
        if(chksubmit(false,true)){
            ...
            if($result){
                ...
                showDialog('红包领取成功', $GLOBALS['_PAGE_URL'] . '&c=MemberRedpacket&a=rp_list','succ');
            } else {
                showDialog('红包领取失败','','error','submiting = false');
            }
        }
        $this->profile_menu('rp_binding');
        $this->render('member_redpacket.binding');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'rp_list','menu_name'=>'我的红包','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberRedpacket&a=rp_list'),
            2=>array('menu_key'=>'rp_binding','menu_name'=>'领取红包','menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberRedpacket&a=rp_binding'),
        );
