...
class MemberMessageController extends BaseMemberController {
    ...
    public function message() {
        ...
        $this->render('member_message.box');
    }
    /**
     * 收到(私信)站内信列表
     *
    ...
    public function privatemsg(){
        ...
        $this->render('member_message.sendlist');
    }
    /**
     * 系统站内信列表
     *
    ...
    public function ggmsg(){
        ...
        $this->render('member_message.sys.box');
    }
    /**
     * 发送站内信页面
     *
    ...
    public function sendmsg(){
        ...
        $this->render('member_message.send');
    }
    /**
     * 站内信保存操作
     *
    ...
    public function savemsg() {
        ...
        if (chksubmit()) {
            ...
            if($_GET['type'] == 'sns_board'){
                ...
                showDialog(Language::get('home_message_send_success'),$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=privatemsg','succ');
            }
        }
    }
    /**
    ...
    public function showmsgcommon() {
        ...
        $this->render('member_message.view');
    }
    /**
     * 系统站内信查看操作
     *
    ...
    public function savereply() {
        ...
        if (!$isallowsend){
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_noallowsend'),$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=message','html','error');
            }
        }
        if ($_POST['form_submit'] == 'ok') {
            $message_id = intval($_POST["message_id"]);
            if ($message_id <=0){
                showMessage(Language::get('wrong_argument'),$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=message','html','error');
            }
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_send_success'),$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=privatemsg');
            }
        }else {
            if($_GET['inajax'] == 1){
    ...
    public function setting() {
        ...
        if (chksubmit()) {
            ...
            if ($result) {
                showDialog(L('spd_common_save_succ'), '', 'succ');
            } else {
        ...
        $this->render('member_message.setting');
    }
    /**
     * 统计未读消息
     */
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'message', 'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=message'),
            array('menu_key'=>'private', 'menu_name'=>Language::get('home_message_private_message'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=privatemsg'),
            array('menu_key'=>'system',  'menu_name'=>Language::get('home_message_system_message'),  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=systemmsg'),
            array('menu_key'=>'ggmsg',  'menu_name'=>'系统公告',  'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=ggmsg'),
            array('menu_key'=>'close',   'menu_name'=>Language::get('home_message_close'),   'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=personalmsg'),
            array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlMember('member_message', 'setting'))
        );
        if($menu_key == 'sendmsg') {
            $menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=MemberMessage&a=sendmsg');
