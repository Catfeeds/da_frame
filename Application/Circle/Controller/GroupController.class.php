...
class GroupController extends BaseCircleThemeController{
    ...
    public function index(){
        ...
        $this->assign('todaythcount', $todaythcount);

        //展示形式，默认以图文展示 list/preview
        if($_GET['type'] != ''){
            $display_mode = ($_GET['type'] == 'list')?'list':'preview';
            setDaCookie('circleDisplayMode', $display_mode, 30*24*60*60);
        }else{
        ...
        $this->assign('m_readperm', $this->m_readperm);
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if(intval($this->circle_info['circle_joinaudit']) == 0){
                    ...
                    $model->table('circle')->where(array('circle_id'=>$this->c_id))->update($update);
                    showDialog(L('spd_apply_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
                }
            }
        }
        $this->circleSEO(L('circle_apply_join').$this->circle_info['circle_name']);
        $this->render('group_apply', 'null_layout');
    }
    /**
     * 退出圈子
     */
    ...
    public function group_member(){
        ...
        $this->render('group.member');
    }
    /**
     * 圈子成员编辑
     */
    public function group_memberedit(){
        $model = Model();
        $where = array();
        $where['circle_id'] = $this->c_id;
        ...
        if(chksubmit()){
            ...
            showDialog(L('spd_deit_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
        }
        $member_info = $model->table('circle_member')->where($where)->find();
        $this->assign('member_info', $member_info);
        $this->render('group.member_edit', 'null_layout');
    }
    /**
     * 圈子商品列表
     */
    ...
    public function manage_apply(){
        ...
        if(chksubmit()){
            ...
            showDialog(L('spd_common_op_succ'), 'reload', 'succ', 'DialogManager.close(\'manage_apply\')');
        }

        $this->render('group.mapply', 'null_layout');
    }
    /**
     * Level introduction
     */
    public function level_intr(){
        // 圈子信息
        $this->circleInfo();
        ...
        $this->assign('mld_array', $mld_array);
        $this->render('group.level');
    }
}
