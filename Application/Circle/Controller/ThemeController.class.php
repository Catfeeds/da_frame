...
use Circle\Controller\BaseCircleThemeController;
...
class ThemeController extends BaseCircleThemeController {
     ...
     */
    public function ajax_themeinfo(){
        // 话题信息
        $this->themeInfo();
     ...
     */
    public function ajax_quickreply(){
        // 话题信息
        $this->themeInfo();

        $data = array();
        $data['form_action'] = CIRCLE_SITE_URL. '&c=Theme&a=save_reply&type=quick&c_id='.$this->c_id.'&t_id='.$this->t_id;
        ...
        if(intval(C('circle_contentleast')) > 0){
            $data['c_contentmsg']   = sprintf(L('spd_content_min_length'), intval(C('circle_contentleast')));
        }else{
            $data['c_contentmsg']   = L('spd_content_not_null');
        }
    ...
    public function save_theme(){
        if(chksubmit()){
            ...
            $validate_arr[] = array("input"=>$_POST["name"], "require"=>"true","message"=>Language::get('spd_name_not_null'));
            $validate_arr[] = array("input"=>$_POST["name"], "validator"=>'Length',"min"=>4,"max"=>30,"message"=>Language::get('spd_name_min_max_length'));
            $validate_arr[] = array("input"=>$_POST["themecontent"], "require"=>"true","message"=>Language::get('spd_content_not_null'));
            ...
            if($themeid){
                ...
                if(intval(C('circle_intervaltime')) > 0){
                    setDaCookie('circle_intervaltime', true, intval(C('circle_intervaltime')));
                }
                ...
                showDialog(L('spd_release_op_succ'), CIRCLE_SITE_URL. '&c=Theme&a=theme_detail&c_id='.$this->c_id.'&t_id='.$themeid, 'succ');
            }else{
                showDialog(L('spd_release_op_fail'));
            }
        }
        @header("location: ".CIRCLE_SITE_URL);
    }
    ...
    public function choose_goods(){
        ...
        $this->assign('favorites_goods', $favorites_goods);

        $this->render('theme.choose_goods', 'null_layout');
    }
    ...
    public function choose_image(){
        ...
        $this->render('theme.choose_image', 'null_layout');
    }
    /**
     * 附件批量上传 话题
     */
    ...
    public function theme_detail(){
        ...
        $this->assign('goods_list', $goods_array[0]); unset($goods_array[0]);
        $this->assign('reply_goods', $goods_array);
        ...
        if(!empty($affix_array)){
            ...
            $this->assign('reply_affix', $reply_affix);
        }
        ...
        $this->assign('theme_onlike', $theme_nolike);
        ...
        if($this->theme_info['theme_special'] == 1){
            $poll_info = $model->table('circle_thpoll')->where(array('theme_id'=>$this->t_id))->find();
            $this->assign('poll_info', $poll_info);
            ...
            if($poll_info['poll_endtime'] == 0 || $poll_info['poll_endtime'] > time()){
                $this->assign('vote_end', false);
                // Whether members have had voted
                $voter_info = $model->table('circle_thpollvoter')->where(array('theme_id'=>$this->t_id, 'member_id'=>$_SESSION['member_id']))->find();
                if(!empty($voter_info)){
                    $this->assign('partake', true);
                }else{
                    $this->assign('partake', false);
                }
            }else {
                $this->assign('vote_end', true);
            }
        }

        // breadcrumb navigation
        $this->breadcrumd();
        $this->render('theme.detail');
    }
    /**
     * 发布话题
     */
    public function new_theme(){
        ...
        $this->assign('thclass_list', $thclass_list);

        // Read Permission
        $readperm = $this->readPermissions($this->cm_info);
        $this->assign('readperm', $readperm);
        ...
        if($_GET['sp'] == 1){
            ...
            $this->render('theme.new_poll');
        }else{
            // breadcrumb navigation
            $this->breadcrumd(L('circle_new_theme'));
            $this->render('theme.new_theme');
        }
    }
    /**
     * 编辑话题
    ...
    public function edit_theme(){
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('spd_deit_op_succ'), CIRCLE_SITE_URL. '&c=Theme&a=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id, 'succ');
            }else{
                showDialog(L('spd_deit_op_fail'));
            }
        }
        ...
        $this->circleSEO(L('spd_edit_theme'));

        // breadcrumb navigation
        $this->breadcrumd(L('spd_edit_theme'));
        ...
        if($this->theme_info['theme_special'] == 1){
            ...
            $this->render('theme.edit_themepoll');
        }else{
            $this->render('theme.edit_theme');
        }
    }
    /**
     * 赞
     */
    public function ajax_likeyes(){
        // 话题信息
        $this->themeInfo();
    ...
    public function reply(){
        ...
        if($_GET['answer_id'] != ''){
            ...
            if(!empty($reply_info)) $this->assign('answer', $reply_info);
        }
        ...
        $this->circleSEO(L('spd_reply_theme'));

        // breadcrumb navigation
        $this->breadcrumd(L('spd_advanced_reply'));
        $this->render('theme.reply');
    }
    /**
     * 编辑回复
     */
    public function edit_reply(){
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($rs){
                    ...
                    showDialog(L('spd_common_op_succ'), $GLOBALS['_PAGE_URL'] . '&c=Theme&a=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id, 'succ');
                }
            }
        }
        ...
        // breadcrumb navigation
        $this->breadcrumd(L('spd_edit_reply'));

        $this->render('theme.edit_reply');
    }
    /**
     * 话题回复保存
     */
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($reply_id){
                    ...
                    if($_GET['type'] == 'quick'){
                        showDialog(L('spd_common_op_succ'), '', 'succ', '$(\'li[datype="li'.$this->t_id.'"]\').find(\'.quick-reply-2\').removeClass(\'t\').html(\'\').end().find(\'.quick-reply-list-2\').remove().end().end().find(\'a[datype="reply"]\').click().click();');
                    }else{
                        showDialog(L('spd_common_op_succ'), $GLOBALS['_PAGE_URL'] . '&c=Theme&a=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id, 'succ');
                    }
                }
