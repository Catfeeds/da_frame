...
class CmsPictureController extends SystemController {
    //文章状态草稿箱
    ...
    public function index() {
        $this->cms_picture_list();
    }

    /**
     * cms文章列表
     **/
    public function cms_picture_list()
    {
     ...
     * 待审核文章列表
     */
    public function cms_picture_list_verify()
    {
     ...
     * 已发布文章列表
     */
    public function cms_picture_list_published()
    {
    ...
    protected function doAction($state, $menuKey)
    {
        ...
        $this->setDirquna('cms');
$this->render("cms_picture.list");
    }

    public function cms_picture_list_xml()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            ...
            $o .= '<li><a target="_blank" href="' .
                    CMS_SITE_URL .
                    '&c=Picture&a=picture_detail&picture_id=' .
                    $val['picture_id'] .
                    '">查看画报</a></li>';
...
EOB;
     ...
     * cms文章审核
     */
    public function cms_picture_verify() {
     ...
     * cms文章回收
     */
    public function cms_picture_callback() {
     ...
     * cms文章分类排序修改
     */
    public function update_picture_sort() {
        if(intval($_GET['id']) <= 0) {
     ...
     * cms文章分类排序修改
     */
    public function update_picture_click() {
     ...
     * cms文章删除
     **/
     public function cms_picture_drop() {
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('spd_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=CmsPicture&a=cms_picture_list'),
            'list_verify'=>array('menu_type'=>'link','menu_name'=>Language::get('cms_article_list_verify'),'menu_url'=> $GLOBALS['_PAGE_URL'] . '&c=CmsPicture&a=cms_picture_list_verify'),
            'list_published'=>array('menu_type'=>'link','menu_name'=>Language::get('cms_article_list_published'),'menu_url'=> $GLOBALS['_PAGE_URL'] .  '&c=CmsPicture&a=cms_picture_list_published'),
        );
