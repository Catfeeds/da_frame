...
class CmsArticleController extends SystemController {
    //文章状态草稿箱
     ...
     * 待审核文章列表
     */
    public function cms_article_list_verify()
    {
     ...
     * 已发布文章列表
     */
    public function cms_article_list_published()
    {
    ...
    protected function doAction($state, $menuKey)
    {
        ...
        $this->assign('currentState', $state);

        $states = $this->get_article_state_list();
        $this->assign('states', $states);

        $this->setDirquna('cms');
		$this->render("cms_article.list");
    }

    public function cms_article_list_xml()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            ...
            $o .= '<li><a target="_blank" href="' .
                    CMS_SITE_URL .
                    '&c=Article&a=article_detail&article_id=' .
                    $val['article_id'] .
                    '">查看文章</a></li>';
            ...
            $i['is_show_on_mobile'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="is_show_on_mobile">' .
            		$val['is_show_on_mobile'] . '</span>';
...
EOB;
     ...
     * cms文章审核
     */
    public function cms_article_verify() {
     ...
     * cms文章收回
     */
    public function cms_article_callback() {
     ...
     * cms文章分类排序修改
     */
    public function update_article_click() {
     ...
     * cms 文章是否修改
     */
    public function update_is_show_on_mobile() {
    	if(intval($_GET['id']) <= 0 || intval($_GET['value']) < 0) {
    		echo json_encode(array('result'=>FALSE,'message'=>Language::get('param_error')));die;
    	}
    	$model_class = Model("cms_article");
    	$result = $model_class->modify(array('is_show_on_mobile'=>$_GET['value']),array('article_id'=>$_GET['id']));
    	if($result) {
    		echo json_encode(array('result'=>TRUE,'message'=>''));die;
    	} else {
    		echo json_encode(array('result'=>FALSE,'message'=>Language::get('param_error')));die;
     ...
     * cms文章删除
     **/
     public function cms_article_drop() {
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('spd_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=CmsArticle&a=cms_article_list'),
            'list_verify'=>array('menu_type'=>'link','menu_name'=>Language::get('cms_article_list_verify'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=CmsArticle&a=cms_article_list_verify'),
            'list_published'=>array('menu_type'=>'link','menu_name'=>Language::get('cms_article_list_published'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=CmsArticle&a=cms_article_list_published'),
        );
