...
class CmsIndexController extends SystemController {
    ...
    public function cms_index() {
        ...
        $this->setDirquna('cms');
		$this->render('cms_index');
    }

    /**
     * 获取
     */
    public function get_module_frame_list() {
        $data = array();
     ...
     * 模块删除
     */
    public function drop_page_module() {
        $data = array();
        $data['result'] = TRUE;
     ...
     * 启用页面模块
     */
    public function update_page_module_show() {
     ...
     * 关闭页面模块
     */
    public function update_page_module_hide() {
     ...
     * 更新模块标题
     */
    public function update_page_module_title() {
     ...
     * 更新模块主题
     */
    public function update_page_module_style() {
     ...
     * 更新模块显示样式
     */
    public function update_page_module_view() {
     ...
     * 更新页面模块排序
     */
    public function update_page_module_index() {
        $data = array();
        $data['result'] = TRUE;
    ...
    public function cms_index_preview() {
        $model_index_module = Model('cms_index_module');
        ...
        $this->assign('module_list', $module_list);
        $this->setDirquna('cms');
		$this->render('cms_index.template', 'null_layout');
    }

    /**
     * 首页静态文件生成
     */
    public function cms_index_build() {
        $data = array();
        $data['result'] = TRUE;
        ...
        $template_file = $html_path.'index.html';
    ...
    public function save_module() {
        ...
        $this->assign('frame_structure', json_decode($module_frame['frame_structure'], TRUE));
        $this->assign('frame_block', $_POST['frame_block']);
        if($_POST['module_display_title_state'] !== 'disable') {
            $this->assign('module_display_title', TRUE);
        } else {
            $this->assign('module_display_title', FALSE);
        }
        $html_path =  $this->get_realpath_of_module_template();
        ...
        $template_name = $this->get_realpath_of_module_template($new_module);
 
        ob_start();
        $this->setDirquna('cms');
		$this->render('cms_module.template', 'null_layout');
        $result = file_put_contents($template_name, ob_get_clean());
    ...
    public function drop_module() {
        ...
        //删除模板文件
        //$module_template = BASE_UPLOAD_PATH.DS.ATTACH_CMS.DS.'module_template'.DS.'cms_module.'.$module_detail['module_name'].'.html';
        $module_template = $this->get_realpath_of_module_template($module_detail);
    ...
    public function module_edit() {
        ...
        $this->assign('module_detail', $module_detail);
        ...
        //获取表单组件
        $module_templage =  $this->get_module_template_path($module_detail);
 
        $this->assign('module_template',$module_templage);

        //编辑标志，用于显示模板中的编辑功能
        $this->assign('edit_flag', TRUE);

        $this->show_menu('module_edit');
        $this->setDirquna('cms');
		$this->render('cms_index_module.edit');
    }

    /**
     * cms首页模块保存
     */
    public function save_page_module() {
        ...
        if($result) {
            ...
            showMessage(Language::get('spd_common_save_succ'), $GLOBALS['_PAGE_URL'] . '&c=CmsIndex&a=cms_index');
        } else {
            $this->log(Language::get('cms_log_index_edit').$_POST['module_id'], 0);
            showMessage(Language::get('spd_common_save_fail'), '', '','error');
        }
    }
     ...
     * 获取模块模板路径
     */
    private function get_module_template_path($module_detail = array()) {

        if($module_detail['module_type'] === $module_detail['module_name']) 
        {
            $ret = 'cms_index_widget.'.$module_detail['module_type'];
        } 
        else 
        {
            $ret = 'module_template'. DS . 'cms_module' . DS . $module_detail['module_name'];
        }
 
        return $ret;
    }
    
    private function get_realpath_of_module_template($module_detail = array())
    {
    	if (empty($module_detail))
    	{
    		$ret = ADMIN_BASE_PATH.DS.ADMIN_MODULES_CMS.DS. 'View' . DS . TPL_ADMIN_NAME . DS .'module_template'.DS.'cms_module' . DS;
    		return $ret;
    	}
    	if($module_detail['module_type'] === $module_detail['module_name'])
    	{
    		$ret = ADMIN_BASE_PATH.DS.ADMIN_MODULES_CMS.DS. 'View' . DS . TPL_ADMIN_NAME . DS . 'cms_index_widget.'.$module_detail['module_type'] . ".html";
    	}
    	else 
    	{
    		$ret = ADMIN_BASE_PATH.DS.ADMIN_MODULES_CMS.DS. 'View' . DS . TPL_ADMIN_NAME . DS .'module_template'. DS . 'cms_module' . DS . $module_detail['module_name'] . ".html";
    	}
    	return $ret;
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'cms_index'=>array('menu_type'=>'link','menu_name'=>Language::get('spd_manage'),'menu_url'=> $GLOBALS['_PAGE_URL'] . '&c=CmsIndex&a=cms_index'),
        );
        if($menu_key == 'module_edit') {
            $menu_array['module_edit'] = array('menu_type'=>'link','menu_name'=>Language::get('spd_edit'),'menu_url'=>'###');
        }
    ...
    public function get_article_list() {
        ...
        $this->setDirquna('cms');
		$this->render('cms_widget_article_list','null_layout');
    }
    ...
    public function get_store_list() {
        ...
        $this->assign('show_page',$model_store->showpage());
        $this->assign('store_list', $store_list);
        $this->setDirquna('cms');
$this->render('cms_widget_store_list', 'null_layout');
    }
    ...
    public function get_member_list() {
        ...
        $this->assign('show_page',$model_member->showpage());
        $this->assign('member_list', $member_list);
        $this->setDirquna('cms');
$this->render('cms_widget_member_list', 'null_layout');
    }
    ...
    public function get_brand_list() {
        ...
        $this->setDirquna('cms');
$this->render('cms_widget_brand_list','null_layout');
    }

    /**
     * 商品分类列表
     */
    public function get_goods_class_list_json() {
     ...
     * 商品分类详细列表
     */
    public function get_goods_class_detail(){
        $model_class = Model('goods_class');
        ...
        $this->assign('gc_parent',$gc_parent);
        $this->assign('goods_class',$goods_class);
        $this->setDirquna('cms');
$this->render('cms_widget_goods_class_list','null_layout');
    }
