...
class connect_apiLogic {
	...
	const TP_LOGIN_CHANNEL_SINA = 'sina';
	const TP_LOGIN_CHANNEL_QQ = 'qq';
	const TP_LOGIN_CHANNEL_WEIXIN = 'weixin';
    ...
    public function getWxOAuth2Url(){
		...
		$connect_url = url('connect', 'index', array());
        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$weixin_appid.
            '&redirect_uri='.urlencode(MOBILE_SITE_URL.'&'.$connect_url).
        ...
        if($state == true) {
            ...
            if($state == true){
                ...
                if($result){
                    ...
                    $state = false;
                    $msg = '手机短信发送失败' . $GLOBALS['sms_send_res'];
                }
            }
        }
        $state_data = array(
    ...
    public function smsRegister($phone, $captcha, $password, $client){
        ...
        if($state == true){
            ...
            $num = substr($phone,-4);
            $member_name = $this->getMemberName('shopda', $num);
			//SHOPDA 分销 会员邀请
     ...
     * @return array
     */
    public function getUserToken($member, $client, $tp_union_id = '', $channel_name = ''){
        ...
        $mb_user_token_info['tp_unionid'] = $this->getTpUnionId($tp_union_id, $channel_name);
    ...
    public function wxRegister($user_info, $client){
        ...
        //检查是否已经注册
        $tp_union_id = $unionid;
        $channel_name = self::TP_LOGIN_CHANNEL_WEIXIN;
        $member_info = $this->getMemberByTpInfo($tp_union_id, $channel_name);
        if (!empty($member_info)) {
        	$ret = array("token" => $member_info['token'], "member" => $member_info);
        	return $ret;
        }
        ...
        if($result) {
            ...
            $member_id = $result;
            $token = $this->getUserToken($member, $client, $tp_union_id, $channel_name);
            if($token) {
...
//             var_dump($result);
    ...
    public function getSinaOAuth2Url($sina_type = ''){
        ...
        require_once (BASE_ROOT_PATH .DS.'Api'.DS.'sina'.DS.'saetv2.ex.class.php');
    ...
    public function sinaRegister($user_info, $client){
        ...
        //检查是否已经注册
        $tp_union_id = $openid;
        $channel_name = self::TP_LOGIN_CHANNEL_SINA;
        $member_info = $this->getMemberByTpInfo($tp_union_id, $channel_name);
        if (!empty($member_info)) {
        	$ret = array("token" => $member_info['token'], "member" => $member_info);
        	return $ret;
        ...
        if($result) {
            ...
            $member_id = $member['member_id'];
            $token = $this->getUserToken($member, $client, $tp_union_id, $channel_name);//手机接口登录
            if($token) {
                $state_data['token'] = $token;
                $state_data['member']['member_name'] = $member_name;
                $state_data['member']['member_id'] = $member_id;
    ...
    public function getQqOAuth2Url($qq_type = ''){
        ...
        require_once (BASE_ROOT_PATH .DS.'Api'.DS.'qq'.DS.'oauth.qq.class.php');
    ...
    public function getQqUserInfo($code, $client = '', $token = ''){
        ...
        //var_dump($client_id, $client_secret, $client);
        require_once (BASE_ROOT_PATH .DS.'Api'.DS.'qq'.DS.'oauth.qq.class.php');
        $oauth2 = new \OauthQq($client_id, $client_secret);
        $token = '';
        if(!empty($code)){
    ...
    public function qqRegister($user_info, $client){
        ...
        //检查是否已经注册
        $tp_union_id = $openid;
        $channel_name = self::TP_LOGIN_CHANNEL_QQ;
        $member_info = $this->getMemberByTpInfo($tp_union_id, $channel_name);
        if (!empty($member_info)) {
        	$ret = array("token" => $member_info['token'], "member" => $member_info);
        	return $ret;
     ...
     * 获取第三方唯一标识
     */
    public function getTpUnionId($tp_union_id, $channel_name)
    {
    	$ret = $channel_name . "____" . $tp_union_id;
    	return $ret;
    }
     ...
     * 根据第三方唯一标识获取用户信息
     * */
    public function getMemberByTpInfo($tp_union_id, $channel_name)
    {
 
    	$ret = array();
    	$tp_union_id = $this->getTpUnionId($tp_union_id, $channel_name);
    	$model = Model("mb_user_token");
    	$mb_userinfo = $model->getByTpUnionId($tp_union_id);
    	
    	if (empty($mb_userinfo))
    	{
    		return $ret;
    	}
    	$member_id = $mb_userinfo['member_id'];
    	$mod = Model("member");
    	$member_info = $mod->getMemberInfoByID($member_id);
    	$member_info = array_merge($mb_userinfo, $member_info);
    	return $member_info;
    }
}
