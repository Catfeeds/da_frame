        ...
        if ($param['trace_title']){
            ...
            if (!empty($matches[1])){
                ...
                foreach ($member_list as $k=>$v){
					...
					$href_url = url('member_snshome', 'index', array('mid' => $v['member_id']));
                    $param['trace_title'] = preg_replace("/@(".$v['member_name'].")([\s|:|：]|$)/is",'<a href=\"%siteurl%' . $href_url .'\" target="_blank">@${1}</a>${2}',$param['trace_title']);
                }
            }
            unset($matches);
        }
