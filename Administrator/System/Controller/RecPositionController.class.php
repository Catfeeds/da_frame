...
class RecPositionController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('rec_position', 'Home');
    }
    ...
    public function rec_list(){
        ...
        $this->render('rec_position.index');
    }
    ...
    public function get_xml(){
        ...
        foreach ($list as $k => $info) {
            ...
            $operation_detail = "<li><a href='{$GLOBALS['_PAGE_URL']}&c=RecPosition&a=rec_edit&rec_id={$info['rec_id']}'>编辑内容</a></li>";
            $operation_detail .= "<li><a href=\"javascript:void(0);\" rec_id=\"{$info['rec_id']}\" datype=\"jscode\"></i>调用代码</a></li>";
            $operation_detail .= "<li><a href=\"{$GLOBALS['_PAGE_URL']}&c=RecPosition&a=rec_view&rec_id={$info['rec_id']}\" target=\"_blank\">预览效果</a></li>";
            if ($operation_detail) {
            ...
            if (is_array($_GET['rec_id'])) {
                foreach($_GET['rec_id'] as $rec_id) {
                    ...
                    if ($result){
                        ...
                        $this->log(L('spd_del,rec_position').'[ID:'.$rec_id.']',1);
                    }else{
                        exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
                    }
                }
    ...
    public function rec_save(){
            ...
            if (is_array($_FILES['pic']['tmp_name'])){
                foreach($_FILES['pic']['tmp_name'] as $k=>$v){
                    ...
                    if (in_array($ext,array('jpg','jpeg','gif','png'))){
                        ...
                        if ($_FILES['pic']['error'][$k] != 0) showMessage(Language::get('spd_common_op_fail'));
                        $c = count($array['body']);
        ...
        $model = Model('rec_position');
        $model->insert($data);
        $this->log(L('spd_add,rec_position').'['.$_POST['rtitle'].']',1);
        showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=RecPosition&a=rec_list');
    }
    ...
    public function rec_edit_save(){
            ...
            if (is_array($_FILES['pic']['tmp_name'])){
                foreach($_FILES['pic']['tmp_name'] as $k=>$v){
                    ...
                    if (in_array($ext,array('jpg','jpeg','gif','png','bmp'))){
                        ...
                        if ($_FILES['pic']['error'][$k] != 0) showMessage(Language::get('spd_common_save_fail'));
        ...
        if ($result){
            ...
            showMessage(Language::get('spd_common_save_fail'),$GLOBALS['_PAGE_URL'] . '&c=RecPosition&a=rec_list');
        }
    }

    public function rec_code(){
		$this->setDirquna('system');
        $this->render('rec_position.code','null_layout');
    }
