    ...
    public function get_xml(){
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Article&a=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Article&a=article',
                            'msg'=>"{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Article&a=article_add&ac_id='.intval($_POST['ac_id']),
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Article&a=article_edit&article_id='.intval($_POST['article_id']),
