...
class DbController extends SystemController {
	public function __construct(){
		parent::__construct();
		Language::read('db', 'Home');
	}
	...
	public function db(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			showMessage($lang['db_index_backup_to_wait'],$GLOBALS['_PAGE_URL'] . '&c=Db&a=db&step=1');
		}
		/**
		 * 数据库备份步骤
		 */
		if (intval($_GET['step']) >= 1){
			...
			if (
				...
				$url = array(
					array(
						'url'=>$GLOBALS['_PAGE_URL'] . '&c=Db&a=db',
				...
				$url = array(
					array(
						'url'=>$GLOBALS['_PAGE_URL'] . '&c=Db&a=db&step='.(intval($_GET['step'])+1),
	...
	public function db_import(){
		...
		if ($_GET['dir_name'] != '' && file_exists(BASE_DATA_PATH.DS.'sql_back'.DS.$_GET['dir_name'])){
			...
			if ($result == 'succ'){
				$url = array(
					array(
						'url'=>$GLOBALS['_PAGE_URL'] . '&c=Db&a=db_restore',
			...
			if ($result == 'continue'){
				$url = array(
					array(
						'url'=>$GLOBALS['_PAGE_URL'] . '&c=Db&a=db_import&dir_name='.$_GET['dir_name'].'&step='.(intval($_GET['step'])+1),
