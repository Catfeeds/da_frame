...
class AdminLogController extends SystemController {
    const EXPORT_SIZE = 1000;

    public function __construct(){
        parent::__construct();
        Language::read('admin_log', 'Home');
    }
    ...
    public function list_del(){
        ...
        if (!Model('admin_log')->where($condition)->delete()){
            $this->log(L('spd_del,spd_admin_log'),0);
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }else{
            $this->log(L('spd_del,spd_admin_log'),1);
    ...
    public function export_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=AdminLog&a=lists');
				$this->setDirquna('system');
    ...
    private function createExcel($data = array()){
        ...
        $excel_obj->addWorksheet($excel_obj->charset(L('spd_admin_log'),CHARSET));
        $excel_obj->generateXML($excel_obj->charset(L('spd_admin_log'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
