...
class AdvController extends SystemController {
        ...
        if(!chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result&&$result2){
                    ...
                    showMessage($lang['adv_add_succ'],$GLOBALS['_PAGE_URL'] . '&c=Adv&a=adv&ap_id='.$_POST['ap_id']);
                }else {
                    showMessage($lang['adv_add_fail'],$GLOBALS['_PAGE_URL'] . '&c=Adv&a=adv&ap_id='.$_POST['ap_id']);
                }
        }
      }
    }
    ...
    public function ap_manage(){
		$this->setDirquna('system');
        $this->render('ap_manage');
    }

    public function get_ap_xml(){
        $lang = Language::getLangContent();
        $adv  = Model('adv');
        $condition  = array();
        ...
        foreach ((array)$ap_list as $k => $ap_info) {
            ...
            $operation_detail = "<li><a href=\"{$GLOBALS['_PAGE_URL']}&c=Adv&a=adv&ap_id={$ap_info['ap_id']}\"></i>管理广告</a></li>";
            $operation_detail .= "<li><a href='{$GLOBALS['_PAGE_URL']}&c=Adv&a=ap_edit&ap_id={$ap_info['ap_id']}'>编辑属性</a></li>";
     ...
     * js代码调用
     */
    public function ap_copy(){
		$this->setDirquna('system');
        $this->render('ap_copy', 'null_layout');
    }
        ...
        if($_POST['form_submit'] != 'ok'){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showMessage($lang['ap_add_succ'],$GLOBALS['_PAGE_URL'] . '&c=Adv&a=ap_manage','html','succ',1,4000);
                }else {
     ...
     */
    public function adv(){
        $this->assign('ap_name',Model()->table('adv_position')->getfby_ap_id(intval($_GET['ap_id']),'ap_name'));
		$this->setDirquna('system');
        $this->render('adv.index');
    }

    public function get_adv_xml(){
        $lang = Language::getLangContent();
        $adv  = Model('adv');
        $condition  = array();
        $condition['ap_id'] = intval($_GET['ap_id']);
        ...
        foreach ((array)$adv_list as $k => $adv_info) {
            $list = array();$operation_detail = '';
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$adv_info['adv_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Adv&a=adv_edit&adv_id={$adv_info['adv_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
