...
class StoreController extends SystemController {
    ...
    public function __construct(){
		...
		$this->links = array(
    	    //shopda 
		    array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=distribution_list','text'=>'分销申请'),
		    array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=distribution_setting','text'=>'分销规则')
        );
    }
    ...
    public function store(){
        ...
        //输出子菜单
        $this->assign('top_link',$this->sublink($this->_links,'distribution_list'));
    ...
    public function distribution_list(){
        ...
        $this->render('store_distribution.list');
    }
    ...
    public function distribution_setting(){
		...
		if (chksubmit()){
			$update_array = array();
			$update_array['shopda_invite2'] = $_POST['shopda_invite2'];
			$update_array['shopda_invite3'] = $_POST['shopda_invite3'];
        ...
        $this->assign('top_link',$this->sublink($this->_links,'distribution_setting'));
		$this->setDirquna('system');
        $this->render('shopda.distribution_setting');
    }

    /**
     * 输出XML数据
     */
    public function get_distribution_xml() {
     ...
     * 审核店铺分销申请
     */
    public function distribution_check() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_distribution = Model('store_distribution');
            $condition = array();
     ...
     * 删除店铺分销申请
     */
    public function distribution_del() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_distribution = Model('store_distribution');
            $condition = array();
