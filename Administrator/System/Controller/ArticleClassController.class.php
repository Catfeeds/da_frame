...
class ArticleClassController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('article_class', 'Home');
    }
    ...
    public function article_class(){
        ...
        $this->render('article_class.index');
    }
    ...
    public function article_class_add(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=ArticleClass&a=article_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=ArticleClass&a=article_class',
                            'msg'=>$lang['article_class_add_back_to_list'],
                        )
                    );
                    $this->log(l('spd_add,article_class_index_class').'['.$_POST['ac_name'].']',1);
        ...
        $this->assign('ac_parent_id',intval($_GET['ac_parent_id']));
        $this->assign('parent_list',$parent_list);
		$this->setDirquna('system');
        $this->render('article_class.add');
    }
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=ArticleClass&a=article_class_edit&ac_id='.intval($_POST['ac_id']),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('spd_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],$GLOBALS['_PAGE_URL'] . '&c=ArticleClass&a=article_class');
                }else {
		...
		$this->setDirquna('system');
        $this->render('article_class.edit');
    }
    ...
    public function article_class_del(){
        ...
        if (intval($_GET['ac_id']) > 0){
            ...
            $this->log(l('spd_add,article_class_index_class').'[ID:'.intval($_GET['ac_id']).']',1);
            showMessage($lang['article_class_index_del_succ'],$GLOBALS['_PAGE_URL'] . '&c=ArticleClass&a=article_class');
        }else {
            showMessage($lang['article_class_index_choose'],$GLOBALS['_PAGE_URL'] . '&c=ArticleClass&a=article_class');
        }
    }
    /**
     * ajax操作
