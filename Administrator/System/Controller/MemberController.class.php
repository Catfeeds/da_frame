...
class MemberController extends SystemController {
    ...
    public function __construct(){
		...
		$this->links = array(
	    array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Member&a=index','lang'=>'member_index_manage'),
		array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add','lang'=>'member_index_predeposit'), 
         );
    }
    ...
    public function member(){
		//输出子菜单
        $this->assign('top_link',$this->sublink($this->links,'index'));
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                    array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=Member&a=member',
                    'msg'=>$lang['member_edit_back_to_list'],
                    ),
                    array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=Member&a=member_edit&member_id='.intval($_POST['member_id']),
                    'msg'=>$lang['member_edit_again'],
                    ),
                    );
                    $this->log(L('spd_edit,member_index_name').'[ID:'.$_POST['member_id'].']',1);
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=Member&a=member_add',
                    'msg'=>$lang['member_add_again'],
                    ),
                    );
                    $this->log(L('spd_add,member_index_name').'[ '.$_POST['member_name'].']',1);
	...
	public function member_del(){
		...
		if (!empty($_GET['member_id'])){
			...
			showMessage('删除成功',$GLOBALS['_PAGE_URL'] . '&c=Member&a=index');
		}else {
			showMessage('删除失败',$GLOBALS['_PAGE_URL'] . '&c=Member&a=index');
    ...
    public function get_xml() {
        ...
        foreach ($member_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Member&a=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>
    ...
    public function export_csv() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Member&a=index');
				$this->setDirquna('system');
    ...
    private function createCsv($member_list) {
        ...
        $this->assign('member_list' .$_GET['curpage'] . '-'.date('Y-m-d'), $data, $header);
	 ...
	 * 添加余额控件
	 */
	public function predeposit_add(){
		if (chksubmit()){
			$obj_validate = new Validate();
			$obj_validate->validateparam = array(
			...
			if (!is_array($member_info) || count($member_info)<=0){
				showMessage(Language::get('admin_points_userrecord_error'),$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add','','error');
			}
			...
			if ($_POST['operatetype'] == 2 && $money > $available_predeposit){
				showMessage(('预存款不足，会员当前预存款').$available_predeposit,$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 3 && $money > $available_predeposit){
				showMessage(('可冻结预存款不足，会员当前预存款').$available_predeposit,$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 4 && $money > $freeze_predeposit){
				showMessage(('可恢复冻结预存款不足，会员当前冻结预存款').$freeze_predeposit,$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add','','error');
			}
			...
			switch ($_POST['operatetype'])
			{
				...
				case 4:
					...
					break;
				default:
					showMessage('操作失败',$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add');
					break;  
			}
			try {
				...
				$model_pd->commit();
				$this->log($log_msg,1);
				showMessage('操作成功',$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add');
			} catch (Exception $e) {
				$model_pd->rollback();
				$this->log($log_msg,0);
				showMessage($e->getMessage(),$GLOBALS['_PAGE_URL'] . '&c=Member&a=predeposit_add','html','error');
			}
		}else{
		   //输出子菜单
          $this->assign('top_link',$this->sublink($this->links,'predeposit_add'));
		  $this->setDirquna('system');
		  $this->render('member.predeposit.add');
		}
	}
