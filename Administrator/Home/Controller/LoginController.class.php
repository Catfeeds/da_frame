...
class LoginController extends SystemController {
    ...
    public function __construct(){
        ...
        Language::read('common,layout,login', 'Home');
        ...
        if ($result){
            ...
            if ($error != '') {
                ...
                if(is_array($admin_info) and !empty($admin_info)) {
                    ...
                    $this->log(L('spd_login'),1);
                    Process::clear('admin');
                    @header("Location: {$GLOBALS['_PAGE_URL']}");exit;
                } else {
                    Process::addprocess('admin');
                    showMessage(L('login_index_username_password_wrong'),"{$GLOBALS['_PAGE_URL']}&c=Login&a=login");
                }
            }
        }
        $this->assign('html_title',L('login_index_need_login'));
