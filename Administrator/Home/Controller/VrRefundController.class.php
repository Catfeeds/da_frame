...
class VrRefundController extends SystemController {
    ...
    public function wxpay() {
        ...
        if(!empty($detail_array) && in_array($detail_array['refund_code'],array('wxpay','wx_jsapi','wx_saoma'))) {
            ...
            if ($refund_amount > 0) {
                ...
                $api_file = BASE_ROOT_PATH.DS.'Api'.DS.'refund'.DS.'wxpay'.DS.'WxPay.Api.php';
                include $api_file;
                ...
                if(!empty($data) && $data['return_code'] == 'SUCCESS') {//请求结果
                    if($data['result_code'] == 'SUCCESS') {//业务结果
                        $detail_array = array();
                        $detail_array['pay_amount'] = daPriceFormat($data['refund_fee']/100);
    ...
    public function alipay() {
        ...
        if(!empty($detail_array) && $detail_array['refund_code'] == 'alipay') {
            ...
            if ($refund_amount > 0) {
                ...
                $api_file = BASE_ROOT_PATH.DS.'Api'.DS.'refund'.DS.'alipay'.DS.'alipay.class.php';
                include $api_file;
                ...
                $parameter['notify_url'] = BASE_API_URL."/refund/alipay/vr_notify_url.php";
    ...
    public function get_detail() {
        ...
        if($detail_array['pay_time'] > 0) {
            $result = array('state'=>'true','msg'=>'成功退款:'.daPriceFormat($detail_array['pay_amount']));
        }
        exit(json_encode($result));
    }
}
