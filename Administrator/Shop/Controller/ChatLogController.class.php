...
class ChatLogController extends SystemController {
    ...
    public function msg_log() {
		...
		$this->setDirquna('shop');
        $this->render('chat_msg_log.list');
    }

    /**
     * 输出XML数据
     */
    public function get_msg_xml() {
        $model_chat = Model('web_chat');
        $log_list = array();
        if (!empty($_GET['msg'])) {
            $add_time_from = strtotime($_GET['add_time_from']);
        ...
        foreach ($log_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='{$GLOBALS['_PAGE_URL']}&c=ChatLog&a=chat_log&f_name=".$value['f_name']."&t_name=".$value['t_name']."&add_time_from=".date("Y-m-d",$value['add_time'])."&add_time_to=".date("Y-m-d",$value['add_time'])."' class='url'><i class='fa fa-list-alt'></i>详情</a>";
            $param['t_msg'] = $value['t_msg'];
            $param['add_time'] = date('Y-m-d H:i:s', $value['add_time']);
            $param['f_name'] = $value['f_name'];
            $param['f_id'] = $value['f_id'];
    ...
    public function chat_info() {
		...
		$this->setDirquna('shop');
        $this->render('chat_log.info', 'null_layout');
    }
}
