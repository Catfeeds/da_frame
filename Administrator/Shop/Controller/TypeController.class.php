...
class TypeController extends SystemController {
    ...
    public function get_xml() {
        ...
        foreach ((array)$type_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick='fg_del(". $value['type_id'] .")'><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Type&a=type_edit&t_id=".$value['type_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function type_add(){
        ...
        if (chksubmit()){
            ...
            $url = array(
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=Type&a=type_add',
                    'msg'=>$lang['type_index_continue_to_dd']
                ),
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=Type&a=type',
                    'msg'=>$lang['type_index_return_type_list']
                )
            );
            $this->log(L('spd_add,type_index_type_name').'['.$_POST['t_mane'].']',1);
        ...
        if(is_array($spec_list) && !empty($spec_list)){
            foreach($spec_list as $k=>$val){
                ...
                $s_list[$val['class_id']]['name'] = $val['class_name']==''?L('spd_default'):$val['class_name'];
            }
        }
        ksort($s_list);
        // 一级分类列表
    ...
    public function type_edit(){
        ...
        if (chksubmit()){
            ...
            if ($return){
                ...
                $this->log(L('spd_edit,type_index_type_name').'['.$_POST['t_mane'].']',1);
                showMessage($lang['spd_common_save_succ'],$url);
            } else {
                $this->log(L('spd_edit,type_index_type_name').'['.$_POST['t_mane'].']',0);
                showMessage($lang['spd_common_save_fail']);
            }
        }
        ...
        $this->assign('type_info', $type_info['0']);
    ...
    public function attr_edit(){
        ...
        $this->assign('attr_value_list', $attr_value_list);
		$this->setDirquna('shop');

        $this->render('type_attr.edit', 'null_layout');
    }
    ...
    public function type_del(){
        ...
        if(is_array($type_list) && !empty($type_list)){
            ...
            $this->log(L('spd_delete,type_index_type_name').'[ID:'.$id.']',1);
            exit(json_encode(array('state'=>true,'msg'=>L('type_index_del_succ'))));
        }else{
            $this->log(L('spd_delete,type_index_type_name').'[ID:'.$id.']',0);
            exit(json_encode(array('state'=>false,'msg'=>L('param_error'))));
        }
    }
}
