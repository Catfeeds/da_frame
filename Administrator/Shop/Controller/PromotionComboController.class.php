...
class PromotionComboController extends SystemController {

    public function __construct(){
        parent::__construct();
		
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionCombo&a=index','text'=>'商品列表'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionCombo&a=combo_quota_list','text'=>'套餐列表'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionCombo&a=combo_setting','text'=>'设置')
    );
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionCombo&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function index() {
        ...
        $this->combo_goods_list();
    }

    /**
     * 活动商品列表
     */
    public function combo_goods_list() {
        $this->assign('top_link',$this->sublink($this->links,'index'));
		$this->setDirquna('shop');
        $this->render('promotion_combo.goods');
    }

    /**
     * 活动商品管理XML
     */
    public function get_combo_goods_xml() {
        $condition = array();
        if ($_POST['query'] != '') {
     ...
     * 删除订金预售商品活动
     */
    public function del_combo_goods() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function combo_quota_list() {
        $this->assign('top_link',$this->sublink($this->links,'combo_quota_list'));
		$this->setDirquna('shop');
        $this->render('promotion_combo.quota');
    }
    ...
    public function combo_setting() {
        ...
        $this->assign('top_link',$this->sublink($this->links,'combo_setting'));
		$this->setDirquna('shop');
        $this->render('promotion_combo.setting');
    }
}
