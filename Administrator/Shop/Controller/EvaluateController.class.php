...
class EvaluateController extends SystemController {

    public function __construct() {
        parent::__construct();
		
		    $this->links = array(
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Evaluate&a=evalgoods_list','text'=>'来自买家的评价'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Evaluate&a=evalstore_list','text'=>'店铺动态评价')
    );
		
        Language::read('evaluate', 'Home');
        if ($_GET['a'] == 'index') $_GET['a'] = 'evalgoods_list';
        ...
        foreach ($evalgoods_list as $k => $evalgoods_info) {
            ...
            if(!empty($evalgoods_info['geval_image'])) {
                ...
                foreach ($image_array as $value) {
                    $list['geval_image'] .= "<li><a datype=\"nyroModal\"  href=\"".snsThumb($value, 1024)."\"> <img src=\"".snsThumb($value)."\"> </a></li>";
                }
            ...
            if(!empty($evalgoods_info['geval_image_again'])) {
                ...
                foreach ($image_array as $value) {
                    $list['geval_image_again'] .= "<li><a datype=\"nyroModal\"  href=\"".snsThumb($value, 1024)."\"> <img src=\"".snsThumb($value)."\"> </a></li>";
                }
