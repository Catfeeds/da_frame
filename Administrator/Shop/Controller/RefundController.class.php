...
class RefundController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		    ...
		    $this->links = array(
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Refund','text'=>'待处理'),
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Refund&a=refund_all','text'=>'所有记录'),
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Refund&a=reason','text'=>'退款退货原因')
    );
    ...
    public function index() {
        ...
        $this->render('refund_manage.list');
    }

    /**
     * 待处理列表
     */
    public function get_manage_xml() {
        $model_refund = Model('refund_return');
        $condition = array();
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"{$GLOBALS['_PAGE_URL']}&c=Refund&a=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function edit() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Refund&a=index');
            } else {
                showMessage(Language::get('spd_common_save_fail'));
            }
        }
    ...
    public function reason() {
        ...
        $this->render('refund_reason.list');
    }
    ...
    public function add_reason() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Refund&a=reason');
            } else {
                showMessage(Language::get('spd_common_save_fail'));
            }
        }
		$this->setDirquna('shop');
        $this->render('refund_reason.add');
    }
    ...
    public function edit_reason() {
		...
		$this->setDirquna('shop');
        $this->render('refund_reason.edit');
    }
    ...
    public function del_reason() {
        ...
        if ($state) {
            ...
            showMessage(Language::get('spd_common_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=Refund&a=reason');
        } else {
    ...
    private function createCsv($refund_list) {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            $list[$k]['refund_amount'] = daPriceFormat($refund_info['refund_amount']);
            if(!empty($refund_info['pic_info'])) {
                $info = unserialize($refund_info['pic_info']);
                if (is_array($info) && !empty($info['buyer'])) {
                    foreach($info['buyer'] as $pic_name) {
