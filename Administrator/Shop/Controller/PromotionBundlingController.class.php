...
class PromotionBundlingController extends SystemController {

    public function __construct(){
        parent::__construct();

        //读取语言包
        Language::read('promotion_bundling', 'Home');
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionBundling&promotion_allow=1',
    ...
    public function bundling_quota()
    {
        ...
        $this->render('promotion_bundling_quota.list');
    }

    /**
     * 套餐管理XML
     */
    public function bundling_quota_xml()
    {
        $condition = array();
    ...
    {
		$this->setDirquna('shop');
        $this->render('promotion_bundling.list');
    }

    /**
     * 活动管理XML
     */
    public function bundling_list_xml()
    {
        $condition = array();
    ...
    public function bundling_setting() {
        ...
        if (chksubmit()){
            ...
            if($return){
                $this->log(L('spd_set,spd_promotion_bundling'));
        ...
        $this->setDirquna('shop');
        $this->render('promotion_bundling.setting');
    }
