...
class StatGoodsController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		   ...
		   $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatGoods&a=pricerange','lang'=>'stat_goods_pricerange'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatGoods&a=hotgoods','lang'=>'stat_hotgoods'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatGoods&a=goods_sale','lang'=>'stat_goods_sale'),
    );
        ...
        //处理搜索时间
        if (!isset($this->search_arr['a']) || in_array($this->search_arr['a'],array('pricerange','hotgoods','goods_sale','get_goods_xml'))){
    ...
    public function pricerange(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'pricerange'));
		$this->setDirquna('shop');
        $this->render('stat.goods.prange');
    }
    /**
     * 热卖商品
     */
    public function hotgoods(){
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $this->assign('searchtime',implode('|',$searchtime_arr));
        $this->assign('top_link',$this->sublink($this->links, 'hotgoods'));
		$this->setDirquna('shop');
        $this->render('stat.goods.hotgoods');
    }
    /**
     * 热卖商品列表
     */
    public function hotgoods_list(){
		...
		$this->setDirquna('shop');
        $this->render('stat.goods.hotgoods.list','null_layout');
    }
    ...
    public function goods_sale(){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $this->assign('top_link',$this->sublink($this->links, 'goods_sale'));
			$this->setDirquna('shop');
            $this->render('stat.goodssale');
        }
    }
    ...
    public function get_goods_xml(){
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                $out_array['goodsamount'] = daPriceFormat($v['goodsamount']);
