...
class VrBillController extends SystemController {
    /**
     * 每次导出订单数量
     * @var int
     */
...
//         $this->render('vr_bill_order_statis.index');
//     }
...
//             $list = array();
//             $list['operation'] = "<a class=\"btn green\" href=\"{$GLOBALS['_PAGE_URL']}&c=VrBill&a=show_statis&os_month={$bill_info['os_month']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
...
//     }
    ...
    public function get_bill_xml(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"{$GLOBALS['_PAGE_URL']}&c=VrBill&a=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"{$GLOBALS['_PAGE_URL']}&c=VrBill&a=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function show_bill(){
		...
		$this->setDirquna('shop');
        $this->render('vr_bill_order_bill.show');
    }

    public function get_bill_info_code_xml(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0) {
            exit();
        }
        ...
        foreach ($code_list as $code_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn green\" href=\"{$GLOBALS['_PAGE_URL']}&c=VrOrder&a=show_order&order_id={$code_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            $list['pay_price'] = daPriceFormat($code_info['pay_price']);
            $list['commis_rate'] = daPriceFormat($code_info['pay_price']*$code_info['commis_rate']/100);
    ...
    public function bill_pay(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                ...
                showMessage('保存成功',$GLOBALS['_PAGE_URL'] . '&c=VrBill');
            }else{
                $this->log('账单付款,账单号：'.$ob_id,1);
    ...
    public function bill_print(){
        ...
        $this->render('vr_bill.print','null_layout');
    }
    ...
    public function export_order(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                $this->assign('list',$array);
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=VrBill&a=show_bill&ob_id='.$ob_id);
				$this->setDirquna('shop');
                $this->render('export.excel');
                exit();
            }else{
