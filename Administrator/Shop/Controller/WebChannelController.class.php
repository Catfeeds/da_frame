...
class WebChannelController extends SystemController {

    public function __construct(){
        parent::__construct();
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=web_channel','text'=>'频道列表'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=floor_list','text'=>'模块列表')
    );
     ...
     * 频道列表
     */
    public function web_channel() {
        $this->assign('top_link',$this->sublink($this->links, 'web_channel'));
		$this->setDirquna('shop');
        $this->render('web_channel.list');
    }

    /**
     * 输出频道列表XML数据
     */
    public function get_channel_xml() {
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $operation .= '<li><a href="' . $GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=edit_channel&channel_id='.$v['channel_id'].'">基本设置</a></li>';
                $operation .= '<li><a href="' . $GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=set_channel&channel_id='.$v['channel_id'].'">编辑模块</a></li>';
                $operation .= '</ul></span>';
    ...
    public function floor_list() {
        $this->assign('top_link',$this->sublink($this->links, 'floor_list'));
		$this->setDirquna('shop');
        $this->render('web_floor.list');
    }

    /**
     * 输出模块列表XML数据
     */
    public function get_floor_xml() {
        $model_channel = Model('web_channel');
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $operation = '';
                $operation .= '<a class="btn purple" href="' . $GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=edit_floor&web_id='.$v['web_id'].'"><i class="fa fa-cog"></i>设置</a>';
                $operation .= '<a class="btn orange" href="' . $GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=edit_'.$v['web_page'].'&web_id='.$v['web_id'].'"><i class="fa fa-steam"></i>模块设计</a>';
                $out_array['operation'] = $operation;
     ...
     * 中部模块数据
     */
    public function get_channel_fl() {
        $model_channel = Model('web_channel');
        $condition = array();
        if (!empty($_GET['name'])) {
        ...
        $this->assign('show_page',$model_channel->showpage());
		$this->setDirquna('shop');
        $this->render('web_channel_fl.list','null_layout');
    }
    ...
    public function add_channel() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=web_channel');
            } else {
                showMessage(Language::get('spd_common_save_fail'));
            }
        }
		$this->setDirquna('shop');
        $this->render('web_channel.add');
    }
    ...
    public function add_floor() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=WebChannel&a=floor_list');
            } else {
                showMessage(Language::get('spd_common_save_fail'));
            }
        }
		$this->setDirquna('shop');
        $this->render('web_floor.add');
    }
    ...
    public function set_channel() {
        ...
        $this->assign('gc_id',$channel['gc_id']);
        $this->assign('top_id',$channel['top_id']);
    ...
    public function edit_floor() {
        ...
        $this->assign('style_array',$style_array);
		$this->setDirquna('shop');
        $this->render('web_floor.edit');
    }

    /**
     * 编辑顶部模块
     */
    public function edit_channel_tp() {
        $model_web_config = Model('web_config');
        ...
        $this->assign('floor',$floor_list[0]);
        $this->setDirquna('shop');
        $this->render('web_channel_tp.edit');
    }

    /**
     * 编辑中部模块
     */
    public function edit_channel_fl() {
        $model_web_config = Model('web_config');
        $web_id = intval($_GET["web_id"]);
        $code_list = $model_web_config->getCodeList(array('web_id'=>"$web_id"));
        if(is_array($code_list) && !empty($code_list)) {
        ...
        $this->assign('floor',$floor_list[0]);
         $this->setDirquna('shop');
        $this->render('web_channel_fl.edit');
    }
    ...
    public function get_category_list() {
        ...
        $this->assign('goods_class',$goods_class);
		$this->setDirquna('shop');
        $this->render('web_channel_category','null_layout');
    }

    /**
     * 检查分类是否已经绑定到频道
     */
    public function check_goods_class() {
        $model_channel = Model('web_channel');
    ...
    public function upload_gc_pic() {
        ...
        if ($gc_id > 0 && !empty($pic_name)) {
            $this->assign('pic',$pic_name);
            $this->assign('var_name',$gc_id);
			$this->setDirquna('shop');
