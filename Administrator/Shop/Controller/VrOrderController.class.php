...
class VrOrderController extends SystemController {
    /**
     * 每次导出订单数量
     * @var int
     */
    ...
    public function get_xml(){
        ...
        foreach ($order_list as $k => $order_info) {
            ...
            $list['operation'] = "<a class=\"btn green\" href=\"{$GLOBALS['_PAGE_URL']}&c=VrOrder&a=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($if_system_receive_pay) {
                $operation_detail .= "<li><a href=\"{$GLOBALS['_PAGE_URL']}&c=VrOrder&a=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">收到货款</a></li>";
            }
            if ($operation_detail) {
            ...
            $list['rcb_amount'] = daPriceFormat($order_info['rcb_amount']);
            $list['pd_amount'] = daPriceFormat($order_info['pd_amount']);
            ...
            $list['refund_amount'] = daPriceFormat($order_info['refund_amount']);
    ...
    public function export_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=VrOrder&a=index');
				$this->setDirquna('shop');
