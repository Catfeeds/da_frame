...
class GoodsAlbumController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('goods_album', 'Home');
    }
    ...
    public function get_xml() {
        ...
        foreach ($album_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick='fg_del(". $value['aclass_id'] .")'><i class='fa fa-trash-o'></i>删除</a><a class='btn green' href='{$GLOBALS['_PAGE_URL']}&c=GoodsAlbum&a=pic_list&aclass_id=".$value['aclass_id']."'><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['aclass_cover'] = "<a href='javascript:void(0);' class='pic-thumb-tip' onMouseOut='toolTip()' onMouseOver='toolTip(\"<img src=".($value['aclass_cover'] != '' ? cthumb($value['aclass_cover'], 60, $value['store_id']) : ADMIN_STATIC_URL.'/images/member/default_image.png').">\")'><i class='fa fa-picture-o'></i></a>";
    ...
    public function pic_list(){
		...
		$this->setDirquna('shop');
        $this->render('goods_album.pic_list');
    }
    ...
    public function aclass_del(){
        ...
        $this->log(L('spd_delete,g_album_one').'[ID:'.intval($_GET['aclass_id']).']',1);
        exit(json_encode(array('state'=>true,'msg'=>L('spd_common_del_succ'))));
    }
    ...
    public function del_album_pic(){
        ...
        $this->log(L('spd_delete,g_album_pic_one').'[ID:'.$apic_id.']',1);
        exit('1');
    }
    ...
    public function del_more_pic(){
        ...
        $this->log(L('spd_delete,g_album_pic_one').'[ID:'.implode(',',$_POST['delbox']).']',1);
        redirect();
    }
