...
class SnstraceController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('snstrace', 'Home');
    }
    ...
    public function get_xml() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $operation .= "<li><a href='{$GLOBALS['_PAGE_URL']}&c=Snstrace&a=commentlist&tid=". $value['trace_id'] ."'>查看评论</a></li>";
            ...
            $param['trace_title'] = str_replace("%siteurl%", SHOP_SITE_URL, $value['trace_title']);
    ...
    public function traceinfo() {
        ...
        if(!empty($trace_info['trace_title'])){
            //替换标题中的siteurl
            $trace_info['trace_title'] = str_replace("%siteurl%", SHOP_SITE_URL, $trace_info['trace_title']);
        }
        if(!empty($trace_info['trace_content'])){
            //替换内容中的siteurl
            $trace_info['trace_content'] = str_replace("%siteurl%", SHOP_SITE_URL, $trace_info['trace_content']);
            //将收藏商品和店铺连接剔除
        ...
        $this->assign('trace_info', $trace_info);
		$this->setDirquna('shop');
        $this->render('snstrace.info', 'null_layout');
    }
    ...
    public function tracedel(){
        ...
        if($result){
            ...
            $this->log(L('spd_del,admin_snstrace_comment'),1);
    ...
    public function traceedit(){
        ...
        if($result){
            ...
            $this->log(L('spd_edit,admin_snstrace_comment'),1);
            showDialog(L('spd_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();');
        }else{
    ...
    public function commentlist(){
		...
		$this->setDirquna('shop');
        $this->render('snscomment.index');
    }

    /**
     * 输出XML数据
     */
    public function get_comment_xml() {
    ...
    public function comminfo() {
		...
		$this->setDirquna('shop');
        $this->render('snscomment.info', 'null_layout');
    }
    ...
    public function commentdel(){
        ...
        if($result){
            $this->log(L('spd_del,admin_snstrace_pl'),1);
            exit(json_encode(array('state'=>true,'msg'=>'删除成功')));
        }else{
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }
    ...
    public function commentedit(){
        ...
        if($result){
            $this->log(L('spd_edit,admin_snstrace_pl'),1);
            showDialog(L('spd_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();');
        }else{
            showDialog(L('spd_common_op_fail'));
        }
