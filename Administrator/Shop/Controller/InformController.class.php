...
class InformController extends SystemController {

    public function __construct(){
        parent::__construct();
		
		    $this->links = array(
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_list','text'=>'未处理'),
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_handled_list','text'=>'已处理'),
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_subject_type_list','text'=>'类型设置'),
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_subject_list','text'=>'主题设置')
    );
		
        Language::read('inform', 'Home');
        if ($_GET['a'] == 'index') $_GET['a'] = 'inform_list';
    ...
    public function get_xml(){
        ...
        if ($inform_list && is_array($inform_list)) {
            ...
            foreach ($inform_list as $k => $v){
                ...
                $list['operation'] = "<a class=\"btn orange\" href=\"{$GLOBALS['_PAGE_URL']}&c=Inform&a=show_handle_page&inform_id={$v['inform_id']}&inform_goods_name={$v['inform_goods_name']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
                } else {
                    $list['operation'] = '--';
                }
    ...
    public function inform_subject_type_list() {
		...
		$this->setDirquna('shop');
        $this->render('inform_subject_type.list') ;
    }
    ...
    public function inform_subject_list(){
        ...
        $this->assign('show_page',$page->show()) ;
		$this->setDirquna('shop');
        $this->render('inform_subject.list') ;
    }
...
$this->setDirquna('shop');
        $this->render('inform_subject_type.add') ;
    ...
    public function inform_subject_type_save(){
        ...
        if ($error != ''){
            ...
            $this->log(L('spd_add,inform_type').'['.$_POST['inform_type_name'].']',1);
            showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_subject_type_list');
        }
    }
    ...
    public function inform_subject_type_drop(){
        ...
        if(empty($inform_type_id)) {
            showMessage(Language::get('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Inform');
        }
        ...
        $this->log(L('spd_del,inform_type').'[ID:'.$_POST['inform_type_id'].']',1);
        showMessage(Language::get('spd_common_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_subject_type_list');
    ...
    public function inform_subject_add(){
        ...
        $this->assign('list', $inform_type_list) ;
		$this->setDirquna('shop');
        $this->render('inform_subject.add') ;
    ...
    public function inform_subject_save(){
        ...
        if ($error != ''){
            ...
            $this->log(L('spd_add,inform_subject').'['.$input['inform_subject_type_name'].']',1);
            showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_subject_list');
        }
    }
    ...
    public function inform_subject_drop(){
        ...
        $this->log(L('spd_del,inform_subject').'['.$_POST['inform_subject_id'].']',1);
        showMessage(Language::get('spd_common_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_subject_list');
    }
    ...
    public function inform_handle(){
        ...
        if($model_inform->updateInform($update_array,$where_array)) {
            ...
            showMessage(Language::get('spd_common_op_succ'),$GLOBALS['_PAGE_URL'] . '&c=Inform&a=inform_list');
        }
        else {
