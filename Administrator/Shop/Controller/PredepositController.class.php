...
class PredepositController extends SystemController {
    const EXPORT_SIZE = 1000;
    public function __construct(){
        parent::__construct();
        Language::read('predeposit', 'Home');
    }
    ...
    public function recharge_edit(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=predeposit','','error');
        }
        //查询充值信息
        ...
        if (empty($info)){
            showMessage(Language::get('admin_predeposit_record_error'),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=predeposit','','error');
        }
        if (!chksubmit()) {
            //显示支付接口列表
        ...
        try {
            ...
            showMessage(Language::get('admin_predeposit_recharge_edit_success'),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=predeposit');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=predeposit','html','error');
        }
    }
    ...
    public function recharge_info(){
        ...
        $this->assign('info',$info);
		$this->setDirquna('shop');
        $this->render('pd.info', 'null_layout');
    ...
    public function pd_cash_list(){
		$this->setDirquna('shop');
        $this->render('pd_cash.list');
    }

    /**
     * 删除提现记录
     */
    public function pd_cash_del(){
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_pd = Model('predeposit');
            $condition = array();
     ...
     * 更改提现为支付状态
     */
    public function pd_cash_pay(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list','','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_id'] = $id;
        ...
        if (!is_array($info) || count($info)<0){
            showMessage(Language::get('admin_predeposit_record_error'),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list','','error');
        }
        ...
        try {
            ...
            $model_pd->commit();
            $this->log($log_msg,1);
            showMessage(Language::get('admin_predeposit_cash_edit_success'),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list','html','error');
        }
    }
    ...
    public function pd_cash_view(){
        ...
        $this->assign('info',$info);
		$this->setDirquna('shop');
        $this->render('pd_cash.view', 'null_layout');
    }
    ...
    public function export_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=predeposit');
				$this->setDirquna('shop');
    ...
    private function createExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$v){
            ...
            $tmp[] = array('format'=>'Number','data'=>daPriceFormat($v['pdr_amount']));
    ...
    public function export_cash_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('list',$array);
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_cash_list');
				$this->setDirquna('shop');
                $this->render('export.excel');
            }else{  //如果数量小，直接下载
    ...
    private function createCashExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$v){
            ...
            $tmp[] = array('format'=>'Number','data'=>daPriceFormat($v['pdc_amount']));
    ...
    public function export_mx_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('list',$array);
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Predeposit&a=pd_log_list');
				$this->setDirquna('shop');
                $this->render('export.excel');
            }else{  //如果数量小，直接下载
    ...
    private function createmxExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$v){
            ...
            if (floatval($v['lg_av_amount']) == 0){
                ...
                $tmp[] = array('format'=>'Number','data'=>daPriceFormat($v['lg_av_amount']));
            }
            if (floatval($v['lg_freeze_amount']) == 0){
                $tmp[] = array('data'=>'');
            } else {
                $tmp[] = array('format'=>'Number','data'=>daPriceFormat($v['lg_freeze_amount']));
            }
    ...
    public function get_xml() {
        ...
        foreach ($recharge_list as $value) {
            ...
            $operation .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('recharge_info','查看充值编号“". $value['pdr_sn'] ."”的明细','{$GLOBALS['_PAGE_URL']}&c=Predeposit&a=recharge_info&id=".$value['pdr_id']."', '640')\"><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['pdr_amount'] = daPriceFormat($value['pdr_amount']);
    ...
    public function get_cash_xml() {
        ...
        foreach ($cash_list as $value) {
            ...
            $param['operation'] .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('cash_info','查看提现编号“". $value['pdc_sn'] ."”的明细', '{$GLOBALS['_PAGE_URL']}&c=Predeposit&a=pd_cash_view&id=". $value['pdc_id'] ."', 640)\" ><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['pdc_amount'] = daPriceFormat($value['pdc_amount']);
    ...
    public function get_log_xml() {
        ...
        foreach ($log_list as $value) {
            ...
            $param['lg_av_amount'] = daPriceFormat($value['lg_av_amount']);
            $param['lg_freeze_amount'] = daPriceFormat($value['lg_freeze_amount']);
