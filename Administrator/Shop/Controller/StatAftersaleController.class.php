...
class StatAftersaleController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		   ...
		   $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatAftersale&a=refund','lang'=>'stat_refund'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatAftersale&a=evalstore','lang'=>'stat_evalstore'),
    );
        ...
        //处理搜索时间
        if (!isset($this->search_arr['a']) || in_array($this->search_arr['a'],array('refund','refundlist','get_refundlist_xml','get_refund_highcharts'))){
    ...
    public function refund(){
        $this->assign('top_link',$this->sublink($this->links, 'refund'));
		$this->setDirquna('shop');
        $this->render('stat.aftersale.refund');
    }
    /**
     * 退款统计
     */
     ...
     * 退款统计
     */
    public function get_refund_highcharts(){
        $where = array();
     ...
     * 输出退款统计XML数据
     */
    public function get_refundlist_xml(){
        $where = array();
        $model = Model('stat');
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where['add_time'] = array('between',$searchtime_arr);
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $out_array['refund_amount'] = daPriceFormat($v['refund_amount']);
    ...
    public function evalstore(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'evalstore'));
		$this->setDirquna('shop');
        $this->render('stat.aftersale.evalstore');
    }

    /**
     * 输出店铺动态评分统计XML数据
     */
    public function get_evalstore_xml(){
        $model = Model('stat');
        $where = array();
        if(intval($_GET['store_class']) > 0){
            $where['sc_id'] = intval($_GET['store_class']);
