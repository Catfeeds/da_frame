...
class RedpacketController extends SystemController {
    //每次导出订单数量
    ...
    public function __construct(){
        ...
        if (C('redpacket_allow') != 1){
            showDialog('需开启“平台红包”功能',$GLOBALS['_PAGE_URL'] . '&c=Operation','succ');
        }
        ...
        if (chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(L('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Redpacket&a=rptlist','succ');
                }else{
                    showDialog(L('spd_common_save_fail'),'','error');
                }
            }
        }else {
            $this->assign('gettype_arr',$this->gettype_arr);
            $this->assign('member_grade',$this->member_grade_arr);
			$this->setDirquna('shop');
            $this->render('redpacket.templateadd');
        }
    }
    /**
     * 红包列表
    ...
    {
        $this->assign('gettype_arr',$this->gettype_arr);
        $this->assign('templateState',$this->templatestate_arr);
		$this->setDirquna('shop');
        $this->render('redpacket.templatelist');
    }

    /**
     * 红包模板列表XML
     */
    public function rptlist_xml()
    {
        $where = array();
     ...
     * 红包模版编辑
     */
    public function rptedit(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            $t_id = intval($_POST['tid']);
        }
        if ($t_id <= 0){
            showDialog(L('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Redpacket&a=rptlist');
        }
        $model_redpacket = Model('redpacket');
        //查询模板信息
        $where = array();
        ...
        if(chksubmit()){            
			...
			$this->setDirquna('shop');
            $this->render('redpacket.templateedit');
        }
    }
    ...
    */
    public function rptinfo(){
        ...
        $this->assign('t_info',$t_info);
		$this->setDirquna('shop');
        $this->render('redpacket.templateinfo');
    }
    /**
     * 红包列表XML
     */
    public function rplist_xml()
    {
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            echo $this->flexigridXML(array());
            exit;
        }
        $model_redpacket = Model('redpacket');
    ...
    public function export_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Redpacket&a=rptinfo&tid='.$t_id);
				$this->setDirquna('shop');
