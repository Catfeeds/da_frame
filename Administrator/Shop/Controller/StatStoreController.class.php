...
class StatStoreController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		    ...
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=newstore','lang'=>'stat_newstore'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=hotrank','lang'=>'stat_storehotrank'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=storesales','lang'=>'stat_storesales'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=degree','lang'=>'stat_storedegree'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=storearea','lang'=>'stat_storearea'),
    );
        ...
        //处理搜索时间
        if (in_array($_REQUEST['a'],array('hotrank','storesales','get_storesales_xml','storesales_list','storesales_trends'))){
    ...
    public function newstore(){
        //导出excel连接地址
        $actionurl = $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=newstore';
        $where = array();
        //所属店铺分类
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $this->assign('top_link',$this->sublink($this->links, 'newstore'));
			$this->setDirquna('shop');
            $this->render('stat.newstore');
        }
    }
    /**
     * 热卖排行
    ...
    public function hotrank(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'hotrank'));
		$this->setDirquna('shop');
        $this->render('stat.store.hotrank');
    }
    /**
     * 热卖排行列表
     */
    public function hotrank_list(){
                ...
                if ($i <= $datanum){
                    ...
                    switch ($_GET['type']){
                       case 'ordernum':
                           ...
                           break;
                       case 'orderamount':
                           $tmp['orderamount'] = daPriceFormat($store_arr[$k]['orderamount']);
                           break;
                    }
		...
		$this->setDirquna('shop');
        $this->render('stat.store.hotrank.list','null_layout');
    }
    /**
     * 店铺等级
     */
    public function degree(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'degree'));
		$this->setDirquna('shop');
        $this->render('stat.storedegree');
    }
    /**
     * 查看店铺列表
     */
    public function showstore(){
        ...
        $this->links[] = array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=showstore','lang'=>'stat_storelist');
        $this->assign('top_link',$this->sublink($this->links, 'showstore'));
		$this->setDirquna('shop');
        $this->render('stat.info.storelist');
    }
    /**
     * 输出店铺列表XML数据
     */
    public function get_store_xml(){
        $model = Model('stat');
    ...
    public function storesales(){
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $this->assign('searchtime',implode('|',$searchtime_arr));
        $this->assign('top_link',$this->sublink($this->links, 'storesales'));
		$this->setDirquna('shop');
        $this->render('stat.store.sales');
    }
    /**
     * 店铺销售统计列表
     */
     ...
     * 输出店铺销售统计XML数据
     */
    public function get_storesales_xml(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                $out_array['operation'] = "<a class='btn green' href='javascript:void(0);' spd_type='showtrends' data-param='{\"storeid\":\"{$v['store_id']}\"}'><i class='fa fa-line-chart'></i>走势图</a>";
                $out_array['orderamount'] = daPriceFormat($v['orderamount']);
                $statlist[$v['store_id']] = $out_array;
            }
        }
    ...
    public function storesales_trends(){
        ...
        if ($storeid <= 0){
			...
			$this->setDirquna('shop');
            $this->render('stat.store.salestrends');
            exit();
        }
        if (!$_GET['search_type']){
        ...
        $this->assign('stat_json',$stat_json);
		$this->setDirquna('shop');
        $this->render('stat.store.salestrends','null_layout');
    }
    /**
     * 地区分布
     */
    public function storearea(){
        ...
        $this->assign('stat_json',$stat_json);
        $this->assign('statlist',$statlist);
        $actionurl = $GLOBALS['_PAGE_URL'] . '&c=StatStore&a=showstore&type=storearea';
        if (trim($_GET['search_time'])){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'storearea'));
		$this->setDirquna('shop');
        $this->render('stat.storearea');
    }
}
