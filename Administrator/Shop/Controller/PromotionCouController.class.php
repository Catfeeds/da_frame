...
class PromotionCouController extends SystemController {
    public function __construct()
    {
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionCou&promotion_allow=1',
    ...
    public function cou_list()
    {
        ...
        $this->assign('couStates', $couStates);

        $this->showMenu('cou_list');
		$this->setDirquna('shop');
        $this->render('promotion_cou.list');
    }

    /**
     * 活动列表
     */
    public function cou_list_xml()
    {
        $condition = array();
...
EOB;
    ...
    public function cou_detail()
    {
        ...
        $this->assign('couDetail', $couDetail);
		
		$this->setDirquna('shop');

        $this->render('promotion_cou.detail', 'null_layout');
    }

    /**
     * 活动取消
     */
    public function cou_cancel()
    {
    ...
    public function cou_setting()
    {
		...
		$this->setDirquna('shop');
        $this->render('promotion_cou.setting');
    }

    public function cou_setting_save()
    {
    ...
    public function cou_quota()
    {
        $this->showMenu('cou_quota');
		$this->setDirquna('shop');
        $this->render('promotion_cou.quota');
    }

    public function cou_quota_xml()
    {
        $condition = array();
    ...
    private function showMenu($menu_key)
    {
        $menu_array = array(
            'cou_list' => array(
                'menu_type' => 'link',
                'menu_name' => '活动列表',
                'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionCou&a=cou_list',
            ),
            'cou_detail' => array(
                'menu_type' => 'link',
                'menu_name' => '活动详情',
            ...
            'cou_quota' => array(
                'menu_type' => 'link',
                'menu_name' => '套餐管理',
                'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionCou&a=cou_quota',
            ),
            'cou_setting' => array(
                'menu_type' => 'link',
                'menu_name' => '设置',
                'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionCou&a=cou_setting',
            ),
        );
