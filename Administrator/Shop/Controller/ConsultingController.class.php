...
class ConsultingController extends SystemController {

    public function __construct(){
        parent::__construct();
		
				    $this->links = array(
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Consulting&a=consulting','text'=>'咨询管理'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Consulting&a=type_list','text'=>'类型设置'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Consulting&a=setting','text'=>'头部文字设置')
    );
		
        Language::read('consulting', 'Home');
        if ($_GET['a'] == 'index') $_GET['a'] = 'consulting';
    ...
    public function consulting(){
        ...
        $this->render('consulting.index');
    }

    public function get_consulting_xml(){
    ...
    public function delete(){
        ...
        if (preg_match('/^[\d,]+$/', $_GET['consult_id'])) {
            ...
            if ($del) {
                $this->log(L('spd_delete,consulting').'[ID:'.implode(',',$_GET['consult_id']).']',null);
    ...
    public function setting() {
		...
		$this->setDirquna('shop');
        $this->render('consulting.setting');
    }
    ...
    public function type_list() {
        ...
        $this->assign('top_link',$this->sublink($this->links,'type_list'));
						
		$this->setDirquna('shop');
        $this->render('consulting.type_list');
    }
    ...
    public function type_add() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('spd_common_save_succ'), urlAdminShop('consulting', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
                showMessage(L('spd_common_save_fail'));
            }
		...
		$this->setDirquna('shop');
        $this->render('consulting.type_add');
    }
    ...
    public function type_edit() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('spd_common_op_succ'), urlAdminShop('consulting', 'type_list'));
            }else {
		...
		$this->setDirquna('shop');
        $this->render('consulting.type_edit');
    }
