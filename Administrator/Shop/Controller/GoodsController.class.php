...
class GoodsController extends SystemController {
    ...
    public function __construct() {
        parent::__construct ();
        Language::read('goods', 'Home');
		    $this->links = array(
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Goods&a=goods','text'=>'所有商品'),
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Goods&a=lockup_list','text'=>'违规下架'),
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Goods&a=waitverify_list','text'=>'等待审核'),
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Goods&a=goods_set','text'=>'商品设置'),
    );
    }
    ...
    public function goods() {
        ...
        $this->assign('top_link',$this->sublink($this->links,'goods'));
     ...
     * 违规下架商品管理
     */
    public function lockup_list() {
        $this->assign('type', 'lockup');
        $this->assign('top_link',$this->sublink($this->links,'lockup_list'));
     ...
     * 等待审核商品管理
     */
    public function waitverify_list() {
        $this->assign('type', 'waitverify');
        $this->assign('top_link',$this->sublink($this->links,'waitverify_list'));
    ...
    public function get_xml() {
        ...
        foreach ($goods_list as $value) {
            ...
            $param['goods_price'] = daPriceFormat($value['goods_price']);
            ...
            $param['goods_marketprice'] = daPriceFormat($value['goods_marketprice']);
            $param['goods_costprice'] = daPriceFormat($value['goods_costprice']);
            $param['goods_freight'] = $value['goods_freight'] == 0 ? '免运费' : daPriceFormat($value['goods_freight']);
    ...
    public function goods_lockup() {
        if (chksubmit()) {
            ...
            showDialog(L('spd_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();CUR_DIALOG.close()');
        }
        $common_info = Model('goods')->getGoodsCommonInfoByID($_GET['id']);
        $this->assign('common_info', $common_info);
						
		$this->setDirquna('shop');
        $this->render('goods.close_remark', 'null_layout');
    }
    ...
    public function goods_verify(){
		...
		$this->setDirquna('shop');
        $this->render('goods.verify_remark', 'null_layout');
    }

    /**
     * ajax获取商品列表
     */
    public function get_goods_sku_list() {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            foreach ($goods_spec as $k => $v) {
                $spec_array[] = '<div class="goods_spec">' . $spec_name[$k] . L('spd_colon') . '<em title="' . $v . '">' . $v .'</em>' . '</div>';
            }
...
//         echo json_encode($goods_list);
		...
		$this->setDirquna('shop');
        $this->render('goods.sku_list', 'null_layout');
    }
    ...
    public function goods_set() {
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('spd_edit,spd_goods_set'),1);
                showMessage(L('spd_common_save_succ'));
            }else {
                $this->log(L('spd_edit,spd_goods_set'),0);
        ...
        $this->assign('top_link',$this->sublink($this->links,'goods_set'));
						
		$this->setDirquna('shop');
        $this->render('goods.setting');
    }
    ...
    public function export_csv() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Goods&a=index');
