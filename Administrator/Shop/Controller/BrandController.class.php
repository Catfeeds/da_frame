...
class BrandController extends SystemController {
    ...
    public function get_xml() {
        ...
        foreach ($brand_list as $value) {
            ...
            if ($_GET['type'] == 'apply') {
                ...
                $operation .= "<a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Brand&a=brand_edit&brand_id=" . $value['brand_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Brand&a=brand_add',
                            'msg'=>$lang['brand_add_again'],
                        ),
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Brand&a=brand',
                            'msg'=>$lang['brand_add_back_to_list'],
                        )
                    );
                    $this->log(L('spd_add,brand_index_brand').'['.$_POST['brand_name'].']',1);
    ...
    public function brand_edit(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
				...
				if ($_FILES['brand_filebgpic']['name'] != ''){
					$upload = new UploadFile();
					$upload->set('default_dir',ATTACH_BRAND . DS . "bgpic");
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Brand&a=brand_edit&brand_id='.intval($_POST['brand_id']),
                    ...
                    $this->log(L('spd_edit,brand_index_brand').'['.$_POST['brand_name'].']',1);
                    showMessage($lang['spd_common_save_succ'],$url);
                }else {
                    $this->log(L('spd_edit,brand_index_brand').'['.$_POST['brand_name'].']',0);
                    showMessage($lang['spd_common_save_fail']);
                }
            }
        }
    ...
    public function brand_del(){
        ...
        $this->log(L('spd_delete,brand_index_brand').'[ID:'.$brand_id.']',1);
    ...
    public function export_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('list',$array);
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Brand&a=brand');
