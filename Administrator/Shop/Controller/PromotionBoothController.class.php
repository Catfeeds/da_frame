...
class PromotionBoothController extends SystemController {
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionBooth&promotion_allow=1',
                    'msg'=>L('open'),
                ),
            );
    ...
    public function goods_list()
    {
        ...
        $this->render('promotion_booth_goods.list');
    }
...
EOB;
     ...
     * 套餐列表
     */
    public function booth_quota_list()
    {
		$this->setDirquna('shop');
        $this->render('promotion_booth_quota.list');
    }

    /**
     * 套餐列表XML
     */
    public function booth_quota_list_xml()
    {
        $condition = array();
        if (strlen($q = trim($_REQUEST['query']))) {
            switch ($_REQUEST['qtype']) {
    ...
    public function booth_setting() {
        ...
        $this->setDirquna('shop');
        $this->render('promotion_booth.setting');
    }
}
