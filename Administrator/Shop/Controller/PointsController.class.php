...
class PointsController extends SystemController {
    const EXPORT_SIZE = 5000;
    public function __construct(){
        parent::__construct();
        Language::read('points', 'Home');
        //判断系统是否开启积分功能
        if (C('points_isuse') != 1){
            showMessage(Language::get('admin_points_unavailable'),$GLOBALS['_PAGE_URL'] . '&c=Setting','','error');
        }
    }
    ...
    public function addpoints(){
        if (chksubmit()){
            ...
            if (!is_array($member_info) || count($member_info)<=0){
                showMessage(Language::get('admin_points_userrecord_error'),$GLOBALS['_PAGE_URL'] . '&c=Points&a=addpoints','','error');
            }

            $pointsnum = intval($_POST['pointsnum']);
            if ($_POST['operatetype'] == 2 && $pointsnum > intval($member_info['member_points'])){
                showMessage(Language::get('admin_points_points_short_error').$member_info['member_points'],$GLOBALS['_PAGE_URL'] . '&c=Points&a=addpoints','','error');
            }
            ...
            if ($result){
                ...
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Points&a=addpoints');
            }else {
                showMessage(Language::get('spd_common_save_fail'),$GLOBALS['_PAGE_URL'] . '&c=Points&a=addpoints','','error');
            }
        }else {
			$this->setDirquna('shop');
            $this->render('points.add');
        }
    }
