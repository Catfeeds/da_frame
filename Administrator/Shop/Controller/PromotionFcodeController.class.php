...
class PromotionFcodeController extends SystemController {

    public function __construct(){
        parent::__construct();
		
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionFcode&a=index','text'=>'商品列表'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionFcode&a=fcode_quota_list','text'=>'套餐列表'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionFcode&a=fcode_setting','text'=>'设置')
    );
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionFcode&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function goods_list() {
        ...
        $this->render('promotion_fcode.goods');
    }
     ...
     * 套餐列表
     */
    public function fcode_quota_list() {
        $this->assign('top_link',$this->sublink($this->links,'fcode_quota_list'));
		$this->setDirquna('shop');
        $this->render('promotion_fcode.quota');
    }
    ...
    public function fcode_setting() {
        ...
        $this->assign('top_link',$this->sublink($this->links,'fcode_setting'));
		$this->setDirquna('shop');
        $this->render('promotion_fcode.setting');
    }
}
