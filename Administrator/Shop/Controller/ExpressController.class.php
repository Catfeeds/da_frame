...
class ExpressController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('express', 'Home');
    }

    public function index(){
						
		$this->setDirquna('shop');
        $this->render('express.index');
    }
    ...
    public function get_xml() {
        ...
        foreach ($list as $value) {
            ...
            $operation .= "<li><a href='{$GLOBALS['_PAGE_URL']}&c=Express&a=edit_express&id=".$value['id']."'>编辑快递公司</a></li>";
    ...
    public function ajax(){
        switch ($_GET['column']){
            case 'e_state':
                ...
                dkcache('express');
                $this->log(L('spd_edit,express_name,express_state').'[ID:'.intval($_GET['id']).']',1);
    ...
    function add_express(){
		...
		$param['e_code_kdniao'] = trim($_POST['e_code_kdniao']);
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Express&a=index',
                            'msg'=>'返回快递公司列表',
                        ),
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Express&a=add_express',
                            'msg'=>'继续新增快递公司',
                        ),
                    );
                    dkcache('express');
    ...
    function edit_express(){
		...
		$param['e_code_kdniao'] = trim($_POST['e_code_kdniao']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Express&a=edit_express&id='.intval($_POST['id']),
                            'msg'=>'重新编辑该快递公司',
                        ),
                    );
                    dkcache('express');
        ...
        $this->assign('express_array',$express_array);
        $this->setDirquna('shop');
        $this->render('express.edit');
    }
