...
class StoreController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
        Language::read('store,store_grade', 'Home');
        
        $this->_links = array(
        		array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=store','text'=>'管理'),
        		array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=store_joinin','text'=>'开店申请'),
        		array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=reopen_list','text'=>'续签申请'),
        		array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=store_bind_class_applay_list','text'=>'经营类目申请'),
        		array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Store&a=bill_cycle','text'=>'结算周期设置')
        );
    }
    ...
    public function store(){
        ...
        //输出子菜单
        $this->assign('top_link',$this->sublink($this->_links,'store'));
     ...
     * 店铺结算周期
     */
    public function bill_cycle(){

        $this->assign('top_link',$this->sublink($this->_links,'bill_cycle'));
		$this->setDirquna('shop');

        $this->render('store.bill_cycle');
    }
    ...
    public function get_xml() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='{$GLOBALS['_PAGE_URL']}&c=Store&a=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('spd_set') . " <i class='arrow'></i></em><ul><li><a href='{$GLOBALS['_PAGE_URL']}&c=Store&a=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='{$GLOBALS['_PAGE_URL']}&c=Store&a=del&store_id=" . $value['store_id'] . "'>删除该店铺</a></li><li><a href='{$GLOBALS['_PAGE_URL']}&c=Store&a=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
     ...
     * 输出XML数据
     */
    public function get_bill_cycle_xml() {
        $model_store = Model('store');
        $condition = array();
        if ($_GET['store_name'] != '') {
            $condition['store_name'] = array('like', '%' . $_GET['store_name'] . '%');
        ...
        foreach ($store_list as $value) {
            $param = array();
            $store_state = $this->getStoreState($value);
            $operation = "<a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Store&a=bill_cycyle_edit&store_id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='". urlShop('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
    ...
    public function export_csv() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Store&a=index');
				$this->setDirquna('shop');
    ...
    public function store_edit(){
	    ...
	    // 新增编辑店铺地址 公司名称
            ...
            if ($result){
                $url = array(
                array(
                'url'=>$GLOBALS['_PAGE_URL'] . '&c=Store&a=store',
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>$GLOBALS['_PAGE_URL'] . '&c=Store&a=store_edit&store_id='.intval($_POST['store_id']),
                'msg'=>$lang['countinue_add_store'],
                ),
                );
                $this->log(L('spd_edit,store').'['.$_POST['store_name'].']',1);
                showMessage($lang['spd_common_save_succ'],$url);
            }else {
                $this->log(L('spd_edit,store').'['.$_POST['store_name'].']',1);
		...
		$this->assign('store_zizhi',$store_array['is_person']);
     ...
     * 店铺结算周期编辑
     */
    public function bill_cycyle_edit(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage($lang['spd_common_save_succ'],$GLOBALS['_PAGE_URL'] . '&c=Store&a=bill_cycle');
            }else {
                $this->log('设置店铺结算周期['.$_POST['store_name'].']',1);
                showMessage($lang['spd_common_save_fail'],$GLOBALS['_PAGE_URL'] . '&c=Store&a=bill_cycle');
            }
        }
        ...
        $this->assign('store_array',$store_array);
		$this->setDirquna('shop');
        $this->render('store.bill_cycle_edit');
    }
    ...
    public function edit_save_joinin() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('spd_common_op_succ'), $GLOBALS['_PAGE_URL'] . '&c=Store&a=store');
            } else {
    ...
    public function store_bind_class_update() {
        ...
        if ($new_commis_rate < 0 || $new_commis_rate >= 100) {
            ...
            if($result) {
                ...
                echo json_encode(array('result'=>FALSE,'message'=>L('spd_common_op_fail')));
                die;
            }
        }
    }

    public function shopdashop_add()
    {
        if (chksubmit())
        {
            ...
            showMessage('操作成功',$GLOBALS['_PAGE_URL'] . '&c=Store&a=store');
            return;
        }
		$this->setDirquna('shop');

        $this->render('store.add');
    }
    ...
    public function store_joinin(){
        ...
        //输出子菜单
        $this->assign('top_link',$this->sublink($this->_links,'store_joinin'));
		$this->setDirquna('shop');
     ...
     * 输出XML数据
     */
    public function get_joinin_xml() {
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"{$GLOBALS['_PAGE_URL']}&c=Store&a=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"{$GLOBALS['_PAGE_URL']}&c=Store&a=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
            $param['member_id'] = $value['member_id'];
            $param['member_name'] = $value['member_name'];
            $param['sg_id'] = $value['sg_name'];
            $param['paying_amount'] = daPriceFormat($value['paying_amount']);
    ...
    public function store_bind_class_applay_list(){
        $this->assign('top_link',$this->sublink($this->_links,'store_bind_class_applay_list'));
		$this->setDirquna('shop');
        $this->render('store.bind_class_applay_list');
    }

    /**
     * 输出XML数据
     */
    public function get_bind_class_applay_xml() {
        $model_store_bind_class = Model('store_bind_class');
        // 设置页码参数名称
        $condition = array();
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = '{$GLOBALS['_PAGE_URL']}&c=Store&a=store_bind_class_applay_check&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = '{$GLOBALS['_PAGE_URL']}&c=Store&a=store_bind_class_applay_del&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
    ...
    public function reopen_list(){
        $this->assign('top_link',$this->sublink($this->_links,'reopen_list'));
		$this->setDirquna('shop');
        $this->render('store_reopen.list');
    }

    /**
     * 输出XML数据
     */
    public function get_reopen_xml() {
        ...
        foreach ($reopen_list as $value) {
            ...
            $param['re_grade_price'] = daPriceFormat($value['re_grade_price']);
            $param['re_year'] = $value['re_year'];
            $param['re_pay_amount'] = daPriceFormat($value['re_pay_amount']);
    ...
    public function store_joinin_detail(){
        ...
        //5.2 shopda 分开显示个人 企业入驻信息
	if ($joinin_detail['is_person']) {
			$this->render('store_joinin_c2c.detail');
		} else {
			$this->render('store_joinin.detail');
		}
    }
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('店铺入驻申请审核完成',$GLOBALS['_PAGE_URL'] . '&c=Store&a=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
	    ...
	    $shop_array['is_person']= $joinin_detail['is_person'];
            ...
            if($state) {
                ...
                showMessage('店铺开店成功',$GLOBALS['_PAGE_URL'] . '&c=Store&a=store_joinin');
            } else {
                showMessage('店铺开店失败',$GLOBALS['_PAGE_URL'] . '&c=Store&a=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝',$GLOBALS['_PAGE_URL'] . '&c=Store&a=store_joinin');
        }
    }
    ...
    public function remind_renewal() {
        ...
        if (!empty($store_info) && $store_info['store_end_time'] < (TIMESTAMP + 864000) && cookie('remindRenewal'.$store_id) == null) {
            ...
            setDaCookie('remindRenewal'.$store_id, 1, 86400 * 10);  // 十天
            showMessage('消息发送成功');
        }
            showMessage('消息发送失败');
    }
