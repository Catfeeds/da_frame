    ...
    public function activity_xml()
    {
        ...
        foreach ($list as $val) {
            ...
            if ($val['activity_state'] == 0 || $val['activity_end_date'] < time()) {
                $o .= '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="' . $GLOBALS['_PAGE_URL'] . '&c=Activity&a=del&activity_id=' .
                    $val['activity_id'] .
            ...
            $o .= '<li><a href="' . $GLOBALS['_PAGE_URL'] . '&c=Activity&a=edit&activity_id=' .
                    $val['activity_id'] .
                    '">编辑活动</a></li>';

            $o .= '<li><a href="' . $GLOBALS['_PAGE_URL'] . '&c=Activity&a=detail&id=' .
                $val['activity_id'] .
                '">处理申请</a></li>';
...
EOB;
    ...
    public function create(){
        ...
        if($result){
            $this->log(L('spd_add,activity_index').'['.$_POST['activity_title'].']',null);
            showMessage(Language::get('spd_common_op_succ'),$GLOBALS['_PAGE_URL'] . '&c=Activity&a=activity');
        }else{
    ...
    public function del()
    {
        ...
        if($activity_detail->del($id_new)){
            if($activity->del($id_new)){
                $this->log(L('spd_del,activity_index').'[ID:'.$id.']',null);
                $this->jsonOutput();
            }
        }
    ...
    public function edit(){
        ...
        if($result){
            ...
            $this->log(L('spd_edit,activity_index').'[ID:'.$_POST['activity_id'].']',null);
            showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Activity&a=activity');
        }else{
            if($_FILES['activity_banner']['name']!=''){
                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$upload->file_name);
            }
    ...
    public function detail()
    {
		...
		$this->setDirquna('shop');
        $this->render('activity_detail.index');
    }

    /**
     * 活动细节列表XML
     */
    public function detail_xml()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            ...
            if ($val['activity_detail_state'] != 1) {
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . $GLOBALS['_PAGE_URL'] . '&c=Activity&a=deal&state=1&activity_detail_id=' .
                    $val['activity_detail_id'] .
                    '">通过</a></li>';
            }

            if ($val['activity_detail_state'] != 2) {
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . $GLOBALS['_PAGE_URL'] . '&c=Activity&a=deal&state=2&activity_detail_id=' .
                    $val['activity_detail_id'] .
                    '">拒绝</a></li>';
            }
            ...
            if ($val['activity_detail_state'] != 1) {
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' .
                    $GLOBALS['_PAGE_URL'] . '&c=Activity&a=del_detail&activity_detail_id=' .
                    $val['activity_detail_id'] .
                    '">删除</a></li>';
            }
    ...
    public function deal()
    {
        ...
        if($activity_detail->updates(array('activity_detail_state'=>intval($_GET['state'])),$id)){
            $this->log(L('spd_edit,activity_index').'[ID:'.$id.']',null);
    ...
    public function del_detail()
    {
        ...
        if($activity_detail->delList($condition_arr)){
            $this->log(L('spd_del,activity_index_content').'[ID:'.$id.']',null);
