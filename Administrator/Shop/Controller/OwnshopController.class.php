...
class OwnshopController extends SystemController {
    ...
    public function add()
    {
        if (chksubmit())
        {
            ...
            showMessage('操作成功', urlAdminShop('ownshop', 'lists'));
            return;
        }
$this->setDirquna('shop');
        $this->render('ownshop.add');
    }
    ...
    public function bind_class()
    {
        ...
        $this->assign('showpage',$model_store_bind_class->showpage());
$this->setDirquna('shop');
        $this->render('ownshop.bind_class');
    }
    ...
    public function get_xml() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $operation = "<a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Ownshop&a=edit&id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            if ($value['bind_all_gc'] != 1 || empty($stores_with_goods[$value['store_id']])) {
                ...
                if ($value['bind_all_gc'] != 1) {
                    $operation .= "<li><a href='{$GLOBALS['_PAGE_URL']}&c=Ownshop&a=bind_class&id=" . $value['store_id'] . "'>编辑经营类目</a></li>";
                }
