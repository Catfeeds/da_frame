...
class ComplainController extends SystemController {
    ...
    public function __construct() {
        parent::__construct();
				    ...
				    $this->links = array(
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_new_list','text'=>'新投诉'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_appeal_list','text'=>'待申诉'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_talk_list','text'=>'对话中'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_handle_list','text'=>'待仲裁'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_finish_list','text'=>'已关闭'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_subject_list','text'=>'主题设置'),
            array('url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_setting','text'=>'时效设置')
    );
		
        Language::read('complain', 'Home');
        if ($_GET['a'] == 'index') $_GET['a'] = 'complain_new_list';
        $this->assign('top_link',$this->sublink($this->links,$_GET['a']));
    ...
    private function get_complain_list($complain_state,$op) {
        ...
        $this->render('complain.list') ;
    }

    /*
     * 获取投诉列表
    */
    public function get_new_xml() {
        $page = new Page();
        ...
        foreach ($complain_list as $k => $complain_info) {
            $list = array();
            if ($_GET['state'] == 99) {
                $list['operation'] = "<a class=\"btn orange\" href=\"{$GLOBALS['_PAGE_URL']}&c=Complain&a=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            } else {
                $list['operation'] = "<a class=\"btn orange\" href=\"{$GLOBALS['_PAGE_URL']}&c=Complain&a=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            }
    ...
    public function complain_verify() {
        ...
        if(intval($complain_info['complain_state']) === self::STATE_NEW) {
            ...
            if($model_complain->updateComplain($update_array,$where_array)) {
                ...
                showMessage(Language::get('complain_verify_success'),$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_new_list');
            } else {
                showMessage(Language::get('complain_verify_fail'),$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_new_list');
            }
        } else {
                    ...
                    if (!empty($refund_goods) && is_array($refund_goods)) {
                        ...
                        foreach ($refund_goods as $key => $value) {
                            ...
                            if (!empty($checked_goods) && array_key_exists($goods_id,$checked_goods)) {//验证提交的商品属于订单
                                ...
                                $refund_array['refund_amount'] = daPriceFormat($value['goods_refund']);
    ...
    public function complain_subject_list() {
		...
		$this->setDirquna('shop');
        $this->render('complain_subject.list') ;
    }
    ...
    public function complain_subject_add() {
		...
		$this->setDirquna('shop');
        $this->render('complain_subject.add') ;
    }
    ...
    public function complain_subject_save() {
        ...
        if ($error != '') {
            ...
            showMessage(Language::get('complain_subject_add_success'),$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_subject_list');
        }
    }
    ...
    public function complain_subject_drop() {
        ...
        if(empty($complain_subject_id)) {
            showMessage(Language::get('param_error'), $GLOBALS['_PAGE_URL'] . '&c=Complain');
        }
        $model_complain_subject= Model('complain_subject');
        $update_array = array();
        ...
        if($model_complain_subject->updateComplainSubject($update_array,$where_array)) {
            ...
            showMessage(Language::get('complain_subject_delete_success'),$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_subject_list');
        } else {
            showMessage(Language::get('complain_subject_delete_fail'),$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_subject_list');
        }
    }
    ...
    private function get_complain_state_link($complain_state) {
        switch(intval($complain_state)) {
            case self::STATE_NEW:
                return $GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_new_list';
                break;
            case self::STATE_APPEAL:
                return $GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_appeal_list';
                break;
            case self::STATE_TALK:
                return $GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_talk_list';
                break;
            case self::STATE_HANDLE:
                return $GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_handle_list';
                break;
            case self::STATE_FINISH:
                return $GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_finish_list';
                break;
            default:
                showMessage(Language::get('param_error'));
        }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'complain_new_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_new_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_new_list'),
            'complain_appeal_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_appeal_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_appeal_list'),
            'complain_talk_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_talk_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_talk_list'),
            'complain_handle_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_handle_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_handle_list'),
            'complain_finish_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_finish_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_finish_list'),
            'complain_subject_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_subject_list'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_subject_list'),
            'complain_subject_add'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_subject_add'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_subject_add'),
            'complain_setting'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_setting'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Complain&a=complain_setting'),
