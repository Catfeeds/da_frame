...
class PointprodController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('pointprod,pointorder', 'Home');
    }
     ...
     * 积分礼品列表
     */
    public function pointprod()
    {
		$this->setDirquna('shop');
        $this->render('pointprod.list');
    }

    /**
     * 积分礼品列表
     */
    public function pointprod_xml()
    {
        $condition = array();
...
EOB;
    ...
    public function prod_add(){
        ...
        if (chksubmit()){
            ...
            if($state){
                ...
                showDialog(L('admin_pointprod_add_success'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointprod','succ');
            }
        }
        //模型实例化
        $where = array();
    ...
    public function prod_edit(){
        ...
        if (!$pg_id){
            showDialog(L('admin_pointprod_parameter_error'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointprod','error');
        }
        $model_pointprod = Model('pointprod');
        //查询礼品记录是否存在
        $prod_info = $model_pointprod->getPointProdInfo(array('pgoods_id'=>$pg_id));
        if (!$prod_info){
            showDialog(L('admin_pointprod_record_error'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointprod','error');
        }
        if (chksubmit()){
            //验证表单
            $obj_validate = new Validate();
            ...
            if($state){
                ...
                $this->log(L('spd_edit,admin_pointprodp').'['.$_POST['goodsname'].']');
                showDialog(L('admin_pointprod_edit_success'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointprod','succ');
            }
        }else {
            $where = array();
            $where['upload_type'] = '6';
    ...
    public function prod_drop(){
        ...
        if($result) {
            $this->log(L('spd_del,admin_pointprodp').'[ID:'.$pg_id.']');
            $this->jsonOutput();
        } else {
    ...
    public function prod_dropall()
    {
        ...
        if($result) {
            $this->log(L('spd_del,admin_pointprodp').'[ID:'.implode(',',$pg_id).']');
            $this->jsonOutput();
        } else {
            $this->jsonOutput('操作失败');
        }
...
$this->setDirquna('shop');
        $this->render('pointorder.list');
    }

    /**
     * 积分兑换列表XML
     */
    public function pointorder_list_xml()
    {
        $condition = array();
    ...
    public function order_drop(){
        ...
        if ($data['state']){
            showDialog(L('admin_pointorder_del_success'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','succ');
        } else {
            showDialog($data['msg'],$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','error');
        }
    }
    ...
    public function order_cancel(){
        ...
        if ($data['state']){
            showDialog(L('admin_pointorder_cancel_success'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','succ');
        }else {
            showDialog($data['msg'],$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','error');
        }
    }
    ...
    public function order_ship(){
        ...
        if ($order_id <= 0){
            showDialog(L('admin_pointorder_parameter_error'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','error');
        }
        $model_pointorder = Model('pointorder');
        //获取订单状态
        ...
        if (!$order_info){
            showDialog(L('admin_pointorderd_record_error'),$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','error');
        }
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            $error = $obj_validate->validate();
            if ($error != ''){
                showDialog(L('error').$error,$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','error');
            }
            //发货
            $data = $model_pointorder->shippingPointOrder($order_id, $_POST, $order_info);
            if ($data['state']){
                showDialog('发货修改成功',$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','succ');
            }else {
                showDialog($data['msg'],$GLOBALS['_PAGE_URL'] . '&c=Pointprod&a=pointorder_list','error');
            }
        } else {
