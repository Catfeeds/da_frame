...
class GoodsClassController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
        Language::read('goods_class', 'Home');
		
		    $this->links = array(
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class','lang'=>'spd_manage'),
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class_import','lang'=>'goods_class_index_import'),
        array('url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag','lang'=>'goods_class_index_tag')
    );
    }
    ...
    public function goods_class(){
        $this->assign('show_type', $this->show_type);
        ...
        if ($gc_id > 0){
            ...
            $this->render('goods_class.child_list');
        } else {
            $this->assign('class_list',$class_list);
            $this->assign('top_link',$this->sublink($this->links,'goods_class'));
            $this->setDirquna('shop');
            $this->render('goods_class.index');
        }
    }
    ...
    public function goods_class_add(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
                            'msg'=>$lang['goods_class_add_again'],
                        ),
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class',
                            'msg'=>$lang['goods_class_add_back_to_list'],
                        )
                    );
                    $this->log(L('spd_add,goods_class_index_class').'['.$_POST['gc_name'].']',1);
                    showMessage($lang['spd_common_save_succ'],$url);
                }else {
                    $this->log(L('spd_add,goods_class_index_class').'['.$_POST['gc_name'].']',0);
        ...
        if(is_array($type_list) && !empty($type_list)){
            foreach($type_list as $k=>$val){
                ...
                $t_list[$val['class_id']]['name'] = $val['class_name']==''?L('spd_default'):$val['class_name'];
            }
        }
        ksort($t_list);
        ...
        $this->assign('gc_parent_id',$_GET['gc_parent_id']);
        $this->assign('parent_list',$parent_list);
        $this->assign('top_link',$this->sublink($this->links,'goods_class_add'));
						
		$this->setDirquna('shop');
        $this->render('goods_class.add');
    }
    ...
    public function goods_class_edit(){
        ...
        if (chksubmit()){
             ...
             * 上传品牌大图
             */
            if ($_FILES['gc_pic']['name'] != ''){
            	$upload = new UploadFile();
            	$upload->set('default_dir',ATTACH_GOODS_CLASS . DS . "gc_pic");
            ...
            if (!empty($_POST['gc_pic'])){
            	$update_array['gc_pic'] = trim($_POST['gc_pic']);
            }
            
            $result = $model_class->editGoodsClass($update_array, $where);
            if (!$result){
                $this->log(L('spd_edit,goods_class_index_class').'['.$_POST['gc_name'].']',0);
                showMessage($lang['goods_class_batch_edit_fail']);
            }

            if (!empty($_POST['gc_pic']) && !empty($class_array['gc_pic'])){
            	@unlink(BASE_UPLOAD_PATH.DS.ATTACH_GOODS_CLASS.DS.'/gc_pic'.DS.$class_array['gc_pic']);
            }
            ...
            $url = array(
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class_edit&gc_id='.intval($_POST['gc_id']),
            ...
            $this->log(L('spd_edit,goods_class_index_class').'['.$_POST['gc_name'].']',1);
        ...
        $this->links[] = array('url'=>$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class_edit','lang'=>'spd_edit');
        $this->assign('top_link',$this->sublink($this->links,'goods_class_edit'));
						
		$this->setDirquna('shop');
        $this->render('goods_class.edit');
    }
    ...
    public function goods_class_import(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
                ...
                // 父ID
                $parent_id_1 = 0;
                $model_class->delGoodsClass(array("1"=> "1"));
                while (!feof($fp)) {
                ...
                showMessage($lang['spd_common_op_succ'],$GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=goods_class');
            }else {
                $this->log(L('goods_class_index_import,goods_class_index_class'),0);
                showMessage($lang['goods_class_import_csv_null']);
            }
        }
        $this->assign('top_link',$this->sublink($this->links,'goods_class_import'));
						
		$this->setDirquna('shop');
        $this->render('goods_class.import');
    }
    ...
    public function goods_class_del(){
        if ($_GET['id'] != ''){
            ...
            $this->log(L('spd_delete,goods_class_index_class') . '[ID:' . $_GET['id'] . ']',1);
     ...
     */
    public function tag(){
        $this->assign('top_link',$this->sublink($this->links,'tag'));
						
		$this->setDirquna('shop');
        $this->render('goods_class_tag.index');
    }
    ...
    public function tag_reset(){
        ...
        if(!$return){
            showMessage($lang['goods_class_reset_tag_fail'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
        }
        ...
        if(is_array($goods_class) and !empty($goods_class)) {
            ...
            if($return){
                $this->log(L('spd_reset').'tag',1);
                showMessage($lang['spd_common_op_succ'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
            }else{
                $this->log(L('spd_reset').'tag',0);
                showMessage($lang['spd_common_op_fail'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
            }
        }else{
            $this->log(L('spd_reset').'tag',0);
            showMessage($lang['goods_class_reset_tag_fail_no_class'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
        }
    }
    ...
    public function tag_update(){
        ...
        if(is_array($tag_list) && !empty($tag_list)){
            foreach ($tag_list as $val){
                ...
                if(!$return){
                    $this->log(L('spd_update').'tag',0);
                    showMessage($lang['spd_common_op_fail'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
                }
            }
            $this->log(L('spd_update').'tag',1);
            showMessage($lang['spd_common_op_succ'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
        }else{
            $this->log(L('spd_update').'tag',0);
            showMessage($lang['goods_class_update_tag_fail_no_class'], $GLOBALS['_PAGE_URL'] . '&c=GoodsClass&a=tag');
        }
    ...
    public function tag_edit() {
        ...
        $this->assign('tag_info', $tag_list[0]);
						
		$this->setDirquna('shop');
        $this->render('goods_class_tag.edit', 'null_layout');
    }
    ...
    public function nav_edit() {
        ...
        if(is_array($brand_list) && !empty($brand_list)){
            foreach($brand_list as $k=>$val){
                ...
                $b_list[$val['class_id']]['name'] = $val['brand_class']==''?L('spd_default'):$val['brand_class'];
            }
        }
        ksort($b_list);
		...
		$this->setDirquna('shop');
        $this->render('goods_class.nav_edit');
    }
