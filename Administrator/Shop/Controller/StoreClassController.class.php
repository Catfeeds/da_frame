...
class StoreClassController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('store_class', 'Home');
    }
    ...
    public function store_class(){
        ...
        if (chksubmit()){
            if (!empty($_POST['check_sc_id']) && is_array($_POST['check_sc_id']) ){
                ...
                if ($result) {
                    $this->log(L('spd_del,store_class').'[ID:'.implode(',',$_POST['check_sc_id']).']',1);
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                    array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreClass&a=store_class_add',
                    'msg'=>$lang['continue_add_store_class'],
                    ),
                    array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=StoreClass&a=store_class',
                    'msg'=>$lang['back_store_class_list'],
                    )
                    );
                    $this->log(L('spd_add,store_class').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['spd_common_save_succ'],$url,'html','succ',1,5000);
                }else {
    ...
    public function store_class_edit(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $this->log(L('spd_edit,store_class').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['spd_common_save_succ'],$GLOBALS['_PAGE_URL'] . '&c=StoreClass&a=store_class');
                }else {
                    showMessage($lang['spd_common_save_fail']);
                }
            }
    ...
    public function store_class_del(){
        ...
        if (intval($_GET['sc_id']) > 0){
            ...
            if ($result) {
                 $this->log(L('spd_del,store_class').'[ID:'.$_GET['sc_id'].']',1);
                 showMessage($lang['spd_common_del_succ'],getReferer());
            }
        }
        showMessage($lang['spd_common_del_fail'],$GLOBALS['_PAGE_URL'] . '&c=StoreClass&a=store_class');
    }
