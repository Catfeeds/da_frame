...
class SpecController extends SystemController {
    const EXPORT_SIZE = 5000;
    public function __construct(){
        parent::__construct();
        Language::read('spec', 'Home');
    }
    ...
    public function get_xml() {
        ...
        foreach ((array)$spec_list as $value) {
            ...
            $operation .= "<a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=Spec&a=spec_edit&sp_id=".$value['sp_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if($return) {
                    $url = array(
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Spec&a=spec_add',
                            'msg'=>$lang['spec_index_continue_to_dd']
                        ),
                        array(
                            'url'=>$GLOBALS['_PAGE_URL'] . '&c=Spec&a=spec',
                            'msg'=>$lang['spec_index_return_type_list']
                        )
                    );
                    $this->log(L('spd_add,spec_index_spec_name').'['.$_POST['s_name'].']',1);
                    showMessage($lang['spd_common_save_succ'], $url);
                }else {
                    $this->log(L('spd_add,spec_index_spec_name').'['.$_POST['s_name'].']',0);
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($return) {
                    ...
                    $this->log(L('spd_edit,spec_index_spec_name').'['.$_POST['s_name'].']',1);
                    showMessage($lang['spd_common_save_succ'], $url);
                } else {
                    $this->log(L('spd_edit,spec_index_spec_name').'['.$_POST['s_name'].']',0);
                    showMessage($lang['spd_common_save_fail']);
                }
            }
        }
    ...
    public function spec_del(){
        ...
        if(is_array($spec_list) && !empty($spec_list)){
            ...
            if(!$return){
                exit(json_encode(array('state'=>false,'msg'=>L('spd_common_save_fail'))));
            }
            ...
            $this->log(L('spd_delete,spec_index_spec_name').'[ID:'.$id.']',1);
            exit(json_encode(array('state'=>true,'msg'=>L('spd_common_del_succ'))));
        }else{
            $this->log(L('spd_delete,spec_index_spec_name').'[ID:'.$id.']',0);
            exit(json_encode(array('state'=>false,'msg'=>L('param_error'))));
        }
    }
}
