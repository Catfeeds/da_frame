...
class RechargecardController extends SystemController {
    /**
    ...
    public function index()
    {
        ...
        $this->render('rechargecard.index');
    }
...
<a class="btn green confirm-del-on-click" href="javascript:;" data-href="{$GLOBALS['_PAGE_URL']}&c=Rechargecard&a=del_card&id={$val['id']}"><i class="fa fa-trash"></i>删除</a>
EOB;
    ...
    public function add_card()
    {
        if (!chksubmit()) {
			$this->setDirquna('shop');
            $this->render('rechargecard.add_card');
            return;
        }
    ...
    public function export_step1()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                $this->assign('murl', $GLOBALS['_PAGE_URL'] . '&c=Rechargecard&a=index');
				$this->setDirquna('shop');
    ...
    {
		$this->setDirquna('shop');
        $this->render('rechargecard.log_list');
    }
     ...
     * 导出使用日志
     */
    public function log_export_step1()
    {
        $model = Model('rcb_log');
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                $this->assign('list',$array);
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Rechargecard&a=log_list');
				$this->setDirquna('shop');
                $this->render('export.excel');
                return;
