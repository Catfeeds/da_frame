...
class StatTradeController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		   ...
		   $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatTrade&a=income','lang'=>'stat_sale_income'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatTrade&a=predeposit','lang'=>'stat_predeposit'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatTrade&a=sale','lang'=>'stat_sale')
    );
        ...
        //处理搜索时间
        if (in_array($_REQUEST['a'],array('sale_trend','get_sale_xml','get_plat_sale'))){
    ...
    public function income(){
        ...
        if($_GET['exporttype'] == 'excel'){
            ...
            $this->assign('top_link',$this->sublink($this->links, 'income'));
			$this->setDirquna('shop');
            $this->render('stat.income');
        }
    }
     ...
     * 输出平台总数据
     */
    public function get_plat_income(){
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
            $where['ob_store_name'] = array('like',"%{$_GET['store_name']}%");
		...
		echo '<dl class="row"><dd class="opt"><ul class="spd-row">';
     ...
     * 输出销售收入统计XML数据
     */
    public function get_income_xml(){
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
            $where['ob_store_name'] = array('like',"%{$_GET['store_name']}%");
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                $out_array = array();
                $out_array['operation'] = '';
                //$out_array['operation'] = '<a class="btn green" href="' . $GLOBALS['_PAGE_URL'] . '&c=StatTrade&a=sale&search_type=month&searchmonth_month='.$_GET['search_month'].
    ...
    public function predeposit(){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $this->assign('top_link',$this->sublink($this->links, 'predeposit'));
			$this->setDirquna('shop');
            $this->render('stat.predeposit');
        }
    }
     ...
     * 预存款统计
     */
    public function get_predeposit_highcharts(){
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
        }else{
     ...
     * 获取平台总预存款统计
     */
    public function get_plat_predeposit(){
        $model = Model('stat');
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
     ...
     * 输出销售收入统计XML数据
     */
    public function get_predeposit_xml(){
        $model = Model('stat');
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
    ...
    public function sale(){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $this->assign('top_link',$this->sublink($this->links, 'sale'));
			$this->setDirquna('shop');
            $this->render('stat.sale');
        }
    }
