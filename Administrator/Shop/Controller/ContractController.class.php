...
class ContractController extends SystemController {
    ...
    public function __construct(){
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能',$GLOBALS['_PAGE_URL'] . '&c=Operating','error');
        }
    ...
    public function citemlist()
    {
        ...
        $this->render('contract.itemlist');
    }
    /**
     * 保障服务项目列表XML
     */
    public function citemlist_xml()
    {
        $model_contract = Model('contract');
     ...
     * 保障服务项目编辑
     */
    public function citemedit(){
        $itemid_id = intval($_GET['itemid']);
        if ($itemid_id <= 0){
            $itemid_id = intval($_POST['itemid_id']);
        }
        if ($itemid_id <= 0){
            showDialog(L('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=citemlist');
        }
        $model_contract = Model('contract');
        //查询模板信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=citemlist','succ');
            } else {
                showDialog(L('spd_common_save_fail'),'','error');
            }
        }else{
            $this->assign('itemstate_arr',$this->itemstate_arr);
            $this->assign('item_info',$item_info);
							
		$this->setDirquna('shop');
            $this->render('contract.citemedit');
        }
    }
    ...
    public function contractlist(){
        ...
        $this->assign('contract_joinstate_arr',$this->contract_joinstate_arr);
        $this->assign('contract_closestate_arr',$this->contract_closestate_arr);
        $this->show_menu('contractlist');
						
		$this->setDirquna('shop');
        $this->render('contract.list');
    }

    /**
     * 店铺保障服务列表XML
     */
    public function contractlist_xml()
    {
        $where = array();
    ...
    public function contractedit(){
        ...
        if ($ct_id <= 0){
            showDialog(L('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=contractlist');
        }
        $model_contract = Model('contract');
        //查询店铺保障服务信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if ($c_info['ct_closestate_key'] == $_POST['c_state']) {
                showDialog(L('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=contractlist','succ');
            }
            try {
                ...
                if (!$result) {
                    throw new Exception(L('spd_common_save_fail'));
                }
                //新增保障项目日志
            ...
            $this->assign('item_info',$item_info);
            $this->assign('contractclosestate_arr',$this->contract_closestate_arr);
							
		$this->setDirquna('shop');
            $this->render('contract.contractedit');
        }
    }
    ...
    public function applylist(){
        ...
        $item_list = Model('contract')->getContractItemByCache('all');
        $this->assign('item_list',$item_list);
        $this->assign('join_auditstate_arr',$this->join_auditstate_arr);
        $this->show_menu('applylist');
						
		$this->setDirquna('shop');
        $this->render('contract.applylist');
    }

    /**
     * 服务加入申请列表XML
     */
    public function applylist_xml()
    {
        $where = array();
        $sort = '';
        if ($_REQUEST['advanced']) {
    ...
    public function applyedit(){
        ...
        if ($aid <= 0){
            showDialog(L('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=applylist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['apply_state'] == $apply_info['cta_auditstate_key']) {
                showDialog('不能重复更改申请状态',$GLOBALS['_PAGE_URL'] . '&c=Contract&a=applylist');
            }
            if (!in_array($_POST['apply_state'],array_keys($curr_applystatearr))) {
                showDialog('编辑失败',$GLOBALS['_PAGE_URL'] . '&c=Contract&a=applyedit&aid='.$apply_info['cta_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=applylist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
            ...
            $this->assign('item_info',$item_info);
            $this->assign('curr_applystatearr',$curr_applystatearr);
							
		$this->setDirquna('shop');
            $this->render('contract.applyedit');
        }
    }
    ...
    public function applyinfo(){
		...
		$this->setDirquna('shop');
        $this->render('contract.applyinfo');
    }
    /*
     * 保障服务申请编辑
     */
    public function costedit(){
        ...
        if(chksubmit()){
            ...
            try {
                ...
                $price_msg .= (daPriceFormat(abs($price)).L('currency_zh'));
                //新增保障项目日志
    ...
    public function quitlist(){
        ...
        $item_list = Model('contract')->getContractItemByCache('all');
        $this->assign('item_list',$item_list);
        $this->assign('quit_auditstate_arr',$this->quit_auditstate_arr);
        $this->show_menu('quitlist');
						
		$this->setDirquna('shop');
        $this->render('contract.quitlist');
    }

    /**
     * 服务退出申请列表XML
     */
    public function quitlist_xml()
    {
        $where = array();
        $sort = '';
        if ($_REQUEST['advanced']) {
     ...
     * 保障服务申请编辑
     */
    public function quitedit(){
        $qid = intval($_GET['qid']);
        if ($qid <= 0){
            $qid = intval($_POST['qid']);
        }
        if ($qid <= 0){
            showDialog(L('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=quitlist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['quit_state'] == $quit_info['ctq_auditstate_key']) {
                showDialog('不能重复编辑申请状态',$GLOBALS['_PAGE_URL'] . '&c=Contract&a=quitlist');
            }
            if (!in_array($_POST['quit_state'],array_keys($this->quit_auditstate_arr))) {
                showDialog('编辑失败',$GLOBALS['_PAGE_URL'] . '&c=Contract&a=quitedit&qid='.$quit_info['ctq_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Contract&a=quitlist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
        }else{
            $this->assign('quit_info',$quit_info);
            $this->assign('quit_auditstate_arr',$this->quit_auditstate_arr);
							
		$this->setDirquna('shop');
            $this->render('contract.quitedit');
        }
    }
    ...
    public function contractinfo(){
		...
		$this->setDirquna('shop');
        $this->render('contract.contractinfo');
    }
    /**
     * 保障服务日志列表XML
     */
    public function contractlog_xml()
    {
    ...
    public function costlog(){
		...
		$this->setDirquna('shop');
        $this->render('contract.costlog');
    }
    /**
     * 保证金列表XML
     */
    public function costlist_xml()
    {
        $item_id = intval($_GET['item_id']);
        $store_id = intval($_GET['store_id']);
        if ($item_id <= 0 || $store_id <= 0) {
    ...
    private function show_menu($menu_key=''){
        $menu_array = array(
            1=>array('menu_key'=>'applylist','menu_name'=>'服务加入申请', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Contract&a=applylist'),
            2=>array('menu_key'=>'quitlist','menu_name'=>'服务退出申请', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Contract&a=quitlist'),
            3=>array('menu_key'=>'contractlist','menu_name'=>'店铺保障服务', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Contract&a=contractlist'),
            4=>array('menu_key'=>'citemlist','menu_name'=>'保障服务管理', 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Contract&a=citemlist')
        );
