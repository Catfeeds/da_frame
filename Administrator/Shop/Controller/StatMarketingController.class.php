...
class StatMarketingController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		    ...
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMarketing&a=promotion','lang'=>'stat_promotion'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMarketing&a=group','lang'=>'stat_group'),
    );
        ...
        //处理搜索时间
        if (!isset($this->search_arr['a']) || in_array($this->search_arr['a'],array('promotion','group','get_groupgoods_xml','groupgoods','get_grouplist_xml'))){
    ...
    public function promotion(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'promotion'));
		$this->setDirquna('shop');
        $this->render('stat.marketing.promotion');
    }
    /**
     * 促销销售趋势分析
     */
    ...
    public function group(){
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $this->assign('searchtime',implode('|',$searchtime_arr));
        $this->assign('top_link',$this->sublink($this->links, 'group'));
		$this->setDirquna('shop');
        $this->render('stat.marketing.group');
    }
    /**
     * 输出抢购统计XML数据
     */
    public function get_grouplist_xml(){
        ...
        if (!empty($grouplist) && is_array($grouplist)){
            ...
            foreach ($grouplist as $k => $v){
                ...
                $out_array['goods_price'] = daPriceFormat($v['goods_price']);
                $out_array['groupbuy_price'] = daPriceFormat($v['groupbuy_price']);
     ...
     * 输出抢购商品统计XML数据
     */
    public function get_groupgoods_xml(){
        $model = Model('stat');
        $where = array();
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
