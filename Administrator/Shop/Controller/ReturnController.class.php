...
class ReturnController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		    ...
		    $this->links = array(
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Return&a=return_manage','text'=>'待处理'),
            array('url'=> $GLOBALS['_PAGE_URL'] . '&c=Return&a=return_all','text'=>'所有记录')
    );
		
        $model_refund = Model('refund_return');
        $model_refund->getRefundStateArray();
        if ($_GET['a'] == 'index') $_GET['a'] = 'return_manage';
    ...
    public function return_manage() {
        ...
        $this->render('return_manage.list');
    }
    ...
    public function get_manage_xml() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"{$GLOBALS['_PAGE_URL']}&c=Return&a=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = daPriceFormat($return_info['refund_amount']);
    ...
    public function get_all_xml() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"{$GLOBALS['_PAGE_URL']}&c=Return&a=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = daPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function edit() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Return&a=return_manage');
            } else {
                showMessage(Language::get('spd_common_save_fail'));
            }
        }
