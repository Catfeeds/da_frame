...
class StatGeneralController extends SystemController {

    public function __construct(){
        parent::__construct();
		
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatGeneral&a=general','lang'=>'stat_generalindex'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatGeneral&a=setting','lang'=>'stat_goodspricerange'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatGeneral&a=orderprange','lang'=>'stat_orderpricerange')
    );
    ...
    public function general(){
        ...
        $statnew_arr['orderamount'] = daPriceFormat(($t = $stat_order['orderamount'])?$t:(0));
        $statnew_arr['ordermembernum'] = ($t = $stat_order['ordermembernum'])?$t:0;
        $statnew_arr['orderavg'] = daPriceFormat(($t = $stat_order['orderavg'])?$t:0);
        unset($stat_order);
        ...
        $statnew_arr['priceavg'] = daPriceFormat(($t = $stat_ordergoods['priceavg'])?$t:0);
        ...
        $this->render('stat.general.index');
    }
    /**
     * 统计设置
     */
    public function setting(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('spd_edit,stat_setting'),1);
                showMessage(L('spd_common_save_succ'));
            }else {
                $this->log(L('spd_edit,stat_setting'),0);
        ...
        $this->assign('top_link',$this->sublink($this->links, 'setting'));
		$this->setDirquna('shop');
        $this->render('stat.setting');
    }
    /**
     * 统计设置
     */
    public function orderprange(){
        ...
        $this->assign('list_setting',$list_setting);
        $this->assign('top_link',$this->sublink($this->links, 'orderprange'));
		$this->setDirquna('shop');
        $this->render('stat.setting.orderprange');
    }
}
