...
class PromotionBookController extends SystemController {

    public function __construct(){
        parent::__construct();
		
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionBook&a=index','text'=>'定金预售'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionBook&a=presell_goods_list', 'text'=>'全款预售'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionBook&a=book_quota_list','text'=>'套餐列表'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=PromotionBook&a=book_setting','text'=>'设置')
    );
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionBook&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function index() {
        ...
        $this->book_goods_list();
    }

    /**
     * 活动商品列表
     */
    public function book_goods_list() {
        $this->assign('top_link',$this->sublink($this->links,'index'));
		$this->setDirquna('shop');
        $this->render('promotion_book.book_goods');
    }

    /**
     * 活动商品管理XML
     */
    public function get_book_goods_xml() {
        ...
        foreach ($goods_list as $value) {
            ...
            $param['book_total_payment'] = daPriceFormat($value['book_down_payment'] + $value['book_final_payment']);
     ...
     * 删除订金预售商品活动
     */
    public function del_book_goods() {
        $id = intval($_GET['id']);
        if ($id > 0) {
    ...
    public function presell_goods_list() {
        $this->assign('top_link',$this->sublink($this->links,'presell_goods_list'));
		$this->setDirquna('shop');
        $this->render('promotion_book.presell_goods');
    }

    /**
     * 活动商品管理XML
     */
    public function get_presell_goods_xml() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
     ...
     * 删除订金预售商品活动
     */
    public function del_presell_goods() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function book_quota_list() {
        $this->assign('top_link',$this->sublink($this->links,'book_quota_list'));
		$this->setDirquna('shop');
        $this->render('promotion_book.quota');
    }

    /**
     * 套餐列表XML
     */
    public function get_quota_xml() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
    ...
    public function book_setting() {
        ...
        $this->assign('top_link',$this->sublink($this->links,'book_setting'));
		$this->setDirquna('shop');
        $this->render('promotion_book.setting');
    }
}
