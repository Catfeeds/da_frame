...
class StatMemberController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
		    ...
		    $this->links = array(
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMember&a=newmember','lang'=>'stat_newmember'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMember&a=analyze','lang'=>'stat_memberanalyze'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMember&a=scale','lang'=>'stat_scaleanalyze'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMember&a=area','lang'=>'stat_areaanalyze'),
        array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMember&a=buying','lang'=>'stat_buying'),
    );
        ...
        //处理搜索时间
        if (!isset($this->search_arr['a']) || in_array($_REQUEST['a'],array('newmember','analyze','scale','area','buying','get_scale_xml'))){
    ...
    public function newmember(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            $this->assign('actionurl',$GLOBALS['_PAGE_URL'] . '&c=StatMember&a=newmember&search_type=day&search_time='.date('Y-m-d',$this->search_arr['day']['search_time']));
        }
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            $this->assign('actionurl',$GLOBALS['_PAGE_URL'] . '&c=StatMember&a=newmember&search_type=week&searchweek_year='.$this->search_arr['week']['current_year'].'&searchweek_month='.$this->search_arr['week']['current_month'].'&searchweek_week='.$this->search_arr['week']['current_week']);
        }
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            $this->assign('actionurl',$GLOBALS['_PAGE_URL'] . '&c=StatMember&a=newmember&search_type=month&searchmonth_year='.$this->search_arr['month']['current_year'].'&searchmonth_month='.$this->search_arr['month']['current_month']);
        }
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $this->assign('top_link',$this->sublink($this->links, 'newmember'));
			$this->setDirquna('shop');
            $this->render('stat.newmember');
        }
    }
    /**
     * 会员分析
    ...
    public function analyze(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'analyze'));
		$this->setDirquna('shop');
        $this->render('stat.memberanalyze');
    }
     ...
     * 输出会员分析详细列表XML数据
     */
    public function get_analyzeinfo_xml(){
        $model = Model('stat');
        $where = array();
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
        foreach ((array)$searchtime_arr_tmp as $k => $v){
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                if ($stat_field == 'orderamount') {
                    $out_array['orderamount'] = daPriceFormat($out_array['orderamount']);
                }
    ...
    public function showmember(){
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            //data
            foreach ($member_list as $k => $v){
                $excel_data[$k+1][] = array('data'=>$v['member_name'].'('.L('member_index_true_name,spd_colon').$v['member_truename'].')');
                ...
                $excel_data[$k+1][] = array('data'=>L('member_index_available,spd_colon').$v['available_predeposit'].L('currency_zh').'('.L('member_index_frozen,spd_colon').$v['freeze_predeposit'].L('currency_zh').')');
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        $this->links[] = array('url'=> $GLOBALS['_PAGE_URL'] . '&c=StatMember&a=showmember','lang'=>'stat_memberlist');
        $this->assign('top_link',$this->sublink($this->links, 'showmember'));
		$this->setDirquna('shop');
        $this->render('stat.info.memberlist');
    }

    /**
     * 输出订单统计XML数据
     */
    public function get_member_xml(){
        $model = Model('stat');
        $where = array();
        if (!empty($_GET['t'])){
            $searchtime_arr_tmp = explode('|',$_GET['t']);
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $out_array['available_predeposit'] = daPriceFormat($v['available_predeposit']);
                $out_array['freeze_predeposit'] = daPriceFormat($v['freeze_predeposit']);
    ...
    public function scale(){
		...
		$this->setDirquna('shop');
        $this->render('stat.memberscale');
    }

    /**
     * 输出会员规模XML数据
     */
    public function get_scale_xml(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
    ...
    public function area(){
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $this->assign('searchtime',implode('|',$searchtime_arr));
        $this->assign('top_link',$this->sublink($this->links, 'area'));
		$this->setDirquna('shop');
        $this->render('stat.memberarea');
    }
    /**
     * 区域分析之详细列表
     */
     ...
     * 输出区域分析详细XML数据
     */
    public function get_arealist_xml(){
        $model = Model('stat');
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
    ...
    public function area_map(){
        ...
        $this->assign('stat_json',$stat_json);
		$this->setDirquna('shop');
        $this->render('stat.map','null_layout');
    }
    /**
     * 购买分析
     */
    public function buying(){
        ...
        $this->assign('top_link',$this->sublink($this->links, 'buying'));
		$this->setDirquna('shop');
        $this->render('stat.buying');
    }
}
