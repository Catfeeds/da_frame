...
class StoreGradeController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('store_grade,store', 'Home');
    }
    ...
    public function store_grade(){
            ...
            if (!empty($_POST['check_sg_id'])){
                if (is_array($_POST['check_sg_id'])){
                    ...
                    foreach ($_POST['check_sg_id'] as $k => $v){
                        ...
                        //判断是否默认等级，默认等级不能删除
                        if ($v == 1){
                            //showMessage('默认等级不能删除 ',$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
                            showMessage($lang['default_store_grade_no_del'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
                        }
                ...
                dkcache('store_grade');
                $this->log(L('spd_del,store_grade').'[ID:'.implode(',',$_POST['check_sg_id']).']',1);
                showMessage($lang['spd_common_del_succ']);
            }else {
                showMessage($lang['spd_common_del_fail']);
            }
        }
        ...
        $this->assign('like_sg_name',trim($_POST['like_sg_name']));
        $this->assign('grade_list',$grade_list);
		$this->setDirquna('shop');
        $this->render('store_grade.index');
    }
    ...
    public function store_grade_add(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    dkcache('store_grade');
                    $this->log(L('spd_add,store_grade').'['.$_POST['sg_name'].']',1);
                    showMessage($lang['spd_common_save_succ'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
                }else {
		...
		$this->setDirquna('shop');
        $this->render('store_grade.add');
    }

    /**
     * 等级编辑
     */
    public function store_grade_edit(){
        $lang   = Language::getLangContent();

        $model_grade = Model('store_grade');
        if (chksubmit()){
            if (!$_POST['sg_id']){
                showMessage($lang['grade_parameter_error'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
            }
            /**
             * 验证
             */
            ...
            if ($error != ''){
                ...
                if (!$this->checkGradeName(array('sg_name'=>trim($_POST['sg_name']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['now_store_grade_name_is_there'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                //验证级别是否存在
                if (!$this->checkGradeSort(array('sg_sort'=>trim($_POST['sg_sort']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['add_gradesortexist'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                $update_array = array();
                ...
                if ($result){
                    dkcache('store_grade');
                    $this->log(L('spd_edit,store_grade').'['.$_POST['sg_name'].']',1);
		...
		$this->setDirquna('shop');
        $this->render('store_grade.edit');
    }

    /**
     * 删除等级
     */
    public function store_grade_del(){
        /**
         * 读取语言包
         */
        $lang   = Language::getLangContent();
        ...
        if (intval($_GET['sg_id']) > 0){
            ...
            if (!$this->isable_delGrade(intval($_GET['sg_id']))){
                showMessage($lang['del_gradehavestore'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
            }
            /**
             * 删除分类
             */
            ...
            dkcache('store_grade');
            $this->log(L('spd_del,store_grade').'[ID:'.intval($_GET['sg_id']).']',1);
            showMessage($lang['spd_common_del_succ'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
        }else {
            showMessage($lang['spd_common_del_fail'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
        }
    }
     ...
     * 等级：设置可选模板
     */
    public function store_grade_templates(){
        /**
         * 读取语言包
         */
        $lang   = Language::getLangContent();
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('spd_edit,store_grade_tpl'),1);
                showMessage($lang['spd_common_save_succ'],$GLOBALS['_PAGE_URL'] . '&c=StoreGrade&a=store_grade');
            }else {
                showMessage($lang['spd_common_save_fail']);
            }
        ...
        //主题配置信息
        $style_data = array();
        $style_configurl = BASE_ROOT_PATH . '/Application/Shop/View/'.TPL_SHOP_NAME . DS . 'store'.DS.'style'.DS."styleconfig.php";
        ...
        $this->assign('grade_array',$grade_array);
		$this->setDirquna('shop');
        $this->render('store_grade_template.edit');
    }
    /**
     * ajax操作
     */
