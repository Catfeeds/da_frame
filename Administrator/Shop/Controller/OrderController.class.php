    ...
    public function get_xml(){
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            if ($order_info['if_system_receive_pay']) {
                ...
                $operation_detail .= "<li><a href=\"{$GLOBALS['_PAGE_URL']}&c=Order&a=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">{$op_name}</a></li>";
            }
            if ($operation_detail) {
			...
			if ($order_info['shipping_fee']) {
			    $list['order_amount'] .= '(含运费'.daPriceFormat($order_info['shipping_fee']).')';
			}
    ...
    public function export_step1(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $this->assign('murl',$GLOBALS['_PAGE_URL'] . '&c=Order&a=index');
