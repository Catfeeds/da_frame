...
class SnsMalbumController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('sns_malbum', 'Home');
    }
    ...
    public function setting(){
        ...
        $this->render('sns_malbum.setting');
    }
    ...
    public function class_list(){
		$this->setDirquna('shop');
        $this->render('sns_malbum.classlist');
    }
    ...
    public function get_xml() {
        ...
        foreach ($ac_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='{$GLOBALS['_PAGE_URL']}&c=SnsMalbum&a=pic_list&id=".$value['ac_id']."' class='url'><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['ac_cover'] = "<a href='javascript:void(0);' class='pic-thumb-tip' onMouseOut='toolTip()' onMouseOver='toolTip(\"<img src=".($value['ac_cover'] != '' ? UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$value['member_id'].DS.$value['ac_cover'] : ADMIN_STATIC_URL.'/images/member/default_image.png').">\")'><i class='fa fa-picture-o'></i></a>";
    ...
    public function pic_list(){
        ...
        if(chksubmit()){
            ...
            $this->log(L('spd_del,spd_member_album_manage').'[ID:'.implode(',',$_POST['id']).']',1);
		...
		$this->setDirquna('shop');
        $this->render('sns_malbum.piclist');
    }
