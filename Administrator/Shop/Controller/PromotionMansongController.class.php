...
class PromotionMansongController extends SystemController {

    public function __construct(){
        parent::__construct();

        //读取语言包
        Language::read('promotion_mansong', 'Home');

        //检查审核功能是否开启
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                array(
                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=PromotionMansong&promotion_allow=1',
    ...
    public function mansong_list()
    {
        ...
        $this->assign('mansong_state_array', $mansong_state_array);

        $this->show_menu('mansong_list');
		$this->setDirquna('shop');
        $this->render('promotion_mansong.list');
    }

    /**
     * 活动列表XML
     */
    public function mansong_list_xml()
    {
        $condition = array();
...
EOB;
    ...
    public function mansong_detail() {
		...
		$this->setDirquna('shop');
        $this->render('promotion_mansong.detail', 'null_layout');
    }
    ...
    public function mansong_quota()
    {
		...
		$this->setDirquna('shop');
        $this->render('promotion_mansong_quota.list');
    }

    /**
     * 套餐管理XML
     */
    public function mansong_quota_xml()
    {
        $condition = array();
    ...
    public function mansong_setting() {
		...
		$this->setDirquna('shop');
        $this->render('promotion_mansong.setting');
    }
    ...
    public function mansong_setting_save() {
        ...
        if ($result === true){
            $this->log(L('spd_config,spd_promotion_mansong,mansong_price'));
