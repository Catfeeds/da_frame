...
class VoucherController extends SystemController {
    ...
    public function __construct(){
        parent::__construct();
        Language::read('voucher', 'Home');
        if (C('voucher_allow') != 1 || C('points_isuse')!=1){
            showMessage(Language::get('admin_voucher_unavailable'),$GLOBALS['_PAGE_URL'] . '&c=Operation&a=point','html','succ',1,4000);
        }
    ...
    public function setting(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('admin_voucher_setting,spd_voucher_price_manage'));
    ...
    public function pricelist()
    {
		...
		$this->setDirquna('shop');
        $this->render('voucher.pricelist');
    }

    /**
     * 代金券面额列表XML
     */
    public function pricelist_xml()
    {
        $model = Model();
    ...
    public function priceadd(){
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('spd_add,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist');
                }else {
                    showMessage(Language::get('spd_common_save_fail'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=priceadd');
                }
            }
        }else {
            $this->show_menu('voucher','priceadd');
			$this->setDirquna('shop');
            $this->render('voucher.priceadd') ;
        }
    }
    ...
    public function priceedit(){
        ...
        if ($id <= 0){
            showMessage(Language::get('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist');
        }
        $model = Model();
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('spd_edit,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist');
                }else {
                    showMessage(Language::get('spd_common_save_fail'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist');
                }
            }
        }else {
    ...
    public function pricedrop(){
        ...
        if ($rs){
            $this->log(L('spd_del,admin_voucher_priceadd').'[ID:'.$voucher_price_id.']');
            showMessage(Language::get('spd_common_del_succ'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist');
        }else{
            showMessage(Language::get('spd_common_del_fail'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist');
        }
    }
    ...
    public function quotalist(){
		...
		$this->setDirquna('shop');
        $this->render('voucher.quotalist');
    }

    /**
     * 套餐管理XML
     */
    public function quotalist_xml()
    {
        $condition = array();
    ...
    public function templatelist()
    {
        ...
        $this->assign('templateState',$templateState);
        $this->show_menu('voucher', 'templatelist');
		$this->setDirquna('shop');
        $this->render('voucher.templatelist');
    }

    /**
     * 代金券列表XML
     */
    public function templatelist_xml()
    {
        $condition = array();
    ...
    public function templateedit(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=templatelist','','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
            showMessage(Language::get('param_error'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=templatelist','html','error');
        }
        if(chksubmit()){
            ...
            if($rs){
                $this->log(L('spd_edit,spd_voucher_price_manage,admin_voucher_styletemplate').'[ID:'.$t_id.']');
                showMessage(Language::get('spd_common_save_succ'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=templatelist','succ');
            }else{
                showMessage(Language::get('spd_common_save_fail'),$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=templatelist','error');
            }
        }else{            
            $this->assign('t_info',$t_info);
            $this->show_menu('templateedit','templateedit');
			$this->setDirquna('shop');
            $this->render('voucher.templateedit');
        }
    }
    ...
    private function show_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                3=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=templatelist'),
                2=>array('menu_key'=>'quotalist','menu_name'=>Language::get('admin_voucher_quota_manage'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=quotalist'),
                5=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist'),
                4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=setting'),
                );
                break;
            case 'priceedit':
                $menu_array = array(
                1=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=setting'),
                2=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=pricelist'),
                3=>array('menu_key'=>'priceedit','menu_name'=>Language::get('admin_voucher_priceedit'), 'menu_url'=>'')
                );
                break;
            case 'templateedit':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=Voucher&a=templatelist'),
