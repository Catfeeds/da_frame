...
class SnsMemberController extends SystemController {
    public function __construct(){
        parent::__construct();
        Language::read('sns_member', 'Home');
    }
    ...
    public function member_tag(){
        ...
        $this->render('sns_membertag.index');
    }
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $url = array(
                            array(
                                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsMember&a=tag_add',
                                    'msg'=>Language::get('sns_member_add_once_more'),
                            ),
                            array(
                                    'url'=>$GLOBALS['_PAGE_URL'] . '&c=SnsMember&a=member_tag',
                                    'msg'=>Language::get('sns_memner_return_list'),
                            )
                    );
                    $this->log(L('spd_add,sns_member_tag').'['.$_POST['membertag_name'].']',1);
                    showMessage(Language::get('spd_common_op_succ'),$url);
                }else {
                    showMessage(Language::get('spd_common_op_fail'));
                }
            }
...
$this->setDirquna('shop');
    ...
    public function tag_edit(){
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $this->log(L('spd_edit,sns_member_tag').'['.$_POST['membertag_name'].']',1);
                    showMessage(Language::get('spd_common_op_succ'),$GLOBALS['_PAGE_URL'] . '&c=SnsMember&a=member_tag');
                }else {
                    showMessage(Language::get('spd_common_op_fail'));
                }
            }
		...
		$this->setDirquna('shop');
        $this->render('sns_membertag.edit');
    }
    /**
     * 删除标签
     */
    public function tag_del(){
        ...
        if ($result){
            $this->log(L('spd_del,sns_member_tag').'[ID:'.$mtag_id.']',1);
    ...
    public function get_xml() {
        ...
        foreach ($tag_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href='{$GLOBALS['_PAGE_URL']}&c=SnsMember&a=tag_del&id=" . $value['mtag_id'] . "'><i class='fa fa-trash-o'></i>删除</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('spd_set') . " <i class='arrow'></i></em><ul><li><a href='{$GLOBALS['_PAGE_URL']}&c=SnsMember&a=tag_edit&id=" . $value['mtag_id'] . "'>编辑标签</a></li><li><a href='{$GLOBALS['_PAGE_URL']}&c=SnsMember&a=tag_member&id=".$value['mtag_id']."'>标签会员</a></li><li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('sns_member','tag_recommend',array('id'=> $value['mtag_id'], 'recommend' => ($value['mtag_recommend'] == 1 ? 0 : 1))) . "\")'>" . ($value['mtag_recommend'] == 1 ? '取消推荐' : '推荐标签') . "</a></li></ul></span>";
    ...
    public function tag_member(){
		...
		$this->setDirquna('shop');
        $this->render('sns_membertag.memberlist');
    }

    /**
     * 推荐标签会员
     */
    public function tag_mrecommend() {
        // 验证
        $mtag_id = intval($_GET['id']);
