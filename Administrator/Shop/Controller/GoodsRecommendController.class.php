...
class GoodsRecommendController extends SystemController {
    ...
    public function index(){
        ...
        $this->render('goods_recommend.index');
    }
    ...
    public function add(){
        ...
        $this->assign('rec_info', is_array($rec_list) ? current($rec_list) : array());
		    	
		$this->setDirquna('shop');
        $this->render('goods_recommend.add');
    }
    ...
    public function save(){
        ...
        if ($insert) {
            showMessage('保存成功',$GLOBALS['_PAGE_URL'] . '&c=GoodsRecommend&a=index');
        }
    }
    ...
    public function get_xml(){
        ...
        foreach ($rec_list as $v) {
            $list = array();
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['rec_gc_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=GoodsRecommend&a=add&rec_gc_id={$v['rec_gc_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function get_goods_list(){
        ...
        foreach($goods_list as $v) {
            ...
            $html .= <<<EOB
            <li>
            <div class="spdgoods-pic" onclick="select_recommend_goods({$v['goods_id']});">
            ...
            <div class="spdgoods-name">
            <a target="_blank" href="{$url}">{$v['goods_name']}</a>
            </div>
            </li>
EOB;
        }
        $admin_tpl_url = ADMIN_STATIC_URL;
