...
class CircleThemeController extends SystemController {
...
$this->render('circle_theme.list');
    ...
    public function get_xml() {
        ...
        foreach ($theme_list as $value) {
            ...
            $operation .= "<li><a href='{$GLOBALS['_PAGE_URL']}&c=CircleTheme&a=theme_info&t_id=".$value['theme_id']."'>查看话题内容</a></li>";
            $operation .= "<li><a href='{$GLOBALS['_PAGE_URL']}&c=CircleTheme&a=theme_reply&t_id=".$value['theme_id']."'>查看话题回复</a></li>";
    ...
    public function theme_del(){
        ...
        showMessage(L('spd_common_op_succ'), $GLOBALS['_PAGE_URL'] . '&c=CircleTheme&a=theme_list');
    }
    /**
     * 话题回复
     */
    public function theme_reply(){
        $model = Model();
        if(chksubmit()){
        ...
        $this->assign('page', $model->showpage(2));
        $this->assign('reply_list', $reply_list);
        $this->setDirquna('circle');
		$this->render('circle_theme.reply');
    }

    /**
     * 输出XML数据
     */
    public function get_reply_xml() {
        $model = Model();
        $condition = array();
    ...
    public function reply_info(){
        ...
        $this->setDirquna('circle');
$this->render('circle_theme.reply_info', 'null_layout');
    }
    /**
     * 话题回复删除
     */
    public function theme_replydel(){
        $t_id = intval($_GET['t_id']);
     ...
     * 推荐/取消话题
     */
    public function theme_recommend(){
