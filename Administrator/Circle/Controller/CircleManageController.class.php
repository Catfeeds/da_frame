...
class CircleManageController extends SystemController {
...
$this->render('circle.list');
    ...
    public function get_xml() {
        ...
        foreach ($cirlce_list as $value) {
            ...
            if ($value['circle_status'] == '2') {
                $operation .= "<a class='btn orange' href='{$GLOBALS['_PAGE_URL']}&c=CircleManage&a=circle_edit&c_id=".$value['circle_id']."' class='url'><i class='fa fa-pencil-square-o'></i>审核</a>";
            } else {
                $operation .= "<a class='btn blue' href='{$GLOBALS['_PAGE_URL']}&c=CircleManage&a=circle_edit&c_id=".$value['circle_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            }
     ...
     * 圈子待审核列表
     */
    public function circle_verify(){
        $this->setDirquna('circle');

$this->render('circle.verify');
    }
    /**
     * 圈子新增
     */
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($circleid){
                    ...
                    $this->log(L('spd_add_circle').'['.$circleid.']');
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($rs){
                    ...
                    $this->log(L('spd_edit_circle').'['.intval($_POST['c_id']).']');
                    showMessage(L('spd_common_op_succ'), $GLOBALS['_PAGE_URL'] . '&c=CircleManage&a=circle_list');
                }else{
                    showMessage(L('spd_common_op_fail'));
                }
            }
...
$this->render('circle.edit');
    }
    /**
     * 选择圈主
     */
    public function choose_master(){
        $this->assign('search_url', (intval($_GET['c_id']) > 0) ? urlAdminCircle('circle_manage', 'search_member', array('c_id' => intval($_GET['c_id']))) : urlAdminCircle('circle_manage', 'search_member'));
        $this->setDirquna('circle');
$this->render('circle.choose_master', 'null_layout');
    }
    /**
     * 搜索会员
     */
    ...
    public function circle_del(){
        ...
        $this->log(L('spd_del_circle').'['.$id.']');
        exit(json_encode(array('state'=>true,'msg'=>L('spd_common_op_succ'))));
    }
    /**
     * 会员名称检测
     */
    ...
    public function ajax(){
        switch ($_GET['branch']){
            case 'status':
                $this->log(L('spd_circle_pass_cerify').'['.intval($_GET['id']).']');
            case 'recommend':
                $update = array(
