...
class PersonalClassController extends SystemController {
    ...
    public function index() {
       $this->personalclass_list();
    }

    /**
     * 微商城商品(随心看)分类管理
     **/
    public function personalclass_list() {
...
$this->render("microshop_personal_class.list");
    }

    /**
     * 微商城个人秀分类添加
     **/
    public function personalclass_add() {
        $this->show_menu_personal_class('personal_class_add');
        $this->setDirquna('microshop');
$this->render('microshop_personal_class.add');
    }

    /**
     * 微商城商品(随心看)分类编辑
     **/
    public function personalclass_edit() {
...
$this->render("microshop_personal_class.add");
     ...
     * 微商城个人秀分类保存
     **/
    public function personalclass_save() {
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"{$GLOBALS['_PAGE_URL']}&c=PersonalClass&a=personalclass_list");
        } else {
            showMessage(Language::get('class_add_fail'),"{$GLOBALS['_PAGE_URL']}&c=PersonalClass&a=personalclass_list",'','error');
        }
     ...
     * ajax修改分类排序
     */
    public function personalclass_sort_update() {
        if(intval($_GET['id']) <= 0) {
     ...
     * ajax修改分类名称
     */
    public function personalclass_name_update() {
     ...
     * 个人秀分类删除
     **/
     public function personalclass_drop() {
     ...
     private function show_menu_personal_class($menu_key) {
         $menu_array = array(
                 'personal_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('spd_manage'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=PersonalClass&a=personalclass_list'),
                 'personal_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('spd_new'),'menu_url'=>$GLOBALS['_PAGE_URL'] . '&c=PersonalClass&a=personalclass_add'),
         );
         if($menu_key == 'personal_class_edit') {
             $menu_array['personal_class_edit'] = array('menu_type'=>'link','menu_name'=>Language::get('spd_edit'),'menu_url'=>'###');
         }
