...
class PersonalController extends SystemController {
    ...
    public function personal_manage()
    {
		...
		$this->render('microshop_personal.manage');
    }

    /**
     * 个人秀管理XML
     */
    public function personal_manage_xml()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="' . $GLOBALS['_PAGE_URL'] . '&c=Personal&a=personal_drop&personal_id=' .
                $val['personal_id'] .
            ...
            $u = MICROSHOP_SITE_URL . '&c=Personal&a=detail&personal_id=' . $val['personal_id'];
...
EOB;
     ...
     * 更新微商城个人秀排序
     */
    public function personal_sort_update() {
        if(intval($_GET['id']) <= 0) {
