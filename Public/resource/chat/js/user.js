	...
	var audio_info = '<object width="1" height="1" style="position: absolute; left: -1px;" id="msg_audio" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">'+
								'<param value="'+CHAT_RESOURCE_SITE_URL+'/audio.swf" name="movie"><param value="always" name="allowscriptaccess">'+
								'<embed width="1" height="1" allowscriptaccess="always" src="'+CHAT_RESOURCE_SITE_URL+'/audio.swf" name="msg_audio"></object>';
    $(function(){
		...
		if(user['u_id'] != '') {
			...
			web_info['msg_dialog'] = '<div id="new_msg_dialog" class="msg-windows"><div class="user-tab-bar"><ul class="user-list" id="user_list"></ul></div>'+
									...
									'<span class="title">输入聊天信息</span><span class="spdchat-log-btn off" onclick="show_chat_log();">聊天记录<i></i></span></div>'+
									...
									'<div id="dialog_chat_right" class="dialog-spdchat-right"></div><div id="dialog_chat_log" class="dialog_chat_log"></div></div>'+
									'<div id="dialog_clear" class="dialog_clear"></div></div>';
			var chat_user_list = '<div class="spdchat-box"><div class="spdchat-list"><div class="spdchat-list-top"><h1><i></i>联系人</h1><span class="minimize-spdchat-list" onclick="chat_show_list();"></span></div>'+
									'<div id="chat_user_list" class="spdchat-list-content"><div><dl id="chat_user_friends"><dt onclick="chat_show_user_list(\'friends\');">'+
									...
									'</div></div>';
			var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_user_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
			    ...
			    success: function(u_list){
			  	for (var u_id in u_list){
					...
					setInterval( function () {
						$.get(CHAT_SITE_URL+'&c=WebChat&a=get_session&key=member_id');
					}, time_max*60000);
	        ...
	        if ( c_a == 'goods_index' ) {//异步调用商品信息
				var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_goods_info&goods_id='+chat_goods_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
					...
					success: function(goods){
					    if (typeof goods['goods_id'] !== "undefined" ) {
                	        ...
                	        var goods_html = '';
                    		goods_html += '<div class="spdgoods-info">';
                    		goods_html += '<div class="spdgoods-pic"><a target="_blank" href="'+goods['url']+'">';
                    		goods_html += '<img src="'+goods['pic24']+'"></a></div>';
							goods_html += '<div class="spdgoods-price">&yen;'+goods['goods_promotion_price']+'</div>';   
                    		goods_html += '<div class="spdgoods-name"><a class="goods_pic" target="_blank" href="'+goods['url']+'">';
                    		goods_html += goods['goods_name']+'</a></div>';          
							goods_html += '<div class="spdgoods-buy"><a class="buynow" href="JavaScript:void(0);" onclick="chat_goods_buy();">立即购买</a></div>';         		
                    		goods_html += '</div>';
			...
			$("#chat_show_user").click(function() {
			    $('div[datype="a-barLoginBox"]').trigger("click");
			});
		}
	});
	...
	function get_state(list){//返回会员的状态并在页面显示
		...
		if(layout == 'layout/store_layout.php' || c_a == 'goods_index') {//店铺页面
		    var store_id = 0;
		    var store_name = '';
	...
	function chat_goods_buy(){
		$('a[datype="buynow_submit"]').trigger("click");
	}
	function send_focus(){
		$("#send_alert").html('');
	}
	...
	function send_msg(){//发消息
		...
		$.ajax({
			type: "POST",
			url: CHAT_SITE_URL+'&c=WebChat&a=send_msg',
			dataType:"jsonp",
			data: msg,
			async: false,
	...
	function get_msg(list){//接收消息
		...
		for (var k in list){
			...
			if ( typeof user_list[u_id]['avatar'] === "undefined" ) {//当没获得会员信息时调用一次
				var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_info&t=member&u_id='+u_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
	...
	function get_chat_log(time_from){
		...
		if(obj_chat_log.html() == '') {
			var chat_log_list = '<div class="spdchat-log-top"><h1><i></i>聊天记录</h1><span class="close-spdchat-log" onclick="show_chat_log();"></span></div>'+
						'<div id="chat_log_list" class="chat_log_list"><div id="chat_log_msg" class="spdchat-log-msg"></div></div><div class="spdchat-log-bottom"><div id="chat_time_from" class="chat_time_from">'+
	...
	function get_chat_msg(t){
		var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_chat_log&page=30&f_id='+user['u_id']+'&t_id='+chat_log['u_id']+'&t='+chat_log['time_from'];
	...
	function get_goods_info(msg){//显示商品图片和名称
	    if ( typeof msg['goods_id'] === "undefined" ) {//没有定义商品编号时调用检测一次
	        ...
	        var re = new RegExp(shop_site+"&amp;c=Goods&amp;a=index&amp;goods_id=(\\d+)$","g");
    	    ...
    	    if ( goods_id > 0 ) {
    	        if ( typeof goods_list[goods_id] === "undefined" ) {
    				var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_goods_info&goods_id='+goods_id;
    				$.ajax({
    					type: "GET",
    					url: ajaxurl,
    					dataType:"jsonp",
	...
	function get_goods_list(s_id){
	    if(layout == 'layout/seller_layout.php' || c_a == 'goods_index') {//商家中心不显示推荐商品
	        return ;
	    }
		...
		var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_goods_list&s_id='+s_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			dataType:"jsonp",
		    ...
		    success: function(list){
		        ...
		        for (var k in list){
		            var goods = list[k];
		            var text_goods = '<li>';
		            text_goods += '<div class="spdgoods-pic"><a href="'+goods['url']+'" target="_blank">';
		            text_goods += '<img title="'+goods['goods_name']+'" alt="'+goods['goods_name']+'" src="'+goods['pic']+'"/></a></div>';
		            text_goods += '<div class="spdgoods-price">&yen;'+goods['goods_promotion_price']+'</div>';
		            text_goods += '</li>';
	...
	function show_chat_log(){
		...
		if (typeof chat_log['u_id'] === "undefined" || chat_log['u_id'] != user_show) {
			...
			obj.find(".spdchat-log-btn").removeClass("off");
			obj.find(".spdchat-log-btn").addClass("on");
			get_chat_log(7);
		} else {
			close_chat_log(user_show);
		}
	...
	function show_msg(u_id){//显示会员的消息
		...
		if(user_show != u_id) {
			...
			if (typeof user_info['s_name'] !== "undefined" && user_info['s_name'] !== "") {
			    ...
			    store_html = '<a target="_blank" href="'+SHOP_SITE_URL+'&c=ShowStore&a=index&store_id='+
	...
	function play_audio(){//提示声音
	    if ( chat_audio === 1 ) {
	        ...
	        if ( typeof swf.pplay === "function" ) {
	            swf.load(CHAT_RESOURCE_SITE_URL +'/msg.mp3');
	...
	function close_chat_log(u_id){
		if (user_show == 0 || chat_log['u_id'] == u_id) {
			...
			$('#chat_log_msg').html('');
			obj.find(".spdchat-log-btn").removeClass("on");
			obj.find(".spdchat-log-btn").addClass("off");
			if ( connect === 1 ) $("#web_chat_dialog").show();
		}
	}
	...
	function update_user(u_id){
		if($("#dialog_chat_right").html() == '') {
    		...
    		text_append += '</div>';
    		if(layout != 'layout/seller_layout.php' && c_a != 'goods_index') {//商家中心页面和商品详细页面不显示推荐商品
    		    text_append += '<div id="chat_goods_list" class="spdgoods-list"></div>';
    		}
