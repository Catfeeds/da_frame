	...
	$(function(){
		if(user['seller_id'] != '') {
            ...
            web_info['html_store_msg'] = '<div id="store_msg_dialog" class="dialog_wrapper" style="z-index: 3100; float: right; margin: 0; position: fixed; width: 280px; right: 50px; bottom: 0;display: none;">'+
									...
									'<div class="dialog_content" style="margin: 0px; padding: 0px;"><div class="spdsc-form-default"><dl><dd style="width:100%; padding:10px;"><p></p></dd></dl>'+
			...
			var chat_user_list = '<div class="spdchat-box"><div class="spdchat-list"><div class="spdchat-list-top"><h1><i></i>联系人</h1><span class="minimize-spdchat-list" onclick="chat_show_list();"></span></div>'+
									'<div id="chat_user_list" class="spdchat-list-content"><div><dl id="chat_user_sellers"><dt onclick="chat_show_user_list(\'sellers\');">'+
									...
									'</div></div>';
			var ajaxurl = CHAT_SITE_URL+'&c=WebChat&a=get_seller_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
	...
	function store_goods_info(f_id){//只在商家中心显示来源商品信息
        ...
        if (typeof user_chat_goods[f_id] === "object") {
	        ...
	        var goods_html = '';
    		goods_html += '<div id="chat_goods_info" class="spdgoods-info">';
	...
	function store_msg(msg){
	    ...
	    if( user['seller_is_admin'] == 1 || msg_limits[code] == 1) {
	        ...
	        var text_append = '<a href="'+SHOP_SITE_URL+'&c=StoreMsg&a=index">'+sm_content+'</a>';
	        store_msg_obj.find(".spdsc-form-default dl p").html(text_append);
	        store_msg_obj.show();
