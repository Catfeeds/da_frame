 ...
 //图片上传
    $("#article_image_upload").fileupload({
        dataType: 'json',
            url: _PAGE_URL + "&c=Publish&a=article_image_upload",
            add: function(e,data) {
                ...
                if(result.status == "success") {
					...
					var image_handle = '<a datype="btn_insert_article_image">插入</a>';
                    image_handle += '<a datype="btn_set_article_image" image_name="'+result.file_name+'">封面</a>';
                    image_handle += '<a datype="btn_drop_article_image" image_name="'+result.file_name+'">删除</a>';
    ...
    //设封面图
    $("[datype='btn_set_article_image']").live("click",function(){
    ...
    //插入编辑器
    $("[datype='btn_insert_article_image']").live("click",function(){
        var html = KE.html();
    ...
    //图片删除
    $("[datype='btn_drop_article_image']").live("click",function(){
        var image_object = $(this).parent().parent();
        var image_name = $(this).attr("image_name");
        $.getJSON(_PAGE_URL + "&c=Publish&a=article_image_drop", { image_name: image_name }, function(result){
            if(result.status == "success") {
                image_object.remove();
            } else {
                showError(result.error);
    ...
    //标签操作
    $("[datype='cms_tag']").live("click",function(){
        ...
        if($("#article_goods_list li").length < 3) { 
            ...
            if(search_keyword != "") {
                if(search_type == "goods_url") {
                    ...
                    $.getJSON(_PAGE_URL + "&c=Api&a=goods_info_by_url", { url: url}, function(data){
                        if(data.result == "true") {
                    ...
                    $("#div_goods_select").load(_PAGE_URL + "&c=Api&a=goods_list&search_type="+search_type+"&search_keyword="+search_keyword);
                }
            }
        }
    });
    ...
    //商品添加
    $("#goods_search_list [datype='btn_goods_select']").live("click",function(){
        if($("#article_goods_list li").length < 3) { 
            var temp = '<li datype="btn_goods_select">'+$(this).html();
    ...
    //商品删除
    $("#article_goods_list [datype='btn_goods_select']").live("click",function(){
        $(this).remove();
    });
    ...
    $("#btn_article_search").click(function(){
        ...
        if(search_keyword != "") {
            $("#div_article_select").load(_PAGE_URL + "&c=Api&a=article_list&search_type="+search_type+"&search_keyword="+search_keyword);
        }
    });
    ...
    //文章添加
    $("#article_search_list [datype='btn_article_select']").live("click",function(){
    ...
    //文章删除
    $("#article_link_list [datype='btn_article_select']").live("click",function(){
        $(this).remove();
        refresh_article_link();
    });
    ...
    function refresh_article_link() {
        ...
        $("#article_link_list [datype='btn_article_select']").each(function(){
