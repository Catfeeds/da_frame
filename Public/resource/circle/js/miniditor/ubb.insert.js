(function($) {
	$.fn.ncUBB = function(options){
		...
		this.each(function() {
			...
			var $this = $(this);
			// 加粗
			$this.find('a[datype="b"]').click(function(){
			...
			// 斜体
			$this.find('a[datype="i"]').click(function(){
				UBBContent.inserEitherEndt({
					part_one : '[I]',
					part_two : '[/I]'
				});	
			...
			// 下划线
			$this.find('a[datype="u"]').click(function(){
				UBBContent.inserEitherEndt({
					part_one : '[U]',
					part_two : '[/U]'
				});	
			...
			$this.find('a[datype="chooseGoods"]').click(function(){
				// 已经插入的商品数量计算
				var count = 10 - $('.insert-goods > dl').length; // 0 已经插入的商品数量
				var _uri = CIRCLE_SITE_URL+'&c=Theme&a=choose_goods&c_id='+c_id+'&count='+count;
				CUR_DIALOG = ajax_form("choosegoods", '选择商品', _uri, 510);
			});
		
			$this.find('a[datype="uploadImage"]').click(function(){
				var _uri = CIRCLE_SITE_URL+'&c=Theme&a=choose_image&c_id='+c_id;
				CUR_DIALOG = ajax_form("uploadimage", '选择图片', _uri, 480);
			});
			// insert affix
		    $this.find('a[datype="affix_insert"]').click(function(){
				affixInsert($(this));
		    });
		    // delete affix
		    $this.find('a[datype="affix_delete"]').click(function(){
				affixDelete($(this));
			});
		    // Administrator delete appendage affix
		    $this.find('a[datype="maffix_delete"]').click(function(){
		    	affixDeleteManage($(this));
			});
		    // delete goods
		    $this.find('.spdgoods-del').click(function(){
				$(this).parent().remove();
				checkInsertGoods();
			});
			// 相册图片插入到话题
			$('a[datype="imagealbum"]').die().live('click',function(){
				var data_str = $('.choose-image').find('a[datype="chooseimage"][class="selected"]').attr('data-param'); eval('data_str = ' + data_str);
				if(typeof(data_str) == 'undefined') {return false;}
				insertImgUBB(data_str.img);
				DialogManager.close('uploadimage');
			});
			// 链接地址插入话题
			$('a[datype="imageurl"]').die().live('click',function(){
				var img = $('input[datype="imageurl"]').val();
				insertImgUBB(img);
				DialogManager.close('uploadimage');
			});

			// 插入到主题
			$('a[datype="insertGoods"]').die().live('click',function(){
			...
			// Senior reply
			$this.find('a[datype="highReply"]').click(function(){
				_uri = _PAGE_URL + '&c=Theme&a=reply&c_id='+c_id+'&t_id='+t_id;
		...
		function insertGoods(o){
			o.each(function(){
				...
				$('<dl></dl>').append('<dt class="spdgoods-name">'+data_str.name+'</dt><dd class="spdgoods-pic"><a href="javascript:void(0);"><img src="'+data_str.img+'" /></a></dd><dd class="spdgoods-price"><em>'+data_str.price+'</em></dd><dd class="spdgoods-del">删除</dd>')
					...
					.appendTo('.insert-goods').find('.spdgoods-del').click(function(){
						$(this).parent().remove();
						checkInsertGoods();
					});
			});
		...
		function affixDelete(o){
			...
			$.getJSON(CIRCLE_SITE_URL+'&c=Theme&a=delimg&c_id='+c_id+'&id='+id, function(){
		...
		function affixDeleteManage(o){
			// 删除图片
			var id = o.attr('data-param');
			$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=delimg&c_id='+c_id+'&id='+id, function(){
				o.parents('li:first').remove();
				checkInsertAffix();
			});
		}
		...
		function updateAffixInsert(param){
			...
			.append('<div class="handle"><a href="javascript:void(0);" datype="affix_insert" data-param="'+param.file_insert+'"><i class="c"></i>插入</a><a href="javascript:void(0);" datype="affix_delete" data-param="'+param.file_id+'"><i class="d"></i>删除</a></div>')
			.find('a[datype="affix_insert"]').click(function(){
				affixInsert($(this));
			}).end().find('a[datype="affix_delete"]').click(function(){
				affixDelete($(this));
			});
		}
		// get unused affixes
		function getUnusedAffix(){
			...
			if(len == 0){
				$.getJSON(_PAGE_URL + '&c=Theme&a=unused_img&c_id='+c_id, function(data){	// 获取未使用附件
					if(data != null){
						$('div[datype="affix"]').hide();
