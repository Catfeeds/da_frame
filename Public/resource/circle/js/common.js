$(function(){
	$('.my-group').mouseover(function(){
		...
		if(!$this.hasClass('t')){
			$this.addClass('t');
			if(_ISLOGIN){
				$.getJSON(CIRCLE_SITE_URL+'&c=Index&a=myjoinedcircle', function(data){
					if(data){
						$.each(data, function(e,d){
							...
							$('<a href="'+CIRCLE_SITE_URL+'&c=Group&c_id='+d.circle_id+'">'+d.circle_name+$i+'</a>').appendTo('span[datype="span-mygroup"]');
						});
					}else{
						$('<a href="javascript:void(0);">暂未加入过</a>').appendTo('span[datype="span-mygroup"]');
					}
				});
			}
		}
	});
	$('a[datype="login"]').click(function(){
		login_dialog();
	});
	...
	// 创建圈子
	$('a[datype="create_circle"]').click(function(){
		if(_ISLOGIN == 0){
			login_dialog();
		}else{
			window.location.href=CIRCLE_SITE_URL+"&c=Index&a=add_group";
		}
	});
					...
					if(options.identity  == 1 || options.identity == 2 || options.identity == 3){ 	// 成员点击展开回复
						if(options.reply_box.css('display') == 'none'){
							if(!options.reply.hasClass('t')){
								...
								// 快速回复
								$.getJSON(CIRCLE_SITE_URL+'&c=Theme&a=ajax_quickreply&c_id='+options.c_id+'&t_id='+options.id, function(data){
									
									// 头像  快速回复栏
									if(data.c_istalk){
										$('<div class="spdmember-avatar-m"><img src="'+data.member_avatar+'" /></div>').appendTo(options.reply);
										...
										$('<div class="bottom"><a class="submit-btn" href="javascript:void(0);" datype="reply_submit">发表回复</a><div datype="warning" id="warning"></div></div>').appendTo(form);						
										
										
										form.find('a[datype="reply_submit"]').click(function(){
										    form.submit();
										...
										form.validate({
									        errorLabelContainer: form.find('div[datype="warning"]'),
									...
									if(data.reply_list){
										$.each(data.reply_list, function(e, d){
											...
											$('<div class="spdmember-avatar-s"><img src="'+d.member_avatar+'" /></div>').appendTo(reply_list);
											...
											var reply_dl = $('<dl></dl>');
											$('<dt class="spdmember-name">'+d.member_name+'<span class="reply-date">'+d.reply_addtime+'</span></dt>').appendTo(reply_dl);
						...
						// 点击展开申请
						CUR_DIALOG = ajax_form('apply_join','申请加入',_PAGE_URL + '&c=Group&a=apply&c_id='+options.c_id,520,1);
					}
				}else{
					login_dialog();		
				}
	...
	// Membership card
	$('[datype="mcard"]').membershipCard({type:'circle'});
});
//弹出框登录
...
function likeYes(o,options){
	$.getJSON(CIRCLE_SITE_URL+'&c=Theme&a=ajax_likeyes&c_id='+options.c_id+'&t_id='+options.t_id, function(data){
		if(data){
			var likeCount = parseInt(o.find('em[datype="like"]').html())+1;
			o.html('取消赞(<em datype="like">'+likeCount+'</em>)');
...
function likeNo(o,options){
	$.getJSON(CIRCLE_SITE_URL+'&c=Theme&a=ajax_likeno&c_id='+options.c_id+'&t_id='+options.t_id, function(data){
		if(data){
			var likeCount = parseInt(o.find('em[datype="like"]').html())-1;
			o.html('赞(<em datype="like">'+likeCount+'</em>)');
			o.unbind().click(function(){
				likeYes(o,options);
			});
		}
