$(function(){
	/* group.top */
	// 申请加入
	$('a[datype="apply"]').click(function(){
		if(_ISLOGIN == 0){
			login_dialog();
		}else{
			CUR_DIALOG = ajax_form('apply_join','申请加入',_PAGE_URL + '&c=Group&a=apply&c_id='+c_id,520,1);
		}
	});
	// 退出圈子
	$('a[datype="quitGroup"]').click(function(){
		showDialog('确定要退出圈子吗？', 'confirm', '', function(){
			var _uri = CIRCLE_SITE_URL+"&c=Group&a=quit&c_id="+c_id;
			ajaxget(_uri);
		});
	});
	//横高局中比例缩放隐藏显示图片
	...
	// 展开收起
	$('a[datype="theme_read"],span[datype="theme_read"]').click(function(){
		var $pli = $(this).parents('li:first');
		var $this = $pli.find('a[datype="theme_read"]');
		if(!$this.hasClass('t')){
			$this.addClass('t');
			...
			$.getJSON(CIRCLE_SITE_URL+'&c=Theme&a=ajax_themeinfo&c_id='+c_id+'&t_id='+$id, function(data){
				// 内容部分
				...
				if(data.goods_list != ''){
					...
					$.each(data.goods_list, function(e, d){
						$('<li></li>').append('<div class="spdgoods-pic thumb"><a href="javascript:void(0);"><img class="t-img" src="'+d.image+'" /></a></div>')
							.append('<div class="spdgoods-name">'+d.goods_name+'</div>')
							.append('<div class="spdgoods-price"><em>'+d.goods_price+'</em></div>')
				...
				if(data.theme_nolike){ // 赞
					var _$like = $('<a href="javascript:void(0);">赞(<em datype="like">'+data.theme_likecount+'</em>)</a>');
					...
					var _$unlike = $('<a href="javascript:void(0);">取消赞(<em datype="like">'+data.theme_likecount+'</em>)</a>');
				...
				// 回复
				var _$reply = $('<a href="javascript:void(0);" datype="reply">回复('+data.theme_commentcount+')</a>');
				_$reply.quick_reply({
						reply		: $reply,
						reply_box	: $reply_box,
						id			: $id,
	...
	/* theme.detail */
	// 赞
	$('a[datype="themeLikeYes"]').click(function(){
		if(_ISLOGIN){
	...
	// 取消赞
	$('a[datype="themeLikeNo"]').click(function(){
		likeNo($(this), {t_id:t_id,c_id:c_id});
	});
	// 加精
	$('a[datype="themeDigestYes"]').click(function(){
		digestYes($(this));
	});
	// 取消加精
	$('a[datype="themeDigestNo"]').click(function(){
		digestNo($(this));
	});
	// 置顶
	$('a[datype="themeTopYes"]').click(function(){
		topYes($(this));
	});
	// 取消置顶
	$('a[datype="themeTopNo"]').click(function(){
		topNo($(this));
	});
	// 关闭
	$('a[datype="themeShutYes"]').click(function(){
		shutYes($(this));
	});
	// 开启
	$('a[datype="themeShutNo"]').click(function(){
		shutNo($(this));
	});
	// 禁言
	$('a[datype="themeCloseYes"]').click(function(){
		var m_id = $(this).attr('data-param');
		var _uri = CIRCLE_SITE_URL+'&c=Manage&a=ajax_nospeak&value=0&c_id='+c_id+'&m_id='+m_id;
		ajaxget(_uri);
	});
	// 解除禁言
	$('a[datype="themeCloseNo"]').click(function(){
		var m_id = $(this).attr('data-param');
		var _uri = CIRCLE_SITE_URL+'&c=Manage&a=ajax_nospeak&value=1&c_id='+c_id+'&m_id='+m_id;
		ajaxget(_uri);
	});
	// 管理删除主题
	$('a[datype="themeDelManage"]').click(function(){
		showDialog('确定要删除主题吗？', 'confirm', '', function(){
			var _uri = CIRCLE_SITE_URL+"&c=Manage&a=del_theme&c_id="+c_id+"&t_id="+t_id;
			ajaxget(_uri);
		});
	});
	// 管理删除回复
	$('a[datype="replyDelManage"]').click(function(){
		var $this = $(this);
		showDialog('确定要删除回复吗？', 'confirm', '', function(){
			var r_id = $this.attr('data-param');
			var _uri = CIRCLE_SITE_URL+'&c=Manage&a=del_reply&c_id='+c_id+'&t_id='+t_id+'&r_id='+r_id;
			ajaxget(_uri);
		});
	});
	...
	$('a[datype="manageApply"]').click(function(){
		var _uri = CIRCLE_SITE_URL+'&c=Group&a=manage_apply&c_id='+c_id;
...
function digestYes(o){
	$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=ajax&column=digest&value=1&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('取消精华');
...
// 取消加精
function digestNo(o){
	$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=ajax&column=digest&value=0&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('精华');
...
// 置顶
function topYes(o){
	$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=ajax&column=top&value=1&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('取消置顶');
...
// 取消置顶
function topNo(o){
	$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=ajax&column=top&value=0&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('置顶');
			$('.theme-title > i').removeClass().addClass('normal');
			o.unbind().click(function(){
...
// 关闭
function shutYes(o){
	$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=ajax&column=shut&value=1&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('开启');
...
// 开启
function shutNo(o){
	$.getJSON(CIRCLE_SITE_URL+'&c=Manage&a=ajax&column=shut&value=0&c_id='+c_id+'&id='+t_id, function(data){
		if(data){
			o.html('关闭');
			$('.theme-title > i').removeClass().addClass('normal');
			o.unbind().click(function(){
...
function speakNo(m_id){
	_uri = CIRCLE_SITE_URL+'&c=Manage&a=ajax_nospeak&value=0&c_id='+c_id+'&m_id='+m_id;
	ajaxget(_uri);
}
// 删除回复