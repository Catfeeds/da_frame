...
function add_screen(add_type) {//增加图片
	for (var i = 1; i <= screen_max; i++) {//防止数组下标重复
		if (screen_obj.find("li[screen_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			'</h4><a class="spdap-btn-mini del" href="JavaScript:del_screen('+i+
			');"><i class="fa fa-trash"></i>删除</a></div><div class="focus-thumb" onclick="select_screen('+i+');" title="点击编辑选中区域内容"><img src="'+ADMIN_STATIC_URL
...
function add_focus(add_type) {//增加
	for (var i = 1; i <= focus_max; i++) {//防止数组下标重复
		if (focus_obj.find("div[focus_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			'<a class="spdap-btn-mini del" href="JavaScript:del_focus('+i+');"><i class="fa fa-trash"></i>删除</a></div><ul></ul></div>';
