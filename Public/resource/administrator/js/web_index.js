...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: _PAGE_URL + '&c=WebConfig&a=code_update',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "recommend_list":
			...
			break;
		case "brand_list":
			$("#show_brand_list").load(_PAGE_URL + '&c=WebConfig&a='+id);//查询数据
...
function get_goods_class() {//查询子分类
	...
	if (gc_id>0) {
		$.get(_PAGE_URL + '&c=WebConfig&a=category_list&id='+gc_id, function(data) {
			$(data).each(function(){
				var obj = $(this);
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load(_PAGE_URL + '&c=WebConfig&a=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
	...
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			...
    			'<dd><ul class="spdgoods-list"><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
			...
			$("#add_recommend_list").before('<div class="spdap-form-default" select_recommend_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'<div class="spdap-form-all" select_recommend_id="'+i+'"><dl class="row"><dt class="tit">推荐商品</dt><dd class="opt"><ul class="dialog-goodslist-s1 spdgoods-list">'+
    			'</ul></dd></dl></div>');
			$("#recommend_list_form dl dd ul").sortable({ items: 'li' });
			break;
		}
...
function select_recommend_goods(goods_id) {//商品选择
	...
	text_append += '<div ondblclick="del_recommend_goods('+goods_id+');" class="spdgoods-pic">';
	...
	text_append += '<div class="spdgoods-name">';
	text_append += '<a href="'+SHOP_SITE_URL+'&c=Goods&goods_id='+goods_id+'" target="_blank">';
...
function update_recommend() {//更新
	...
	if (get_text=='1') {
	  ...
	  $(".middle dl[recommend_id='"+id+"'] dd").html('<ul class="spdgoods-list">'+text_append+'</ul>');
...
function get_recommend_brand() {//查询品牌
	...
	if (brand_name!='' || brand_initial!='') {
		$("#show_brand_list").load(_PAGE_URL + '&c=WebConfig&a=brand_list&'+$.param({'brand_name':brand_name,'brand_initial':brand_initial }));
	}
}
...
function update_brand() {//更新品牌
	...
	if (get_text=='1') {
		$(".spdshopda-btbrand ul.brands").html('');
		$("img[select_brand_id]").each(function() {
		  ...
		  $(".spdshopda-btbrand ul.brands").append('<li><span>'+text_append+'</span></li>');
		});
...
function add_slide_adv() {//增加图片
	for (var i = 1; i <= slide_pic_max; i++) {//防止数组下标重复
		if ($("#upload_adv_form ul li[slide_adv_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			');"></span><span class="thumb size-106x106" onclick="select_slide_adv('+i+');"><img src="'+ADMIN_STATIC_URL+
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_sale_goods_list").load(_PAGE_URL + '&c=WebConfig&a=goods_list&'+$.param({'id':gc_id,'goods_order':goods_order,'goods_name':goods_name }));
	}
}
...
function add_sale_list() {//增加商品推荐
	if ($(".sale-layout").find("dl").size() >= sale_max)
	{
		alert("最多添加 " + sale_max + " 组");
		return;
	}
	...
	for (var i = 1; i <= sale_max; i++) {//防止数组下标重复
		if ($("dl[sale_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			add_html = '<dl sale_id="'+i+'"><dt class="title"><h4>商品推荐</h4><a href="JavaScript:del_sale_list('+i+');" class="spdap-btn-mini del"><i class="fa fa-trash"></i>删除</a><a href="JavaScript:show_sale_dialog('+i+
    			');" class="spdap-btn-mini edit"><i class="fa fa-pencil-square-o"></i>编辑</a><input name="sale_list['+i+'][recommend][name]" value="" type="hidden"></dt>'+
    			'<dd><ul class="spdgoods-list"><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
    			'<li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li></ul></dd></dl>';
			$("#add_list").before(add_html);
			$("#select_sale_list").before('<div class="spdap-form-default" select_sale_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'</dd></dl></div><div class="spdap-form-all" select_sale_id="'+i+'"><dl class="row"><dt class="tit">已选商品</dt><dd class="opt"><ul class="dialog-goodslist-s1 spdgoods-list">'+
    			'</ul></dd></dl></div>');
...
function select_sale_goods(goods_id){//商品选择
	...
	var goods_price = goods.attr("goods_price");
	var market_price = goods.attr("market_price");
	text_append += '<div ondblclick="del_sale_goods('+goods_id+');" class="spdgoods-pic">';
	...
	text_append += '</span></div>';
	text_append += '<div class="spdgoods-name">';
	text_append += '<a href="'+SITEURL+'&c=Goods&goods_id='+goods_id+'" target="_blank">';
  	text_append += goods_name+'</a>';
	text_append += '</div>';
...
function update_sale(){//更新
    ...
    obj.find("img").each(function(){
    	...
    	var goods_price = goods.attr("goods_price");
    	var market_price = goods.attr("market_price");
    	text_append += '<li><div class="spdgoods-thumb"><img title="'+goods_name+'" src="'+goods_pic+'"/></div>';
