$(document).ready(function(){
    ...
    $("input[datype='cms_image']").live("change", function(){
    ...
    $("#picture_image_upload").fileupload({
        dataType: 'json',
            url: _PAGE_URL + "&c=CmsSpecial&a=special_image_upload",
            add: function(e,data) {
                ...
                if(result.status == "success") {
					...
					var image_handle = '<a datype="btn_show_image_insert_link" image_url="'+result.file_url+'" class="insert-link tooltip" title="以图片链接模式插入专题页">&nbsp;</a>';
                    image_handle += '<a datype="btn_show_image_insert_hot_point" image_name="'+result.file_name+'" image_url="'+result.file_url+'" class="insert-hotpoint tooltip " title="以热点链接模式插入专题页">&nbsp;</a>';
                    image_handle += '<a datype="btn_drop_special_image" image_name="'+result.file_name+'" class="delete tooltip " title="删除该图片">&nbsp;</a>';
    ...
    //图片删除
    $("[datype='btn_drop_special_image']").live("click",function(){
        var image_object = $(this).parents("li");
        var image_name = $(this).attr("image_name");
        $.getJSON(_PAGE_URL + "&c=CmsSpecial&a=special_image_drop", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
    ...
    //插入图片链接对话框
    $("[datype='btn_show_image_insert_link']").live('click', function(){
    ...
    //插入图片链接
    $("[datype='btn_image_insert_link']").live('click', function(){
        var html = $("#special_content").val();
        var item = $(this).parents("div");
        var link = item.find("[datype='_image_insert_link']").val();
    ...
    //插入热点图
    $("[datype='btn_show_image_insert_hot_point']").live('click', function(){
        ...
        image_object = dialog_object.find("[datype='img_hot_point']");
        var x1 = dialog_object.find("[datype='x1']");
        var y1 = dialog_object.find("[datype='y1']");
        var x2 = dialog_object.find("[datype='x2']");
        var y2 = dialog_object.find("[datype='y2']");
        var w = dialog_object.find("[datype='w']");
        var h = dialog_object.find("[datype='h']");
        var url = dialog_object.find("[datype='url']");
        ...
        $("[datype='delete_hot_point']").live("click",function(){
            var key = $(this).attr("name");
            $("#"+key).remove();
            $("."+key).remove();
        });

        $("[datype='select_hot_point']").live("click",function(){
        ...
        dialog_object.find("[datype='btn_hot_point_commit']").click(function(){
            count++;
            ...
            dialog_object.find("[datype='list']").append('<li class="'+key+'">热点区域'+count+'<span>('+url.val()+')</span><a datype="select_hot_point" name="'+xy+'" href="javascript:void(0);" class="spdap-btn spdap-btn-blue mr5"><i class="fa fa-location-arrow"></i>选中</a><a datype="delete_hot_point" class="spdap-btn spdap-btn-red" name="'+key+'" href="javascript:void(0);"><i class="fa fa-trash"></i>删除</a></li>');
    ...
    //插入图片链接
    $("[datype='btn_image_insert_hot_point']").live('click', function(){
        var html = $("#special_content").val();
    ...
    //图片删除
    $("[datype='btn_drop_special_image']").click(function() {
    ...
    //选择商品
    $("[datype='btn_special_goods']").live('click', function(){
        var goods_list = $(this).parents("div").find("[datype='_special_goods_list']");
        var link_item = $(this).parents("div").find("[datype='_input_goods_link']");
        var link = link_item.val();
        link_item.val('');
        if(link != '') {
        var url = encodeURIComponent(link);
        $.getJSON(_PAGE_URL + "&c=CmsSpecial&a=goods_info_by_url", { url: url}, function(data){
            if(data.result == "true") {
                var temp = '<li datype="btn_goods_select"><dl>'; 
                ...
                temp += '<dd class="taobao-item-delete" datype="btn_special_goods_delete" title="删除添加的商品">&nbsp;</dd>';
                temp += '</dl></li>';
    ...
    //删除商品
    $("[datype='btn_special_goods_delete']").live('click', function(){
        $(this).parent().parent().remove();
    });

    //插入商品列表
    $("[datype='btn_special_insert_goods']").live('click', function(){
        var html = $("#special_content").val();
        html += "<div class='special-content-spdgoods-list'>";
        var goods_list = $("<div />").append($(this).parents("div").find("[datype='_special_goods_list']").clone());
        goods_list.find("i").remove();
        goods_list.find("[datype='btn_special_goods_delete']").remove();
...
function preview(img, selection) {
    ...
    var dialog_object = $("#fwin_dialog_image_insert_hot_point");
    dialog_object.find("[datype='x1']").val(selection.x1);
    dialog_object.find("[datype='y1']").val(selection.y1);
    dialog_object.find("[datype='x2']").val(selection.x2);
    dialog_object.find("[datype='y2']").val(selection.y2);
    dialog_object.find("[datype='w']").val(selection.width);
    dialog_object.find("[datype='h']").val(selection.height);
}
