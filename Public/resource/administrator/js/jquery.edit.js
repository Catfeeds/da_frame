$(document).ready(function(){
	...
	//给需要修改的位置添加修改行为
	$('span[spd_type="inline_edit"]').click(function(){
		...
		$('<input type="text">')
                        ...
                        if(event.keyCode == 13)
                        {
							...
							//column 修改字段名
							//value 修改内容
                            $.get(_PAGE_URL + '&c='+c+'&a=ajax',{branch:ajax_branch,id:s_id,column:s_name,value:$(this).attr('value')},function(data){
                                if(data === 'false')
                                {
                                    alert('名称已经存在，请您换一个');
		...
		$('span[spd_type="inline_edit_textarea"]').click(function(){
		var s_value  = $(this).text();
		var s_name   = $(this).attr('fieldname');
		var s_id     = $(this).attr('fieldid');
		var req      = $(this).attr('required');
	...
	//给需要修改的图片添加异步修改行为
	$('img[spd_type="inline_edit"]').click(function(){
		...
		$.get(_PAGE_URL + '&c='+c+'&a=ajax',{branch:ajax_branch,id:i_id,column:i_name,value:i_val},function(data){
		if(data == 'true')
			{
				if(i_val == 0)
				{
	...
	$('a[spd_type="inline_edit"]').live('click', function() {
		var i_id    = $(this).attr('fieldid');
		var i_name  = $(this).attr('fieldname');
		var i_src   = $(this).attr('src');
		var i_val   = ($(this).attr('fieldvalue'))== 0 ? 1 : 0;
    ...
    //给每个可编辑的小图片的父元素添加可编辑标题 $('img[spd_type="inline_edit"]').parent().attr('title','可编辑');

    //给列表有排序行为的列添加鼠标手型效果
    $('span[spd_type="order_by"]').hover(function(){$(this).css({cursor:'pointer'});},function(){});
...
//新的inline_edit调用方法
//javacript
//$('span[spd_type="class_sort"]').inline_edit({c: 'microshop',a: 'update_class_sort'});
//html
//<span spd_type="class_sort" column_id="<?php echo $val['class_id'];?>" title="<?php echo $lang['spd_editable'];?>" class="editable tooltip"><?php echo $val['class_sort'];?></span>
//php
//$result = array();
 ...
 $.fn.inline_edit= function(options) {
     ...
     function onClick() {
         ...
         .blur(function(){
             ...
             if(new_value != '') {
                 $.get(_PAGE_URL + '&c='+settings.c+'&a='+settings.a,{branch:s_name,id:column_id,value:new_value},function(data){
                     data = $.parseJSON(data);
                     if(data.result) {
     ...
     function onClick() {
         ...
         $btn_submit.click(function(){
             ...
             if(new_value !== '' && new_value !== old_value) {
                 $.post(_PAGE_URL + '&c=' + settings.c + '&a=' + settings.a, {branch:s_name,id:column_id, value:new_value}, function(data) {
                     data = $.parseJSON(data);
                     if(data.result) {
                         $span.text(new_value);
                     } else {
