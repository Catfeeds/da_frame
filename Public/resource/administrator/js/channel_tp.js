...
$(function(){
	...
	$(".spdap-channel-category").sortable({ items: 'dl' });
	$(".spdap-channel-category ul").sortable({ items: 'li' });
...
function get_goods_class(){//查询子分类
	...
	if (gc_id>0) {
		...
		$.get(_PAGE_URL + '&c=WebChannel&a=get_category_list&id='+gc_id, function(data){
		  $(".spdap-channel-category").append(data);
		  $(".spdap-channel-category").sortable({ items: 'dl' });
		  $(".spdap-channel-category ul").sortable({ items: 'li' });
		  upload_gc(gc_id);
		});
...
function add_slide() {//增加图片
	for (var i = 1; i <= slide_max; i++) {//防止数组下标重复
		if (slide_obj.find("li[slide_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			var add_html = '';
			add_html = '<li slide_id="'+i+'" title="可上下拖拽更改显示顺序"><div class="title"><h4></h4><a class="spdap-btn-mini del" href="JavaScript:del_slide('+i+
			');"><i class="fa fa-trash"></i>删除</a></div><div class="focus-thumb" onclick="select_slide('+i+');" title="点击编辑选中区域内容"><img src="'+ADMIN_STATIC_URL
...
function add_adv() {//增加
	for (var i = 1; i <= adv_max; i++) {//防止数组下标重复
		if (adv_obj.find("div[focus_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			'<a class="spdap-btn-mini del" href="JavaScript:del_adv('+i+');"><i class="fa fa-trash"></i>删除</a></div><ul></ul></div>';
			...
			for (var pic_id = 1; pic_id <= pic_max; pic_id++) {
			    ...
			    text_append += '<img title="" src="'+ADMIN_STATIC_URL+'/images/picture.gif"/>';
				text_append += '</div>';
