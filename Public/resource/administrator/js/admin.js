$(function() {
	...
	subject_explain = $(".fixed-bar .item-title .subject h5").html();
	$(".fixed-bar .item-title .subject h3").attr("title", subject_explain);
	...
	$("#explanationZoom").click(function() {
		$("#explanation").remove();
	});
    ...
    /* BACK TO TOP */
		...
		$('#searchBarOpen').click(function() {
            $('.spdap-search-ban-s').animate({'right': '-40px'},200,
            function() {
                $('.spdap-search-bar').animate({'right': '0'},300);
            });
        });
        $('#searchBarClose').click(function() {
            $('.spdap-search-bar').animate({'right': '-240px'}, 300,
            function() {
                $('.spdap-search-ban-s').animate({'right': '0'},  200);
            });            
        });
		// 搜索项目过多时出现滚动条
