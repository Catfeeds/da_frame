(function($) {
	...
	$.fn.lazyshow = function(options) {
		...
		$("#lazytmp").load(loadurl+'&delaypage='+pageNo,'',function(){
			...
			//修改加载更多的html
			$(dataContainer).find("#lazymore").html('<div class="lazymore"><a href="javascript:void(0);" spd_url="'+loadurl+'">查看更多动态</a></div>');
			//修改分页连接为onclick事件
			$(dataContainer).find('.pagination').find('.demo').each(function(){
				var a_url = $(this).attr('href');
				if(a_url != '' && a_url != undefined){
					$(this).attr('spd_url',a_url);	
				}
				$(this).attr('href','javascript:void(0);');
			});
			
			// Membership card
			$(dataContainer).find('[datype="mcard"]').membershipCard({type:'shop'});
		});
		pageNo++;//延时分页自增一
    };
})(jQuery);