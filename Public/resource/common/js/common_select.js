...
function gcategoryChange()
{
    ...
    {
        var _self = this;
        var url = SITEURL + '&c=Index&a=josn_class&callback=?';
...
function show_gc_1(depth,gc_json){
	var html = '<select name="search_gc[]" id="search_gc_0" spd_type="search_gc" class="querySelect">';;
...
function show_gc_2(chooseid,gc_json){
	if(gc_json && chooseid > 0){
		...
		if(childid){
			var html = '<select name="search_gc[]" id="search_gc_'+gc_json[chooseid].depth+'" spd_type="search_gc" class="querySelect">';;
			html += ('<option value="0">请选择...</option>');
...
function init_gcselect(chooseid_json,gc_json){
	...
	//商品分类select绑定事件
	$("[spd_type='search_gc']").live('change',function(){
        $(this).nextAll("[spd_type='search_gc']").remove();
