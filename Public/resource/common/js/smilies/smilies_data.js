...
function smilies_show(id, smcols, seditorkey,textobj) {
	...
	//image绑定操作函数
	$("#smiliesdiv").find("[datype='smiliecode']").bind('click',function(){
...
function smilies_switch(id, smcols, page, seditorkey) {
	...
	for(i in smilies_array[page]) {
		...
		img[k] = new Image();
		img[k].src = smilieimg;
		smiliesdata += s && s[0] ? '<td id="' + seditorkey + 'smilie_' + s[0] + '_td" datype="smiliecode" codetext="'+s[1]+'"><img id="smilie_' + s[0] + '" width="' + s[3] +'" height="' + s[4] +'" src="' + smilieimg + '" alt="' + s[1] + '" title="'+s[6]+'" />' : '<td>';
		j++; k++;
	}
	smiliesdata += '</table>';
	/* smiliespage = '';
