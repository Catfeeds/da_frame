$(document).ready(function(){
	...
	// 退拽效果
    $('tbody[datype="bundling_data"]').sortable({ items: 'tr' });
...
function count_cost_price_sum(){
	data_price = $('td[datype="bundling_data_price"]');
	if(typeof(data_price) != 'undefined'){
		var S_price = 0;
		data_price.each(function(){
			S_price += parseFloat($(this).html());
		});
		$('span[datype="cost_price"]').html(S_price.toFixed(2));
	}else{
		$('span[datype="cost_price"]').html('');
	}
}

/* 计算商品售价 */
function count_price_sum(){
    data_price = $('input[datype="price"]');
    if(typeof(data_price) != 'undefined'){
        var S_price = 0;
        data_price.each(function(){
