...
function drop_cart_item(cart_id){
    ...
    showDialog('确认删除吗?', 'confirm', '', function(){
        $.getJSON(_PAGE_URL + '&c=Cart&a=del&cart_id=' + cart_id, function(result){
            if(result.state){
                //删除成功
                if(result.quantity == 0){//判断购物车是否为空
                    window.location.reload();    //刷新
                } else {
                    $('tr[spd_group="'+cart_id+'"]').remove();//移除本商品或本套装
...
function change_quantity(cart_id, input){
    ...
    $.getJSON(_PAGE_URL + '&c=Cart&a=update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
        ...
        if(result.state == 'invalid'){
          ...
          $('tr[spd_group="'+cart_id+'"]').addClass('item_disabled');
...
var calc_cart_price = (function() {
    ...
    var realCalculate = function() {
        ...
        obj.children('tbody').each(function(){
            //购物车每个店铺已选择商品的总价格
            var eachTotal = 0;
            $(this).find('em[spd_type="eachGoodsTotal"]').each(function(){
            ...
            allTotal += eachTotal;
            $(this).children('tr').last().find('em[spd_type="eachStoreTotal"]').eq(0).html(number_format(eachTotal,2));
        });
...
$(function(){
    ...
    $('#selectAll').on('click',function(){
        if ($(this).attr('checked')) {
            ...
            if ($('input[spd_type="eachGoodsCheckBox"]:checked').size() > 0) {
    ...
    $('input[spd_type="eachGoodsCheckBox"]').on('click',function(){
        ...
        if (!$(this).attr('checked')) {
            ...
            // 如果都选中 则全选复选框是选中的
            var b = true;
            $('input[spd_type="eachGoodsCheckBox"]').not(this).not(':disabled').each(function() {
                if (!this.checked) {
                    b = false;
                    return false;
                }
...
//        if ($(document).find('input[spd_type="eachGoodsCheckBox"]:checked').size() == 0) {
