$(function(){
    /* 筛选事件 */
    $('span[datype="span_filter"]').click(function(){
    	_i = $(this).find('i');
	...
	$('.slide_tiny').live('mouseover',function(){
		small_image = $(this).attr('datype');
		$(this).parents('.slide-show').find('img:first').attr('src',small_image);
	});

    //复选框筛选
    $("[spd_type='more-filter']").mouseover(function(){
        $("[spd_type='more-filter']").addClass('box-hover');
    });
    $("[spd_type='more-filter']").mouseout(function(){
        $("[spd_type='more-filter']").removeClass('box-hover');
    });

    //鼠标经过弹出图片信息
    $(".item").hover(
        function() {
            $(this).find(".spdgoods-info").animate({"top": "180px"}, 400, "swing");
        },function() {
            $(this).find(".spdgoods-info").stop(true,false).animate({"top": "230px"}, 400, "swing");
        }
    );
    // 加入购物车
    $(window).resize(function() {
        $('a[datype="add_cart"]').click(function() {
            flyToCart($(this));
        });
    });
    $('a[datype="add_cart"]').click(function() {
    ...
    // 图片切换效果
    $('.spdgoods-pic-scroll-show').find('a').mouseover(function(){
        $(this).parents('li:first').addClass('selected').siblings().removeClass('selected');
        var _src = $(this).find('img').attr('src');
        _src = _src.replace('_60.', '_240.');
        _src = _src.replace('-60', '-240');
        $(this).parents('.spdgoods-content').find('.spdgoods-pic').find('img').attr('src', _src);
    });
    
    // 品牌按首字母切换
    $('ul[datype="ul_initial"] > li').mouseover(function(){
        $(this).addClass('current').siblings().removeClass('current');
        if ($(this).attr('data-initial') == 'all') {
            $('ul[datype="ul_brand"] > li').show();
            return;
        }
        $('ul[datype="ul_brand"] > li').hide();
        $('ul[datype="ul_brand"] > li[data-initial="'+$(this).attr('data-initial')+'"]').show();
    });
    // 品牌显示筛选
    $('span[datype="brand_show"]').toggle(
        function(){
            $('ul[datype="ul_initial"]').show();
            $('ul[datype="ul_brand"] > li').show();
            $(this).html('<i class="icon-angle-up"></i>收起');
        },function(){
            $('ul[datype="ul_initial"]').hide();
            $('ul[datype="ul_brand"] > li:gt(13)').hide();
            $('ul[datype="ul_brand"] > li:lt(14)').show();
