$(function(){
    // 商品图片ajax上传
    $('.spdsc-upload-btn').find('input[type="file"]').unbind().bind('change', function(){
        var id = $(this).attr('id');
        ajaxFileUpload(id);
    });
    //凸显鼠标触及区域、其余区域半透明显示
    ...
    // 关闭相册
    $('a[datype="close_album"]').click(function(){
        $(this).hide();
        $(this).prev().show();
        $(this).parent().next().html('');
    });
    // 绑定点击事件
    $('div[datype^="file"]').each(function(){
    ...
    $.ajaxFileUpload({
        ...
        success : function (data, status) {
                    if (typeof(data.error) != 'undefined') {
                        alert(data.error);
                        $('img[datype="' + id + '"]').attr('src',DEFAULT_GOODS_IMAGE);
                    } else {
                        $('input[datype="' + id + '"]').val(data.name);
                        $('img[datype="' + id + '"]').attr('src', data.thumb_name);
                        selectDefaultImage($('div[datype="' + id + '"]'));      // 选择默认主图
                        checkDefaultImage($('div[datype="' + id + '"]'));
                    }
...
function selectDefaultImage($this) {
    ...
    // 删除
    $this.parents('li:first').find('a[datype="del"]').click(function(){
...
function insert_img(name, src, color_id) {
    $_thumb = $('ul[datype="ul'+ color_id +'"]').find('.upload-thumb').each(function(){
        if ($(this).find('input').val() == '') {
