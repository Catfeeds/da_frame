...
function selClass($this){
    ...
    $.getJSON(_PAGE_URL + '&c=StoreGoodsAdd&a=ajax_goods_class', {gc_id : data_str.gcid, deep: deep}, function(data) {
        if (data != null) {
            $('input[datype="buttonNextStep"]').attr('disabled', true);
...
function disabledButton() {
    if ($('#class_id').val() != '') {
        $('input[datype="buttonNextStep"]').attr('disabled', false).css('cursor', 'pointer');
    } else {
        $('input[datype="buttonNextStep"]').attr('disabled', true).css('cursor', 'auto');
    }
}
...
$(function(){
    ...
    // ajax选择分类
    $('li[datype="selClass"]').click(function(){
        selClass($(this));
    });

    // 返回分类选择
    $('a[spd_type="return_choose_sort"]').unbind().click(function(){
        $('#class_id').val('');
        $('#t_id').val('');
        $("#commodityspan").show();
        $("#commoditydt").hide();
    ...
    // 常用分类选择
    $('#commListArea').find('span[datype="staple_name"]').die().live('click',function() {
        $('#dataLoading').show();
        $('.wp_category_list').addClass('blank');
        $this = $(this);
        eval('var data_str = ' + $this.parents('li').attr('data-param'));
        $.getJSON(_PAGE_URL + '&c=StoreGoodsAdd&a=ajax_show_comm&stapleid=' + data_str.stapleid, function(data) {
            if (data.done) {
                ...
                // 绑定ajax选择分类事件
                $('#class_div').find('li[datype="selClass"]').click(function(){
                    selClass($(this));
                });
    ...
    // ajax删除常用分类
    $('#commListArea').find('a[datype="del-comm-cate"]').die().live('click',function() {
        $this = $(this);
        eval('var data_str = ' + $this.parents('li').attr('data-param'));
        $.getJSON(_PAGE_URL + '&c=StoreGoodsAdd&a=ajax_stapledel&staple_id='+ data_str.stapleid, function(data) {
            if (data.done) {
