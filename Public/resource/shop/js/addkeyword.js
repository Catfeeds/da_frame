...
function zsc_show(id){
	...
	if(zsc_class!='add'){
		...
		var url = _PAGE_URL + "&c=SellerWechat&a=addkeyword_ajax&kid="+zsc_id;
...
function btnsave(num,count){
	...
	if(ktype==1){
		...
		if(count==0){
			...
			var url = _PAGE_URL + "&c=SellerWechat&a=keyword_index&opt=add";
			...
			var url = _PAGE_URL + "&c=SellerWechat&a=keyword_index&opt=update";
			var kid = $('#zsc_iid').val();
...
function zscdel(num){
	...
	var url = _PAGE_URL + "&c=SellerWechat&a=keyword_index&opt=del";
		   ...
		   (
			   {
					...
					url: _PAGE_URL + "&c=SellerWechat&a=upload_fm",
