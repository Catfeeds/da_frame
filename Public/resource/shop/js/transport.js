$(function(){
	...
	TransTpl += "全选<\/label>\n";
	TransTpl += "&nbsp;<a class=\"J_BatchSet spdbtn-mini\" href=\"JavaScript:void(0);\">批量设置<\/a>\n";
	TransTpl += "<a class=\"J_BatchDel spdbtn-mini\" href=\"JavaScript:void(0);\">批量删除<\/a>\n";
	TransTpl += "<\/div>\n";
	TransTpl += "<div class=\"tbl-attach\">\n";
	TransTpl += "<div class=\"J_SpecialMessage\"><\/div>\n";
	TransTpl += "<a class=\"J_AddRule spdbtn-mini ml5\" href=\"JavaScript:void(0);\">";
	TransTpl += "<i class=\"icon-map-marker\"></i>为指定地区城市设置运费<\/a>\n";
	TransTpl += "<a class=\"J_ToggleBatch spdbtn-mini\" style=\"display:none\" href=\"JavaScript:void(0);\">";
	...
	//头模板
	RuleHead += "<table cellspacing=\"0\" class=\"spdsc-table-style\" style=\"width:100%\" cellpadding=\"0\" border=\"0\">\n";
	RuleHead += "<thead>\n";
...
/*	选择完区域后，确定事件*/
	$('#dialog_areas').on('click','.J_Submit',function (){
		...
		//首先找市被全部选择的省份
		$('#J_CityList').find('.spdsc-province-tab').each(function(){
		...
		$('#J_CityList').find('.spdsc-province-list').find('input[type="checkbox"]').each(function(){
			if ($(this).attr('checked')){
				CityValue += $(this).val()+',';
			}
		});
...
/*	省份点击事件*/
	$('#dialog_areas').on('click','.J_Province',function(){
		if ($(this).attr('checked')){
			//选择所有未被disabled的子地区
			$(this).parent().find('.spdsc-citys-sub').eq(0).find('input[type="checkbox"]').each(function(){
			...
			//计算并显示所有被选中的子地区数量
			num = '('+$(this).parent().find('.spdsc-citys-sub').eq(0).find('input:checked').size()+')';
			if (num == '(0)') num = '';
			...
			$(this).parent().find('.spdsc-citys-sub').eq(0).find('input[type="checkbox"]').attr('checked',false);
			//取消大区域选择
...
/*	大区域点击事件（华北、华东、华南...）*/
	$('#dialog_areas').on('click','.J_Group',function(){
		if ($(this).attr('checked')){
			...
			//循环显示每个省下面的市级的数量
			$(this).parent().parent().parent().find('.spdsc-province-list').find('.spdsc-province').each(function(){
				//显示该省下面已选择的市的数量
				num = '('+$(this).find('.spdsc-citys-sub').find('input:checked').size()+')';
				//如果是0，说明没有选择，不显示数量
				if (num != '(0)'){
...
/*	省份下拉事件*/
	$('#dialog_areas').on('click','.trigger',function () {
		...
		if ($(this).next().css('display') == 'none'){
			...
			$('.ks-contentbox').find('.spdsc-province').removeClass('showCityPop');
...
//	                if(target.getAttribute("id")==$(objPanel).attr('id'))return true;
	...
	$('.trans-line').on('click','a[entype="J_EditArea"]',function () {
		...
		try{
			...
			//初始化已选中的checkbox
			$('#J_CityList').find('.spdsc-province').each(function(){
				var count = 0;
