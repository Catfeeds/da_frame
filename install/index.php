...
define ( 'DBCHARSET', 'UTF8' ); // 编码 分为 UTF8 GBK，强制使用UTF8
input ( $_GET );
input ( $_POST );

// define variable
$html_title = 'ShopDa大商城 电商系统安装程序';
$version = "V1.0 2017.03.20";
...
$html_footer = <<<EOF
<div class="footer">
  <h6><a href="http://www.shopda.cn" target="_blank">程序来源于 ShopDa大商城 shopda.cn</a></h6>
</div>
EOF;
...
switch ($_GET ['step']) {
	...
	case 3 :
		$install_error = '';
		$install_recover = '';
		$demo_data = file_exists ( './data/sql/utf8_add.sql' ) ? true : false;
...
function step3(&$install_error, &$install_recover) {
	...
	global $html_title, $html_header, $html_footer, $https_open;
	...
	$auto_site_url = strtolower ( $schema . $_SERVER ['HTTP_HOST'] . $sitepath );
...
// 	exit;
	...
	$sql = "";
	$sql_file_list = scan_dir(dirname(__FILE__) . "/data/sql");
	
// 	var_dump($_POST['demo_data']);
// 	var_dump($sql_file_list);
// 	exit;
	
	foreach ($sql_file_list as $sql_file)
	{
		if (strstr(basename($sql_file), "_demo_data.sql"))
		{
			if ($_POST ['demo_data'] == '1') //演示数据
			{
	 ...
	 */
	$md5_key = md5 ( random ( 4 ) . substr ( md5 ( $_SERVER ['SERVER_ADDR'] . $_SERVER ['HTTP_USER_AGENT'] . 
			$db_host . $db_user . $db_pwd . $db_name . substr ( time (), 0, 6 ) ), 8, 6 ) . random ( 10 ) );
...
function write_config($url) {
	...
	$admin = $_POST ['admin'];
	$password = $_POST ['password'];
	$https_open = ($_POST['https_open'] == "1") ? "true" : "false";

	$cookie_pre = strtoupper ( substr ( md5 ( random ( 6 ) . substr ( $_SERVER ['HTTP_USER_AGENT'] . 
			md5 ( $_SERVER ['SERVER_ADDR'] . $db_host . $db_user . $db_pwd . 
					$db_name . substr ( time (), 0, 6 ) ), 8, 6 ) . random ( 5 ) ), 0, 4 ) ) . '_';
	...
	$configfile = str_replace ( "===https_open===", $https_open, $configfile );
	
	//var_dump($configfile);
	file_put_contents ( '../Administrator/Common/Conf/config.php', $configfile );
	file_put_contents ( '../Application/Common/Conf/config.php', $configfile );
}
...
function scan_dir($dir) {
	...
	if ((! is_dir ( $dir )) && (! is_file ( $dir ))) {
		...
		foreach ( $children as $child ) {
			if ($child !== '.' && $child !== '..') {
				if (empty ( $ret )) {
					$ret = scan_dir ( $dir . DIRECTORY_SEPARATOR . $child );
				} else {
					$ret = array_merge ( $ret, scan_dir ( $dir . DIRECTORY_SEPARATOR . $child ) );
				}
			}
		}
	}
